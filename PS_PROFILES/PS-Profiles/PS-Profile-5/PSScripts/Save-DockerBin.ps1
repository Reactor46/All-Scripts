...
if ($latest.body | where { $_ -match "https://get.docker.com/builds/Windows/x86_64/.*.zip" }) {
	...
	Expand-Archive $outFile -DestinationPath $HOME\Downloads -Verbose
	Move-Item $HOME\Downloads\docker\docker.exe $HOME\Scripts

	Remove-Item $HOME\Downloads\docker -Recurse
	Remove-Item $outFile
}
