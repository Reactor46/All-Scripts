...
function UpgradeSite($path){
	...
	Upgrade-SPSite $path -VersionUpgrade -Unthrottled
	...
	do{
		...
		switch($upgradeSessionInfo.Status){
			...
			{      
				...
				try{Enable-spfeature -Identity 7dd46779-0e3b-4b6b-a660-b988e9ce1e0c -url $path -ErrorAction Stop}
				...
				try
				{
					#Pulse Branding Feature
					Enable-spfeature -Identity 61661bcf-2b71-4d88-a13c-62efb146999a -url $path -ErrorAction Stop
				}
					catch [System.Management.Automation.ActionPreferenceStopException]
				{
...
Function UpdateMasterPage($path){
	...
	foreach ($web in $site.AllWebs) {
		$masterPath = $web.ServerRelativeUrl +"/_catalogs/masterpage/Internal_Pulse.master";
        $systemMasterPath = $web.ServerRelativeUrl +"/_catalogs/masterpage/Upgrade2013.master";
		Echo "Master Path: " $masterPath
		$web.CustomMasterUrl =$masterPath; 
		$web.MasterUrl =$systemMasterPath; 
		$web.Update(); 
		Write-host "Pulse.master has been applied to"$web.Url  -foregroundcolor black -backgroundcolor green
	}
	$site.Dispose()
}

[xml]$s = get-content UpgradeSitesPulseINT.xml
Write-Host " "
Write-Host " "
