...
function GetSearchTopo()
{
    ...
    "#################################################################################################################"
    ""
    " Server    ||    Name       ||    ServerId       ||      ComponentId       ||   RootDirectory    ||   IndexPartitionOrdinal      "
    ""
    ...
    foreach($sComponent in $sComponents)
    {
                
        if($sComponent.Name -match "Index")
        {
            $sComponent.ServerName + " || " + $sComponent.Name + " || " + $sComponent.ServerId + " || " + $sComponent.ComponentId + " || " + $sComponent.RootDirectory + " || " + $sComponent.IndexPartitionOrdinal
        }
        else
        {
            $sComponent.ServerName + " || " + $sComponent.Name + " || " + $sComponent.ServerId + " || " + $sComponent.ComponentId
        }
    }
}
...
function GetSPVersion()
{
    If($farm.BuildVersion.Major -eq 16 -or $farm.BuildVersion.Major -eq 15)
    {
        ...
        GetSSA
        Get-Date | Out-File -Width 2000 $outputfile -Append
        GetFarmBuild | Out-File -Width 2000 $outputfile -Append
        GetServersInFarm |  ft -auto | Out-File -Width 2000 $outputfile -Append
        GetServiceInstances | Out-File -Width 2000 $outputfile -Append
        GetServiceApplications | Out-File -Width 2000 $outputfile -Append
        CheckTimerServiceInstances | Out-File -Width 2000 $outputfile -Append
        CheckAdminServiceInstance | Out-File -Width 2000 $outputfile -Append
        CheckTimerJobHistory | Out-File -Width 2000 $outputfile -Append
        CheckFarmSiteSubscriptions | Out-File -Width 2000 $outputfile -Append
        checkIfPartitioned | Out-File -Width 2000 $outputfile -Append
        GetSSARelatedTimerJobs | Out-File -Width 2000 $outputfile -Append
        GetSSAFullObject | Out-File -Width 2000 $outputfile -Append
        GetSSALegacyAdminComponent | Out-File -Width 2000 $outputfile -Append
        CheckCpcFromMssConfiguration |  Out-File -Width 2000 $outputfile -Append
        GetSearchTopo | Out-File -Width 2000 $outputfile -Append
        GetContentSources | Out-File -Width 2000 $outputfile -Append
        GetServerNameMappings | Out-File -Width 2000 $outputfile -Append
        GetCrawlRules | fl | Out-File -Width 2000 $outputfile -Append
        displayGlobalSearchService | Out-File -Width 2000 $outputfile -Append
        GetSSIs | Out-File -Width 2000 $outputfile -Append
        GetSQSS | Out-File -Width 2000 $outputfile -Append
        VerifyServiceEndpoints | Out-File -Width 2000 $outputfile -Append
        GetWebAppAndAamInfo | Out-File -Width 2000 $outputfile -Append
        VerifyApplicationServerSyncJobsEnabled | Out-File -Width 2000 $outputfile -Append
        HealthCheck | Out-File -Width 2000 $outputfile -Append
        ""
