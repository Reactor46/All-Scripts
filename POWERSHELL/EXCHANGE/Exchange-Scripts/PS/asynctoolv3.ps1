...
$FldHash = @{}
$usHash = @{} 
$fieldsline = (Get-Content $fnFileNamelableBox.Text)[3]
write-host $fieldsline
...
get-content $fnFileNamelableBox.Text | Where-Object -FilterScript { $_ -ilike “*&Cmd=*” } | %{ 
	$lnum ++
	...
	if ($linarr[0].substring(0, 1) -ne "#"){
		 ...
		 if ($linarr.Length -gt 8){$lfSourceIP= $linarr[$FldHash["c-ip"]]}
		 if ($linarr.Length -gt 3){$lfMethod  = $linarr[$FldHash["cs-method"]]}
		 if ($linarr.Length -gt 6){$lfAction = $linarr[$FldHash["cs-uri-query"]]}
		 if ($linarr.Length -gt 7){$lfUserName  = $linarr[$FldHash["cs-username"]]}
		 if ($linarr.Length -gt 10){$lfPPCversion = $linarr[$FldHash["cs(User-Agent)"]]}
		 if ($linarr.Length -gt 15){$bcSent = $linarr[$FldHash["sc-bytes"]]}
		 if ($linarr.Length -gt 16){$bcRecieved = $linarr[$FldHash["cs-bytes"]]}
