...
Function Get-Folders($Service)
{
	...
	if($archiveCheck)
	{
        Write-Log ("====>  MAILBOX: {0} | Finding WellKnownFolders in ArchiveMsgFolderRoot (Class Filter Enabled)" -f $MBX)
		Try
        {
...
"@
    ...
    elseif($InPlaceHold)
	{
        Write-Log ("====>  MAILBOX: {0} | Finding WellKnownFolders in RecoverableItemsRoot (No Class Filter)" -f $MBX)
		Try
        {
...
"@
	...
	{
        Write-Log ("====>  MAILBOX: {0} | Finding WellKnownFolders in MsgFolderRoot (Class Filter Enabled)" -f $MBX)
        Try
        {
            ...
            Write-Log ("====>  MAILBOX: {0} | Found {1} folders (PRIMARY MAILBOX)" -f $MBX,$Folders.TotalCount);
        }
	    Catch [Microsoft.Exchange.WebServices.Data.ServiceResponseException]
	    {
            $myError = @"
...
Try
{
    ...
    #Write-Log "====>  Looking for Microsoft Exchange Web Services Managed API 2.2"
    ...
    #    [System.Console]::ForegroundColor = "Yellow"
    #    Write-Host "`n`r||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||"
    #    Write-Host "||||||||||`n`r||||||||||  Microsoft Exchange Web Services Managed API 2.2 cannot be found or is not installed.`n`r||||||||||  Download EWS API 2.2 here:  http://www.microsoft.com/en-us/download/details.aspx?id=42951`n`r||||||||||"
    #    Write-Host "||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||";
    #    [System.Console]::ResetColor()
    #
	#	Write-Log "====>  WARNING  <==== | Microsoft Exchange Web Services Managed API 2.2 was not found, Script will now exit"
    #   Exit 1
	#}
