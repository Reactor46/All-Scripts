<#
.EXTERNALHELP GetSearchIndexForDatabase-help.xml
#>
...
Import-LocalizedData -BindingVariable GetSIForDatabase_LocalizedStrings -FileName GetSearchIndexForDatabase.strings.psd1
...
{
	#
	# If the script does have piped input, no arguments are allowed 
	#
	write-host($GetSIForDatabase_LocalizedStrings.res_0000)
	exit
}
...
if (!$args)
{
	...
	{
		#
		# Populate mbdbList with the list of Mailbox databases
		#
		$mbdbList = @($input | foreach { $_.Name} )
	}
	else
	{
		#
		# No arguments, no piped input, provide usage and exit
		#
		exit
	}
}
else
{
	foreach ($a in $args)
	{
		...
		{
			#
			# Populate mbdbList with all MDBs in organization
			#
			$mbdbList = @(get-mailboxdatabase | foreach { $_.Name} )
			break
		}
		else 
		{
			#
			# Populate mbdbList with database names based on arguments provided
			#
...
# to the Mailbox Database is found and displayed
#
for ($n = 0; $n -lt $mbdbList.Length; ++$n)
{
	$mbdb = @(get-mailboxdatabase $mbdbList[$n])[0]
	$edbFilePath = $mbdb.EdbFilePath.PathName
	$i = $edbFilePath.LastIndexOf('\')
	$edbFilePath = $edbFilePath.Remove($i)
	if ($mbdb -eq $null) {
		write-host ($GetSIForDatabase_LocalizedStrings.res_0001 -f $mbdbList[$n])
	} 
	else 
	{
		$guid = $mbdb.Identity.ObjectGuid.ToString()
		$dir=get-childitem $edbFilePath | where { $_.Name.Contains($guid) } | foreach { $_.Name }
		write-host ([system.environment]::newline+$GetSIForDatabase_LocalizedStrings.res_0002+$mbdblist[$n])
		if($dir)
		{
			write-host ($GetSIForDatabase_LocalizedStrings.res_0003 + $edbFilePath +'\'+ $dir)
		} else 
		{
			write-host ($GetSIForDatabase_LocalizedStrings.res_0004)
		}
		write-host(" ")
	}
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUCuDR0SUsJXn3uUxasoMVvkYz
# u3ugghUvMIIEmTCCA4GgAwIBAgITMwAAAJ0ejSeuuPPYOAABAAAAnTANBgkqhkiG
...
# a8kTo/0xggS7MIIEtwIBATCBkDB5MQswCQYDVQQGEwJVUzETMBEGA1UECBMKV2Fz
# aGluZ3RvbjEQMA4GA1UEBxMHUmVkbW9uZDEeMBwGA1UEChMVTWljcm9zb2Z0IENv
# cnBvcmF0aW9uMSMwIQYDVQQDExpNaWNyb3NvZnQgQ29kZSBTaWduaW5nIFBDQQIT
# MwAAAJ0ejSeuuPPYOAABAAAAnTAJBgUrDgMCGgUAoIHUMBkGCSqGSIb3DQEJAzEM
# BgorBgEEAYI3AgEEMBwGCisGAQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCS