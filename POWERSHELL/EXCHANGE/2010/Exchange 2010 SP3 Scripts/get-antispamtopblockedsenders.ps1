...
# 
# Synopsis: Gets the top N senders that were blocked by antispam agents.
#
# This script takes the following parameters. 
#    startDate: Beginning time period to look for in the agent log files. If not specified, look at all the logs till the endDate.
#    endDate: End time period to look for in the agent log files. If not specified, look at all the logs beginning from startDate.
#             If neither of the dates is specified, look at all the log files.
#    location: Directory location for the agent log files. If not specified, use the default log file location under
#               %INSTALL_DIR%\TransportRoles\Logs\AgentLog (where %INSTALL_DIR% is the installation root for the Exchange server).
#    top: Find the top N blocked senders. By default (i.e. if not specified), N = 10.
#    command: Mandatory parameter. Specify p1 to identify P1 senders and p2 to identify P2 senders.
#
# Usage:
#
#    .\get-AntispamTopBlockedSenders p1 -location c:\Logs
#

param (
    [string] $report = $(throw "Specify p1 or p2"),
    $location = $null,
...
## Get all senders, then group and sort them

switch ($report) {
 "p1"
 {
  get-AgentLog -location:$location -startDate:$startDate -endDate:$endDate |
    where { $_.Action -ne "AcceptMessage" -and $_.P1FromAddress -ne $() } |
    foreach {$_.P1FromAddress} |  
    where { $_.IsValid } | 
    foreach { $h = @{} } { $h[ [string]$_ ]++ } `
    { 
 ...
 {
  get-AgentLog -location:$location -startDate:$startDate -endDate:$endDate |
    where { $_.Action -ne "AcceptMessage" -and $_.P2FromAddresses -ne $() } |
    foreach {$_.P2FromAddresses} |  
    where { $_.IsValid } |  
    foreach { $h = @{} } { $h[ [string]$_ ]++ } `
    { 
      $arr = new-object object[] $h.keys.count 
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUmiW1qNff642Sx9+PD85yLDSh
# hl2gghUmMIIEmTCCA4GgAwIBAgITMwAAAJ0ejSeuuPPYOAABAAAAnTANBgkqhkiG
...
# mu5J4PcBZW+JC33Iacjmbuqnl84xKf8OxVtc2E0bodj6L54/LlUWa8kTo/0xggS6
# MIIEtgIBATCBkDB5MQswCQYDVQQGEwJVUzETMBEGA1UECBMKV2FzaGluZ3RvbjEQ
# MA4GA1UEBxMHUmVkbW9uZDEeMBwGA1UEChMVTWljcm9zb2Z0IENvcnBvcmF0aW9u
...
# BBTYiNconbWB0xWskKod1n0gEUpNNjB8BgorBgEEAYI3AgEMMW4wbKBEgEIAZwBl
# AHQALQBBAG4AdABpAHMAcABhAG0AVABvAHAAQgBsAG8AYwBrAGUAZABTAGUAbgBk
# AGUAcgBzAC4AcABzADGhJIAiaHR0cDovL3d3dy5taWNyb3NvZnQuY29tL2V4Y2hh
# bmdlIDANBgkqhkiG9w0BAQEFAASCAQCdN93rqDjDa05MbKtwTpdx9bx5Ue6WV6E3
# N3Lvmlsb3BS+0rETzMCNq9Cd+mMS4RXntLIi1/einCiE2uNAmnWndTxtHUz0Buxf
# A0o/q+Lhq8CM5YHDVwLqw321ydxsWWcqWjjgpBAF8gAmyKtL5yRrnFVS95BNvQbQ
# 6Ypv81mtKXqaC5xzu+6UXIuVabx5fJsA/EAvLl3qxDLlyk0TrMvenMBXBpiJCavL
# PAau9+otVqV46Hj2BrgSlzTTH+dvLaVpVKWtdXFdna4I38mew3VlEbpiu/wQz+fD
# si40c2hTEvolOLOMmm71ctc94X1F51crmHpDQMZCBok88YDzFYuOoYICHzCCAhsG
# CSqGSIb3DQEJBjGCAgwwggIIAgEBMIGFMHcxCzAJBgNVBAYTAlVTMRMwEQYDVQQI
# EwpXYXNoaW5ndG9uMRAwDgYDVQQHEwdSZWRtb25kMR4wHAYDVQQKExVNaWNyb3Nv
# ZnQgQ29ycG9yYXRpb24xITAfBgNVBAMTGE1pY3Jvc29mdCBUaW1lLVN0YW1wIFBD
# QQIKYQKOQgAAAAAAHzAJBgUrDgMCGgUAoF0wGAYJKoZIhvcNAQkDMQsGCSqGSIb3
# DQEHATAcBgkqhkiG9w0BCQUxDxcNMTMwMTA4MDg0NzA1WjAjBgkqhkiG9w0BCQQx
# FgQUWzsm1/Tz7b/XDKMW6CXodlWFBn4wDQYJKoZIhvcNAQEFBQAEggEAOuv60w/c
# kEZ5SPFx4OHxxSO+pWc2FYTko8cvZHgma0tpFXLcWZswgH4i3BQn5EsyHU1/lNLt
# kdcOoQgForgDUOhetHZySdgf0PTeCJHLS64OZ1GTy41xKuu+QwTzu46Itqirl8+I
# O0UkRvzjx3vxhF6JRW4CORPSeGOlDZh9E0LrFqg1rh3q9cjgvvGcbwdK4STWPyRq
# 2uGtJ0MMl6B6+DGg13mWHlxaFXAIapZBvfRgK2K5JYtywhPMdPQxJ6FnR91DvIYy
# ACqk4KLlb8y0xdtaD98wC4YQHJIcylkRiNmWm9IrbwJaGMXptp/h6gLNevKlENQA
# L2C7D31RwT9ukA==
