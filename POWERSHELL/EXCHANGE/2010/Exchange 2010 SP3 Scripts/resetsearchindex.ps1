<#
.EXTERNALHELP ResetSearchIndex-help.xml
#>
...
Import-LocalizedData -BindingVariable ResetSI_LocalizedStrings -FileName ResetSearchIndex.strings.psd1
...
#
# Find out if this script is receiving piped input
#
if ( $input.movenext() )
{
    $inputExists = $true
...
if ($inputExists -and $args)
{
	#
	# If the script does have piped input, the only argument allowed is -force
	#	
	if ($args[0] -eq '-force')
	{
		$confirmation=$false
	...
	{
	$ResetSI_LocalizedStrings.res_0000
	exit
	}
}
...
{
	#
	# If the script doesn't have piped input and does have arguments:
	...
	foreach ($a in $args)
	{
		...
		if ($a -ieq '-all')
		{
			$mbdbList = @(get-mailboxdatabase)
			break
		}
		else 
		{
			$mdbs = get-mailboxdatabase $a
			if($mdbs -eq $null)
			{
				$ResetSI_LocalizedStrings.res_0001+ $a
				exit
			}
			foreach($mdb in $mdbs)
			{
...
{
	#
	# Piped input exists, and there are no arguments: 
	# proceed to populate the Mailbox database list
	#
	if ($inputExists)
	{
		$mbdbList = @($input)
	...
	{
		#	
		# There is no piped input, neither or there Arguments - provide usage and exit
		#
		exit
	}
}
...
#
# This is the meat of the script - where the index dir corresponding 
# to the Mailbox Database is found/deleted with or without confirmation, 
# depending on whether -force was issued or not
#
stop-service MSExchangeSearch
if ((get-service MSExchangeSearch).status -ne 'stopped')
{
	$ResetSI_LocalizedStrings.res_0002
	exit
}
else
{
	$ResetSI_LocalizedStrings.res_0003
}

for ($n = 0; $n -lt $mbdbList.Length; ++$n)
{
	$ResetSI_LocalizedStrings.res_0004 + $mbdbList[$n]
	$getmdb = @(get-mailboxdatabase $mbdbList[$n])
	if($getmdb.Length -ne "1")
	{
		$ResetSI_LocalizedStrings.res_0005+$mbdbList[$n]
		exit
	}
	$mbdb = @($getmdb)[0]
	$edbFilePath = $mbdb.EdbFilePath.PathName
	$i = $edbFilePath.LastIndexOf('\')
	$edbFilePath = $edbFilePath.Remove($i+1)
	
	if ($mbdb -eq $null) 
	{
		write-host ($ResetSI_LocalizedStrings.res_0006 -f $mbdbList[$n])
	} 
	else 
	{
		# find all catalog directories that mention the guid of our mdb and delete them
		# Every clean exchange install causes a new catalog directory to be created because
		# a new exsearch instance guid is created

		$guid = $mbdb.Identity.ObjectGuid.ToString()

		$dirs = @(get-childitem $edbFilePath | where { $_.Name.Contains($guid) } )
		if ($dirs.Count -eq 0) 
		{ 
			write-host ($ResetSI_LocalizedStrings.res_0007 + $mbdb.Name)
		} 
		$dirs | foreach {
			$dir = $_.Name
			$dirToRemove = $edbFilePath + $dir
			" "
			"removing: "+ $dirToRemove
			" "
			if( $confirmation -eq $true)
			{
				read-host $ResetSI_LocalizedStrings.res_0008
				" "
			}
			remove-item $dirToRemove -force -confirm:$false -recurse
...
if ((get-service MSExchangeSearch).status -ne 'running')
{
	$ResetSI_LocalizedStrings.res_0009
	exit
}
else
{
	$ResetSI_LocalizedStrings.res_0010
}					 

# SIG # Begin signature block
# MIIabgYJKoZIhvcNAQcCoIIaXzCCGlsCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUOwVnXdTuaBXyYBDXU84RwuVp
# gN2gghUvMIIEmTCCA4GgAwIBAgITMwAAAJ0ejSeuuPPYOAABAAAAnTANBgkqhkiG
...
# SIb3DQEJBDEWBBRLj8duBQMF/rzISm8Mgh+/YyPSajBiBgorBgEEAYI3AgEMMVQw
# UqAqgCgAUgBlAHMAZQB0AFMAZQBhAHIAYwBoAEkAbgBkAGUAeAAuAHAAcwAxoSSA
# Imh0dHA6Ly93d3cubWljcm9zb2Z0LmNvbS9leGNoYW5nZSAwDQYJKoZIhvcNAQEB
# BQAEggEAirPWcV8bRPtEXOsC3Kt10d4DM8P9bDfOqSry6o6poWivk3+X4maXLMSB
# 9dGzPvX2sZCuJNfPs60zeRbwBI+ig5+MIbQfArl0GqA3Tf1UkoEbG+roQ9Ixlesz
# PJu8ZKcfPqta7UGesjt24gkqMXEWBPCTJSiTSswZP3+eh4Ztv/ubru5Wdz7m8q+F
# TQXiV1NBnrj1xgP/44xibzqjqcxTK99pRGFzjvo+Yr54crSIhVeO/Fl6V+MOwLr2
# RFcGi2twJBP5dFgn8UCy2UIjQ+kzuJO+gDukUOqmP6URB27Sbscb9TDBUkw3Wci7
# X87RCbV4RNhcGUThx+VW6GOLhpEhK6GCAigwggIkBgkqhkiG9w0BCQYxggIVMIIC
...
# KoZIhvcNAQkFMQ8XDTEzMDIwNTA2MzcyNlowIwYJKoZIhvcNAQkEMRYEFLRF8g87
# 0Tqj6T4zb3XM777ArOzVMA0GCSqGSIb3DQEBBQUABIIBAKBb5ECXTt6OJk7DpCoh
# DqwMxRQmmiCpKAwkAn3d1rAjTXHw/+oBoaE1XOCPEy+2o2wO59uJR1RgJwAjRrh5
# GjDJwwSNcx8pJjZ14zj8yoUrNa1s1qkBzNKvco79SfqxmJpWf8f5S01bjp8Vm8mK
# qQpWLQQRhT1AnU6ZmWnkP2AcKLyZj0aFScBgJYHSne4AKj515x9Yr6A4VSd7+Q0O
# 7A8gBwi9ujp+zqKzsosiA59RYn2SMnExiZkNEkMDgKv6+1bU58ZHtgbrsvVgzQ2a
# bk4mwFB9+bhLBhpT4Yzkx5SSHU7o/vhVxpqrkk4kmsXRwil+l77NclxFnmPLecKg
# HAc=
