strDNofStore = "LDAP://CN=Mailbox Store (MGNDEV2),CN=First Storage Group ..."
Set objstore= GetObject(strDNofStore)
Set objSecurityDescriptor = objstore.Get("ntSecurityDescriptor")
iCurrentControl = objSecurityDescriptor.Control
objSecurityDescriptor.Control = iCurrentControl Or SE_DACL_PROTECTED
Set objParentSD = objstore.Get("ntSecurityDescriptor")
Set objParentDACL = objParentSD.DiscretionaryAcl
Set objCopyDACL = objParentDACL.CopyAccessList()

AddAce objCopyDACL, "domain\user", 983551, 0, 2, 0,"" ,""
Set objNewDACL = ReorderACL(objCopyDACL)
objSecurityDescriptor.DiscretionaryAcl = objNewDACL
objstore.Put "ntSecurityDescriptor", objSecurityDescriptor
objstore.SetInfo
