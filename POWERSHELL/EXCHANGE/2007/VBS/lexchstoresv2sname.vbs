...
While Not Rs.EOF
		...
		Wscript.echo strservername & " " & Rs.Fields("name") & "# Mailboxes: " & mbnum & " EDBSize(GB): " & edbsize & " STMSize(GB): " & stmsize
...
While Not Rs1.EOF
		...
		stmpercentleft = FormatNumber((stmfreespace/stmtotalspace)*100, 0)
		strservername = mid(objmailstore.msExchOwningServer,4,instr(objmailstore.msExchOwningServer,",")-4)
		Wscript.echo strservername & " " & Rs1.Fields("name") & " EDBSize(GB): " & edbsize & " STMSize(GB): " & stmsize
		wscript.echo "Freespace on EDB Drive (" & left(objmailstore.msExchEDBFile,1) & ":) :" &  edbfreespace & " GB	Percent Left :" & edbpercentleft & " %"
		wscript.echo "Freespace on STM Drive (" & left(objmailstore.msExchSLVFile,1) & ":) :" &  stmfreespace & " GB	Percent Left :" & stmpercentleft & " %"
		wscript.echo 
		Rs1.MoveNext
