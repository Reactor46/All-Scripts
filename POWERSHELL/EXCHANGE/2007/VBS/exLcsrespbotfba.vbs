...
while not rs.eof
	...
	lchk = FBALogon(exservername,emailaddress)
	calinfo = CalendarQuery(exservername,emailaddress,lchk)
	calinfo = rs.fields("displayname").value &  vbcrlf & calinfo 
	mailinfo = emailQuery(exservername,emailaddress,fromemail,lchk)
...
function CalendarQuery(exservername,Alias,Logonchk)
...
strURL = "https://" & exservername & "/exchange/" & Alias & "/calendar/"
...
exreq.open "SEARCH", strURL, false, "", ""
exreq.setrequestheader "Content-Type", "text/xml"
exreq.SetRequestHeader "cookie", Logonchk(0)
exreq.SetRequestHeader "cookie", Logonchk(1)
...
function EmailQuery(exservername,Alias,fromemail,Logonchk)


dtListFrom = isodateit(DateAdd("d",-3,now()))
strURL = "https://" & exservername & "/exchange/" & Alias & "/inbox/"
strQuery = "<?xml version=""1.0""?><D:searchrequest xmlns:D = ""DAV:"" >"
...
Else
End If
strURL = "https://" & exservername & "/exchange/" & Alias & "/sent items/"
strQuery = "<?xml version=""1.0""?><D:searchrequest xmlns:D = ""DAV:"" >"
strQuery = strQuery & "<D:sql>SELECT  ""DAV:Displayname"", ""urn:schemas:httpmail:subject"", ""urn:schemas:httpmail:datereceived"" "
strQuery = strQuery & "FROM scope('shallow traversal of """
...
function FBALogon(exservername,emailaddress)

szXml = "destination=https://" & exservername & "/exchange/" & emailaddress & "/&flags=0&username=" & username 
szXml = szXml & "&password=" & password & "&SubmitCreds=Log On&forcedownlevel=0&trusted=0"
set req = createobject("microsoft.xmlhttp")
req.Open "post", "https://" & exservername & "/exchweb/bin/auth/owaauth.dll", False
req.send szXml
dim chkary(1)
exreqhedrarry = split(req.GetAllResponseHeaders(), vbCrLf,-1,1)
for c = lbound(exreqhedrarry) to ubound(exreqhedrarry)
	if instr(lcase(exreqhedrarry(c)),"set-cookie: sessionid=") then chkary(0) = right(exreqhedrarry(c),len(exreqhedrarry(c))-12)
	if instr(lcase(exreqhedrarry(c)),"set-cookie: cadata=") then chkary(1) = right(exreqhedrarry(c),len(exreqhedrarry(c))-12)
next
FBALogon = chkary
