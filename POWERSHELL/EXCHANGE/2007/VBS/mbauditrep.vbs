...
while not rs.eof	
	...
	strQuery = "<LDAP://"  & strDefaultNamingContext & ">;" & GALQueryFilter & ";distinguishedName,proxyaddresses,mailnickname;subtree"
	com.Properties("Page Size") = 100
	Com.CommandText = strQuery
	Set Rs1 = Com.Execute
	while not Rs1.eof
		proxyadd = rs1.fields("proxyaddresses").value
		for each emailadd in proxyadd
			if instr(emailadd,"SMTP") Then 
				call mbaudit(servername,mid(emailadd,6,(instr(emailadd,"@")-6))) 
			end if
		next
		rs1.movenext
	wend
...
sub mbaudit(Servername,mailboxname)
...
sConnString = "http://" & servername & "/exchange/" & mailboxname & "/NON_IPM_SUBTREE"
