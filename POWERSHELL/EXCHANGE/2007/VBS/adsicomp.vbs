...
wfile.writeline("Username,Company,StorageQuoata(MB),OverLimitQuota(MB),HardLimitQuota(MB),Mailbox Size(MB),Percent Used")
...
strSQL = "SHAPE APPEND" & _
           ...
           "      NEW adVarChar(255) AS MOADCompany, " & _
...
Set Rs = Com.Execute
while not rs.eof	
	sgQuery = "<LDAP://" & strNameingContext & ">;(&(objectCategory=msExchPrivateMDB)(msExchOwningServer=" & rs.fields("distinguishedName") & "));cn,name,displayname,distinguishedName,mDBStorageQuota,mDBOverQuotaLimit,company,mDBOverHardQuotaLimit,legacyExchangeDN;subtree"
	...
	strQuery = "<LDAP://"  & strDefaultNamingContext & ">;" & GALQueryFilter & ";distinguishedName,msExchMailboxGuid,company,displayname,mDBStorageQuota,mDBOverQuotaLimit,mDBOverHardQuotaLimit,mDBUseDefaults,msExchHomeServerName,legacyExchangeDN,homemdb;subtree"
	...
	while not rs2.eof
		...
		objChildRS("MOADCompany") = left(rs2.fields("company"),255)
	...
	Do While Not objgrandchild.EOF
		...
		if strUsedefaults = false then
			if strusrQuota <> 0 then
				wfile.writeline  strUsername & "," & objChildRS("MOADCompany") & "," & replace(formatnumber(strusrQuota/1024,2),",","")  & "," & replace(formatnumber(strusrQuotaLimit/1024,2),",","") & "," & replace(formatnumber(strusrHardQuotaLimit/1024,2),",","") & "," & replace(formatnumber(mbsize/1024,2),",","") & "," & formatnumber((mbsize/strusrQuota)*100,2) & "%"
			else
				wfile.writeline  strUsername & "," & objChildRS("MOADCompany") & "," &  replace(formatnumber(strusrQuota/1024,2),",","")  & "," & replace(formatnumber(strusrQuotaLimit/1024,2),",","") & "," & replace(formatnumber(strusrHardQuotaLimit/1024,2),",","") & "," & replace(formatnumber(mbsize/1024,2),",","") & "," & 0 & "%"
			end if
		else
			if strQuota <> 0 then
				wfile.writeline  strUsername & "," & objChildRS("MOADCompany")  & "," & replace(formatnumber(strQuota/1024,2),",","")  & "," & replace(formatnumber(strQuotaLimit/1024,2),",","") & "," & replace(formatnumber(strHardQuotaLimit/1024,2),",","") & "," & replace(formatnumber(mbsize/1024,2),",","") & "," & formatnumber((mbsize/strQuota)*100,2) & "%"
			else
				wfile.writeline  strUsername & "," & objChildRS("MOADCompany")  & "," & replace(formatnumber(strQuota/1024,2),",","")  & "," & replace(formatnumber(strQuotaLimit/1024,2),",","") & "," & replace(formatnumber(strHardQuotaLimit/1024,2),",","") & "," & replace(formatnumber(mbsize/1024,2),",","") & "," & 0 & "%"
			end if
		end if
