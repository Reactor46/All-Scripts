...
report = report & "<td align=""center"" bgcolor=""#000080""><b><font color=""#FFFFFF"">Owner</font></b></td>" & vbcrlf
report = report & "</tr>" & vbcrlf
...
while not rs.eof
	...
	wscript.echo objgroup.managedBy
	if not objgroup.managedBy = "" then
		set objOwner = getobject("LDAP://" & replace(objgroup.managedBy,"/","\/"))
	end if	
	numcheck = 0
	usNumberUsers = 0
	...
	if not objgroup.managedBy = "" then
		report = report & "<td align=""center"">" & objOwner.DisplayName & "&nbsp;</td>" & vbcrlf
	else
		report = report & "<td align=""center"">&nbsp;</td>" & vbcrlf
	end if
	report = report & "</tr>" & vbcrlf
	rs.movenext	
