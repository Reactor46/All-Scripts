...
set wfile = fso.opentextfile("c:\temp\exportquotas.csv",2,true)
	...
	Do While Not objgrandchild.EOF
		...
		if strUsedefaults = false then
			if strusrQuota <> 0 then
				if formatnumber((mbsize/strusrQuota)*100,2) > 90 then
			...
			if strQuota <> 0 then
				if formatnumber((mbsize/strQuota)*100,2) > 90 then
...
if smail = 1 then
	wscript.echo "Send Email to HelpDesk"
	Set objEmail = CreateObject("CDO.Message")
	objEmail.To = "address@domain.com"
	objEmail.From = "Quota Alerts <QuotaAlerts@domain.com>"
	objEmail.Subject = "Users that are 90% of there Quota on " & now()
	objEmail.Textbody = "Attached CSV of users"
	objEmail.AddAttachment("c:\temp\exportquotas.csv")
