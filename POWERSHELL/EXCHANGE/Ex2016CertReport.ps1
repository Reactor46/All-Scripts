...
Param(
   [Parameter(Mandatory=$false)] [string] $ServerFilter="MWTEXCH",
...
#            Write-Host "Connecting to server: " $server
            $Session = New-PSSession -ConfigurationName Microsoft.Exchange -ConnectionUri http://$Server.uson.local/PowerShell/ -Authentication Kerberos -Credential $UserCredential
