...
foreach ($srSearchResult in $srSearchResults){ 
	...
	if ($sgStorageGroup.msExchESEParamBaseName -ne "R00"){
		...
		$stmfile = [WMI]("\\" + $snServerName + "\root\cimv2:CIM_DataFile.Name='" + $msMailStore.msExchSLVFile + "'")
		$StoreFileSize =  $StoreFileSize + [math]::round($stmfile.filesize/1048576,0)
