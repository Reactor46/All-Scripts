...
function Get-MrOutlookVersion {
    ...
    Get-ChildItem -Path '\\mwtexch01\C$\Program Files\Microsoft\Exchange Server\V15\Logging\RPC Client Access\*.log'  -Filter (Where-Object LastWriteTime -gt (Get-Date).AddHours(-5))| 
