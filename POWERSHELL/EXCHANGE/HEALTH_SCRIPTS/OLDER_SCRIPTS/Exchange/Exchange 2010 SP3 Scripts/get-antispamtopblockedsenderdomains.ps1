...
# 
# Synopsis: Gets the top N sender domains that were blocked by antispam agents.
#
...
#    top: Find the top N blocked sender domains. By default (i.e. if not specified), N = 10.
#    command: Mandatory parameter. Specify p1 to identify P1 senders and p2 to identify P2 senders.
#
# Usage: 
#
#    .\get-AntispamTopBlockedSenderDomains p1 -location c:\Logs
#
...
filter get-UniqueP2Domains
{
  $domains = @{}
  
  foreach ($from in $_.P2FromAddresses )
  {
    if ($from.IsValid)
    {
      $domains[$from.DomainPart] = $true;
    }
  }
...
## Get all unique sender domains per entry, then group and sort them

switch ($report) {
 "p1"
 {
  get-AgentLog -location:$location -startDate:$startDate -endDate:$endDate |
    where { $_.Action -ne "AcceptMessage" -and $_.P1FromAddress.DomainPart -ne $() } |
    foreach { $_.P1FromAddress.DomainPart } |
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU8aAcCUvLyV64XVK+aDOup9h/
# b7GgghUmMIIEmTCCA4GgAwIBAgITMwAAAJ0ejSeuuPPYOAABAAAAnTANBgkqhkiG
...
# BBRWHOtexrLLe3+UgbH4PT/MfwoAxjCBiAYKKwYBBAGCNwIBDDF6MHigUIBOAGcA
# ZQB0AC0AQQBuAHQAaQBzAHAAYQBtAFQAbwBwAEIAbABvAGMAawBlAGQAUwBlAG4A
# ZABlAHIARABvAG0AYQBpAG4AcwAuAHAAcwAxoSSAImh0dHA6Ly93d3cubWljcm9z
# b2Z0LmNvbS9leGNoYW5nZSAwDQYJKoZIhvcNAQEBBQAEggEACYYFC6MIbJCVGxqg
# 5DKpPgv/O33y7mm0vk8v8U0JaZTelEbUO3XRsDDQ/l1TIyq1+91Df6k/+JbECuWZ
# 4dQ1rUvnLkZQmd9SpDpjSill8OlzWGLQ7B7e3iOVKG1MdtL5QSe1DxuSpE4SgvF/
# R2yXDixiXrBsFyONSxBZX1f3c/0F1qQ2aZcdn7s3lCO3A8V/r5CtI+/CAqT0iLHz
# t8kimDqKNV5EPTCMoP5xOB1HnkImsKNOdG9A4IUH1uEo5PhA1i9EiGIFVeOaLBq9
# hs4EJ9ks/oFjsBWJ17Qhnt41jozALtsBPAoWYYVReEdcbDX2ItpN1EspAVur5HqW
# 0vyikKGCAh8wggIbBgkqhkiG9w0BCQYxggIMMIICCAIBATCBhTB3MQswCQYDVQQG
# EwJVUzETMBEGA1UECBMKV2FzaGluZ3RvbjEQMA4GA1UEBxMHUmVkbW9uZDEeMBwG
...
# DQEJAzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTEzMDEwODA4NDcwNlow
# IwYJKoZIhvcNAQkEMRYEFBKmxm72TzX3ZUqxjLQVROufBo+bMA0GCSqGSIb3DQEB
# BQUABIIBAEUli8RxQVgWi8fgoWzif8M/JnQG8NEtg+t4o6acXHJv0iBnCk7qsh4R
# dw0M1vCFpfMASC34E7/VoWfhhgpndRh3eIVIwKqwtbzXhG3eguYzAF871Il/jrKg
# MpUUsHjS0vIuMoDHmWvbqOjqd56UCvpp7KND8qMqWS3UkEPtmaEDZHsDKkWVTLmS
# 08p59grpOINzkzaEsdeO7Dpd59A2h4fKk7O16ZbjUkvJKT9JTrwXr1wA9VkdRZQu
# 7QHgYQyz5hbdaJ1iY08GkZPXL28HPFOeLZN75vufY01ZrqCbCkjxCO59k56UWZ2p
# iGvj4aDjGPJvWT6K13Aeei0VE3U/NII=
