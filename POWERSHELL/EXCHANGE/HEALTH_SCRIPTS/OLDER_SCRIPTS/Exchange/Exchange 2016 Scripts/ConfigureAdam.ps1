...
if (-not (Test-Path $dsdbutil))
{
    $dsdbutil =  Join-Path $sysDir "dsdbutil.exe"
}

# Registry location of the ADAM Config
$adamConfigRegistryPath = "HKLM:\SOFTWARE\Microsoft\ExchangeServer\v15\EdgeTransportRole\AdamSettings\MSExchange"
...
function ChangeLDAPPort([int] $ldapPort)
{
    ...
    $statusLog = & $dsdbutil $ActivateInstanceCommand $ldapPortCommand $quit
...
function ChangeSSLPort([int] $sslPort)
{
    ...
    $quit = "quit"
    $statusLog = & $dsdbutil $ActivateInstanceCommand $sslPortCommand $quit
    $isSuccess = VerifyStatusLog($statusLog)
    if($isSuccess -eq $true)
    {
...
function ChangeLogPath([String] $logPath)
{
    ...
    $statusLog = & $dsdbutil $ActivateInstanceCommand $filesCommand $setlogpathCommand $quit $quit
    $isSuccess = VerifyStatusLog($statusLog)
    if($isSuccess -eq $true)
    {
...
function ChangeDataPath([String] $dataPath)
{
    ...
    $statusLog = & $dsdbutil $ActivateInstanceCommand $filesCommand $setdatafilepathCommand $quit $quit
    $isSuccess = verifyStatusLog($statusLog)
    if($isSuccess -eq $true)
    {
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUCS+5tz05n3fHcAT9uDS4XkxW
# vRmgghhkMIIEwzCCA6ugAwIBAgITMwAAAJ1CaO4xHNdWvQAAAAAAnTANBgkqhkiG
...
# NwIBFTAjBgkqhkiG9w0BCQQxFgQUZRRqm4z/Baf4dPVotGAGi18cJwwwXAYKKwYB
# BAGCNwIBDDFOMEygJIAiAEMAbwBuAGYAaQBnAHUAcgBlAEEAZABhAG0ALgBwAHMA
# MaEkgCJodHRwOi8vd3d3Lm1pY3Jvc29mdC5jb20vZXhjaGFuZ2UgMA0GCSqGSIb3
# DQEBAQUABIIBAEMeK+Bxg6OpZ8CJZnW9+R5LIZZHyV/CuMiqyMXWjIl3IrBLVLwM
# oqhFyqGtcF0zuSQYnrKwk4GdPNW02P+QU5bsWMbpUoztEJ82FLH/anbBW4wgLpYN
# AtUZWEsydOB2n+P7gab75tPOp7Xbz42Rr0KuTkwnrnqNSY/Lw/5Jg5aSDV/1PaPb
# zms+pr8+Wr7+xHWSkQ8rphE95byKXDOq7Ca/s6o5VEppoSc9R2S2l2+SCBcfry2L
# D6HfBlCA/B8M20k3kht4+I8l3HlG3oAMiWE3zXIcSMHGtov0vHyEW5Wg0xV3qNQu
# lNkw4d6orO2TVWWvcJNwqSghZJLIhehFpvWhggIoMIICJAYJKoZIhvcNAQkGMYIC
# FTCCAhECAQEwgY4wdzELMAkGA1UEBhMCVVMxEzARBgNVBAgTCldhc2hpbmd0b24x
# EDAOBgNVBAcTB1JlZG1vbmQxHjAcBgNVBAoTFU1pY3Jvc29mdCBDb3Jwb3JhdGlv
# bjEhMB8GA1UEAxMYTWljcm9zb2Z0IFRpbWUtU3RhbXAgUENBAhMzAAAAnUJo7jEc
# 11a9AAAAAACdMAkGBSsOAwIaBQCgXTAYBgkqhkiG9w0BCQMxCwYJKoZIhvcNAQcB
# MBwGCSqGSIb3DQEJBTEPFw0xNjA5MDMxODQ0MzlaMCMGCSqGSIb3DQEJBDEWBBSo
# AMK1yNlB8xdi5A9MAiySFebTojANBgkqhkiG9w0BAQUFAASCAQBUqzo4mV8vUEPb
# lr7uPoFKq5IK+U29+I50KihuOx/2Pv3WrRQ10C9n0athJZMLQx1rMAC3qA7+ogLw
# M6qzLRrSphQEleDiIc6EdtqaJ7QdunEy0k/i65pPfXUBvE6qGX7FZvQ9amehleFa
# 6Ct5UZXdKBePzpD0G4eBPM2g7VQgjHhYHtycKQ9MXnUMCAij70lzH+tG3wSUA5LH
# l0jaPGHZ35GbVtNEmUymfDxq8h02JzdH7xrC1+poNtGa480PM6Ef2V3VzrloVocR
# qPYUGUoQvnhhm2Y/pQPjq2jt2ARiLVUl/6UV2x1X2m21yHpHTqoiDwxZ1KFibT3K
# Rh3dEimT
