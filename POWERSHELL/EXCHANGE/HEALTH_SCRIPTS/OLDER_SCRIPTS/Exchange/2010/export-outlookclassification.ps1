<#
.EXTERNALHELP Export-OutlookClassification-help.xml
#>
...
if ($Organization)
{
    $organizationParameterString = " -Organization:`"$Organization`""
}
if ($Locale)
{
    $localeParameterString = " -IncludeLocales"
}

$command = "Get-MessageClassification" + $organizationParameterString + $localeParameterString

if ($Locale)
{
    $classifications = Invoke-Expression -Command $command | where {$_.Locale -eq $Locale}
}
else
{
    $classifications = Invoke-Expression -Command $command
}
...
if ($classifications)
{
    foreach ($i in $classifications)
    {
        ...
        $holder += "`t<Classification>`n`t`t<Name>$displayName</Name>`n`t`t<Description>$description</Description>`n`t`t<Guid>$id</Guid>$retain`n`t</Classification>`n"
    }
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUOh1pqzNFlNJ0XOiU28mRTWe4
# tqugghUmMIIEmTCCA4GgAwIBAgITMwAAAJ0ejSeuuPPYOAABAAAAnTANBgkqhkiG
...
# BBSVxTwJrMCT/lqJ2VZiUOrcI5vybDB6BgorBgEEAYI3AgEMMWwwaqBCgEAARQB4
# AHAAbwByAHQALQBPAHUAdABsAG8AbwBrAEMAbABhAHMAcwBpAGYAaQBjAGEAdABp
# AG8AbgAuAHAAcwAxoSSAImh0dHA6Ly93d3cubWljcm9zb2Z0LmNvbS9leGNoYW5n
# ZSAwDQYJKoZIhvcNAQEBBQAEggEAKoxntPNJPDQjKxrcSBzqZWZDr9HNQcAa5gu9
# 6JwGVjHKS0uyOMzEePRD7EFsCvVnLPFNQzxaECM+VAfdkR3Df3vwe5kXWiJRSto0
# 2xJ3KP0eiGGhlrg1IQtvQkaZlShl1JcXMmfKk+S7u2fVoBffpyxEtbGFUhAf+bg2
# GAE1KXvbfkxmFa13nFDos2oifZUGkZ7lDAjqG9Ruo5KRteqbA9jxvs2AAR6apdbX
# QlNxGi6gRNZ8jOzIYxJR1APHAqj55GN/wevMGUQ1PAjFSbstsUUB8M0xMkmLkW1+
# oAgKGMwj+Xfq3SadfEAQhbwuXKhqsXUeCSAIU2MlZJvLyAO0X6GCAh8wggIbBgkq
...
# BwEwHAYJKoZIhvcNAQkFMQ8XDTEzMDEwODA4NDcwNVowIwYJKoZIhvcNAQkEMRYE
# FMVGaQr5Y+fCAQAVQSUD4ff6NiRZMA0GCSqGSIb3DQEBBQUABIIBAGCQ1UIAdWL5
# Re6VJPQNybgtzjQp5gkvyeD4loRNI+D63LNBBDNJAX6jxs81ghNexvxt82qSmQri
# Nc165ehYt01bADRd37pAlJNseF+3XWfqWy9YGsH4ulgQVHCcTJDtdoH8h0l5clFl
# eSvitq2OjonovWUh43KBorWEkGlxWdWTaftKQEsBfgzlS81TE6GcuM1U5tDezSNf
# 7eg+FwN4I11sWvn8KdVD04p21e+ISIdYOqXd2G8bW/mcyUjpsNCljMDnBkkHEqK4
# tpT3pqeZtR0OGgcHi0SIYcJmp24hQHBDn85hyEYoGbqNGrogtvsIwzF5pldHHPDu
# CeMAWeGuvJ4=
