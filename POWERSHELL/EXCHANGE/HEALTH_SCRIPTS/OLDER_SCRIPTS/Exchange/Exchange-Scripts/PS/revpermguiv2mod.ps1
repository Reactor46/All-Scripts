...
foreach ($emResult in $srSearchResult) {
	...
	$usUSRinfo.add($uoUserobject.samaccountname.ToString(),$uoUserobject)
...
foreach($key in $nmMailboxPerms.keys){
	...
	if ($usUSRinfo[$key].msExchRecipientTypeDetails -ne $null){
		$highPart = $usUSRinfo[$key].msExchRecipientTypeDetails.Value.GetType().InvokeMember("HighPart" ,[System.Reflection.BindingFlags]::GetProperty, $null,$usUSRinfo[$key].msExchRecipientTypeDetails.Value, $null)
		$lowPart =  $usUSRinfo[$key].msExchRecipientTypeDetails.Value.GetType().InvokeMember("LowPart",[System.Reflection.BindingFlags]::GetProperty, $null, $usUSRinfo[$key].msExchRecipientTypeDetails.Value, $null)
		$recptype = $highPart + $lowPart
	}
	else {$recptype = ""}
	$rs1Table.Rows.Add($key,$usUSRinfo[$key].DisplayName.ToString(),$recptype, $nmMailboxPerms[$key],$fpFolderPerms[$key],$nmDelegatePerms[$key],$nmDefualtPerms[$key],$nmSendRecieve[$key],$rvMbRights,$rvFpRights,$rvDRRights,$rvSRRights)
...
$rs1Table.Columns.Add("DisplayName")
$rs1Table.Columns.Add("RecipientDisplayType")
