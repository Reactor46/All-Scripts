<#
 ...
 Requirements:
	...
	$EnvFile = "C:\Temp\Scripts\ExchangeEnvironment.csv"
 Example:
 	./generatefirewallrules.ps1 -Envfile "./ExchangeEnvironment.csv" -RuleFile "./FirewallRules.csv" -OutputFile "./FirewallOutput.csv"
 Notes:
			...
			Foreach ($Rule in $ApplicableRules) {
				...
				($Rule.Source_Role -eq "Internet"))))) {
					...
					If (($ExchangeServer.Role -eq $Rule.Source_Role) -and ($ExchangeOtherServer.Role -eq $Rule.Destination_Role)) {
						...
						#Skip rule generation if database to non-database and not same site then skip rule
