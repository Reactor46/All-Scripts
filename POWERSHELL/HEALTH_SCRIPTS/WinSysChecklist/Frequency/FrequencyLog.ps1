...
$html           ="$global:ScriptLocation\$computerp.$today.htm"
...
if($version -le 2){
	...
	$SystemDataJSON | out-file "$global:ScriptLocation\system.json"
	
	$AppDataJSON = $Appdata       | Sort-Object Count -Descending | ConvertTo-Json
	$AppDataJSON | out-file "$global:ScriptLocation\application.json"
	#cleanup
    ...
    if($IncludeSecurity){
		...
		$SecDataJSON | out-file "$global:ScriptLocation\security.json"
		#cleanup
