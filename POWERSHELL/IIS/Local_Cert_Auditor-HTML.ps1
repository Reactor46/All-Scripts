...
$servers = Get-Content D:\All-IIS-Servers.txt #Get-ADComputer -Filter "OperatingSystem -like 'Windows Server*' -and Enabled -eq '$true'" | Sort-Object |Foreach {$_.Name}
...
$DOCreport = "$Path\$YearMonth\$currentDay - $DaystoSearch Days Certificate Expiration Report.html"
...
New-Item -Path $fullpath -Name "$currentDay - $DaystoSearch Days Certificate Expiration Report.html" -ItemType File
...
foreach ($server in $servers) { #each server start
	...
	$certs = Invoke-Command -ComputerName $server -ScriptBlock { Get-ChildItem -path Cert:\LocalMachine\My -Recurse -ErrorAction SilentlyContinue | `
    Where-Object {
        (
            $_.Subject -like "CN=*.kelseycareadvantage.com*" -or
            $_.Subject -like "CN=*.ksnet.com*" -or
            $_.Subject -like "CN=*.kscpulse.com*" -or
            $_.Subject -like "CN=*.kelsey-seybold.com*"
        ) -and
        ($_.Subject -notlike "*O=DO_NOT_TRUST*")
    }  | Select-Object Subject, FriendlyName, Thumbprint, NotBefore, NotAfter -ExcludeProperty PSComputerName, RunspaceId, PSSHowComputerName |`
...
$sortexpired | ConvertTo-Html -head $a -property Name,Subject,FriendlyName,Thumbprint,'Issued Date','Expiration Date' >> $DOCreport
...
$From = "WebTeamReports@ksnet.com"
$To = "john.battista@kelsey-seybold.com"
...
$SMTPServer = "smtp-relay.ksnet.com"
$SMTPPort = "25"
