...
# Version 1.0
# 
# Removes IIS Web-Bindings and SSL Bindings
#
...
Function Help{
	...
	Write-host " -ip     IP Address (use 0.0.0.0 for *)" -ForegroundColor yellow
	Write-host " -p      Port" -ForegroundColor yellow
	Write-host "Example:" -ForegroundColor yellow
	Write-Host $($MyInvocation.MyCommand.Name + " -w " + [char]34 + "Default Site" + [char]34 + " -ip 192.168.1.23 -p 80" + [char]10) -ForegroundColor yellow
	Exit 1}
...
Function RemoveIISBinding{
	param($WebSite, $IP, $Port)
	
	# Remove SSL Binding for WebSite
	$sslIISBindings = dir iis:\sslbindings | where {$_.Port -eq $Port} | get-item
    If ($sslIISBindings -ne $null) {
		write-host @("Removing SSL Binding for IP Address: " + $IP + " and port: " + $Port) -Foregroundcolor green
		$sslIISBindings | remove-item}
	Else
		{write-host @("No SSL Binding for IP Address: " + $IP + " and port: " + $Port + " exists.") -Foregroundcolor green}
	
    # Remove Web Binding for WebSite
	$WebBinding = get-webbinding -port $Port -name $WebSite
	If ($WebBinding -ne $null) {
		write-host @("Removing Web Binding for IP Address: " + $IP + " and port: " + $Port) -Foregroundcolor green
		$WebBinding | remove-webbinding}	
	Else
		{write-host @("No Web Binding for IP Address: " + $IP + " and port: " + $Port + " exists.") -Foregroundcolor green}
}
...
If ($bError -eq 0)
	...
	IF ($error.count -ne 0) 
		...
		import-module WebAdministration
		$Error.Clear()}
	# Call Function to remove IIS binding
	If ($bError -eq 0)
		{
            ...
            {
                $bError = RemoveIISBinding $WebSite $IP $Port
            }
        }
	ElseIf ($bError -eq 1)
		{Help}
