...
param
    ...
    $locale = [Convert]::ToInt32((Get-WmiObject Win32_OperatingSystem).locale,16)
    
    # used to get info from netsh command
    $pattern = "Ports : "

    # English - United States: 1033
    # get yours from https://msdn.microsoft.com/en-us/goglobal/bb964664.aspx
    if(($locale -ne 1033)){

        switch($locale){
            # pt-Br
            1046 { $pattern = "Portas : " }
            default {"locale not supported.";break}
        }
    }
    ...
    if($isVistaOrLater)
    {
        ...
        $maxPorts = netsh int ip show dynamicport tcp |
            Select-String -Pattern ($pattern + "(\d*)”)|
