...
Function Get-MailQueues ($transportServerList) {
	...
	$stats_collection = $transportServerList | Sort-Object Name | %{Get-Queue -Server $_ | ?{$_.Identity -notmatch 'Shadow'}}
	Write-Host "Done" -ForegroundColor Green
	return $stats_collection
}
...
Function Get-ExServerComponents ($exServerList) {
	...
	foreach ($exServer in $exServerList) {		
		#$stats_collection += (Get-ServerComponentState $exServer | ?{$_.State -ne 'Active'} | Select Identity,Component,State)
		$stats_collection += (Get-ServerComponentState $exServer | Select Identity,Component,State)
	}	
	Write-Host "Done" -ForegroundColor Green
	return $stats_collection
}
...
Function Create-DAGCopyStatusReport ($mdbCopyStatus) {
	...
	$mbody += '</tr>'
if($errString -ne $null) {$mResult = "<tr><td>Mailbox Database Copy Status</td><td class = ""bad"">Failed</td></tr>" ; $testFailed=1}
Write-Host "Done" -ForegroundColor Green
...
if($errString -ne $null) {$mResult = "<tr><td>Server Health Status</td><td class = ""bad"">Failed</td></tr>" ; $testFailed=1}
Write-Host "Done" -ForegroundColor Green
return $mbody,$errString,$mResult,$testFailed
}
...
if($errString -ne $null) {$mResult = "<tr><td>Mail Queue</td><td class = ""bad"">Failed</td></tr>" ; $testFailed=1}
Write-Host "Done" -ForegroundColor Green
return $mbody,$errString,$mResult,$testFailed
}
...
$mbody += '<table id="data">'
	...
	$mbody += "<hr />"
if($errString -ne $null) {$mResult = "<tr><td>DAG Members Replication</td><td class = ""bad"">Failed</td></tr>" ; $testFailed = 1}
Write-Host "Done" -ForegroundColor Green
return $mbody,$errString,$mResult
}

Function Create-ServerComponentStateReport ($serverComponentStateInfo) {
...
if($errString -ne $null) {$mResult = "<tr><td>Server Component State</td><td class = ""bad"">Failed</td></tr>" ; $testFailed = 1}
Write-Host "Done" -ForegroundColor Green
return $mbody,$errString,$mResult,$testFailed
}
...
if($errString -ne $null) {$mResult = "<tr><td>Disk Space </td><td class = ""bad"">Failed</td></tr>" ; $testFailed = 1}
Write-Host "Done" -ForegroundColor Green
return $mbody,$errString,$mResult,$testFailed
}
...
ForEach ($db in $dblist)
{
	...
	if ($db.mounted -eq $true)
	{
		...
		elseif ($lastfullbackup -eq '[NO DATA]')
		{
			$errString += "<tr><td>Database Backup</td></td><td>$($db.Name) - last full backup date [$lastfullbackup] is OLDER than $($t_lastfullbackup) Day(s)</td></tr>"
			$mbody += '<td class = "bad">' + $lastfullbackup + '</td>'		
		}
...
if($errString -ne $null) {$mResult = "<tr><td>Mailbox Database Status</td><td class = ""bad"">Failed</td></tr>" ; $testFailed = 1}
Write-Host "Done" -ForegroundColor Green
return $mbody,$errString,$mResult,$testFailed
}
...
if($errString -ne $null) {$mResult = "<tr><td>Public Folder Database Status</td><td class = ""bad"">Failed</td></tr>" ; $testFailed = 1}
Write-Host "Done" -ForegroundColor Green
return $mbody,$errString,$mResult,$testFailed
}
...
$nonEx2010 = $ExServerList | ?{$_.AdminDisplayVersion -notlike "Version 14*"}
$nonEx2010transportServers = $ExServerList | ?{$_.AdminDisplayVersion -notlike "Version 14*" -and $_.ServerRole -match 'Mailbox'}
$Ex2010TransportServers = $ExServerList | ?{$_.AdminDisplayVersion -like "Version 14*" -and $_.ServerRole -match 'HubTransport'}
...
if ($RunDAGCopyReport -eq $true) {$dagCopyData = Get-DAGCopyStatus ($ExMailboxDBList) ; $testCount++}
if ($RunDAGReplicationReport -eq $true) {$repldata = Get-ReplicationHealth ; $testCount++}
...
if ($RunServerHealthReport -eq $true) {$serverhealthreport,$sError,$sResult,$sFailed = Create-ServerHealthReport ($serverhealthdata) ; $errSummary += $sError; $testFailed += $sFailed}
if ($RunComponentReport -eq $true -AND $nonEx2010.count -gt 0) {$componentHealthReport,$cError,$cResult,$cFailed = Create-ServerComponentStateReport ($componentHealthData) ; $errSummary += $cError; $testFailed += $cFailed}
if ($RunMdbReport -eq $true) {$mdbreport,$mError,$mdbResult,$mdbFailed = Create-MdbReport ($mdbdata) ; $errSummary += $mError; $testFailed += $mdbFailed}
if ($RunPdbReport -eq $true -AND $ExPFDBList.Count -gt 0) {$pdbreport,$pdbError,$pdbResult,$pdbFailed = Create-PdbReport ($pdbdata) ; $errSummary += $pdbError; $testFailed += $pdbFailed}
if ($RunDAGCopyReport -eq $true) {$dbcopyreport,$dbCopyError,$dbResult,$dbFailed = Create-DAGCopyStatusReport ($dagCopyData) ; $errSummary += $dbCopyError; $testFailed += $dbFailed}
if ($RunDAGReplicationReport -eq $true) {$replicationreport,$rError,$rResult,$rFailed = Create-ReplicationReport ($repldata) ; $errSummary += $rError; $testFailed += $rFailed}
if ($RunQueueReport -eq $true) {$queuereport,$qError,$qResult,$qFailed = Create-QueueReport($queueData) ; $errSummary += $qError; $testFailed += $qFailed}
if ($RunDiskReport -eq $true) {$diskreport,$dError,$dResult,$dFailed = Create-DiskReport ($diskdata) ; $errSummary += $dError; $testFailed += $dFailed}
...
if ($RunPdbReport -eq $true -AND $ExPFDBList.Count -gt 0) {$mail_body += $pdbRResult}
if ($RunDAGCopyReport -eq $true) {$mail_body += $dbResult}
if ($RunDAGReplicationReport -eq $true) {$mail_body += $rResult}
...
if ($RunComponentReport -eq $true -AND $nonEx2010.count -gt 0) {$mail_body += $componentHealthReport ; $mail_body += '</table><hr />'}
...
$mail_body += '</p><p>'
$mail_body += '<a href="https://github.com/junecastillote/Get-ExchangeHealth/tree/v5.2">Exchange Server Health Check v.'+$scriptVersion+'</a></p>'
$mail_body += '</html>'
$mbody = $mbox -replace "&lt;","<"
$mbody = $mbox -replace "&gt;",">"
