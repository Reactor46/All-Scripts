# List of GUIDs (Example GUIDs, replace these with your actual list)
$guids = @(
...
$outputHtmlPath = "D:\AppInstalled-Report.html"

# Initialize an array to hold the report data
$reportData = @()

# Function to search registry and get application details
function Get-AppDetails {
    param (
        [string]$guid
    )
    
    # Search for the GUID in the Uninstall registry key
    $keyPath1 = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\$guid"
    $keyPath2 = "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\$guid"

    $appData = $null

    # Check the first registry path (for 64-bit applications)
    if (Test-Path $keyPath1) {
        $appData = Get-ItemProperty -Path $keyPath1
    }
    # Check the second registry path (for 32-bit applications)
    elseif (Test-Path $keyPath2) {
        $appData = Get-ItemProperty -Path $keyPath2
    }

    if ($appData) {
        $appName = $appData.DisplayName
        $installDate = $appData.InstallDate
        $installLocation = $appData.InstallLocation
...
# Loop through the GUIDs and get the application details
foreach ($guid in $guids) {
    $appDetails = Get-AppDetails -guid $guid
    if ($appDetails) {
        $reportData += $appDetails
    }
}
...
<head>
    ...
    <title>Installed Applications Report</title>
    <style>
        body {
...
<body>
    <h1>Installed Applications Report</h1>
    <table>
        <thead>
            <tr>
...
Write-Host "Report generated at: $outputHtmlPath"
