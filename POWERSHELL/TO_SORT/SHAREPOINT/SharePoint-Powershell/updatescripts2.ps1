...
Function CreateTermSetItems() {
    ...
    [Microsoft.SharePoint.Publishing.Navigation.NavigationTermSet]$editableTermSet = $null
	...
	foreach ($web in $newSite.AllWebs)
 	{
		...
		Try
		{
			...
			$pweb.Update()
			Write-Host "Default Page layout has been set to Detail Page Layout"
			LogComment "Default Page layout has been set to Detail Page Layout"
		}
		Catch
		{
			Write-Host -fore Red $_.Exception.Message
		...
		foreach ($SourcePage in $SourcePages)
		{
			Try
			{
                ...
                $file = $web.GetFile($SourcePage.Url)
				...
				if($file.Exists) {
				    ...
				    if($SourcePageListItem.File.CheckOutStatus -ne "None") {
...
#$spWeb = Get-SPWeb("http://vm119is01wbdv15:8074")
#$spFile = $spWeb.GetFile("http://vm119is01wbdv15:8074/Pages/default.aspx")
