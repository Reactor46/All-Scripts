...
# write config changes
    if(!(Test-Path -Path "C:\solr\zookeeper-3.4.14\conf\zoo.cfg"))
    {
        Write-Host "Rewriting zookeeper config"
 
        $cfg = Get-Content "C:\solr\zookeeper-3.4.14\conf\zoo_sample.cfg"
        Copy-Item "C:\solr\zookeeper-3.4.14\conf\zoo_sample.cfg" "C:\solr\zookeeper-3.4.14\conf\zoo.cfg"

#The /conf/zoo.cfg file should have the content:
$newCfg = $cfg | % { $_ -replace "dataDir=/tmp/zookeeper", "dataDir=C://solr//zookeeper_data" }
Add-Content $newCfg -Value "server.0=internalip1:2888:3888"
Add-Content $newCfg -Value "server.1=internalip2:2889:3889"
Add-Content $newCfg -Value "server.2=internalip3:2890:3890"
...
# Now, to check if the zookeeper is up and working properly,`
# open a new terminal session and go to the zookeeper bin directory again.`
# Then start the zookeeper client with the following command:

zkCli.cmd

# Now close the command prompts and create myid files per zookeeper node:
# These are the IDs and locations of all servers in the ensemble,`
# the ports on whichthey communicate with each other. The server ID must`
# additionally stored in the/myid file and be located in the dataDir of each`
# ZooKeeper instance. TheID identifies each server, so in the case of this first`
# instance, you would createthe file /var/lib/zookeeperdata/1/myid with the content 1.
...
# The -f parameter allows for the service to always restart the currently running Solr process,`
# without trying to spin a new one up. Ultimately, this is the expected behavior.
...
# After confirming Solr was installed and connected to the Zookeeper ensemble as expected,`
# we can kill the Solr instances by closing the corresponding Java processes or by entering`
# the following commands in an elevated prompt:
...
# In node 1 folder, locate the configset folder under`
# the “C:\solr\solr-7.5.0\server\solr\configsets” and copy`
# the _default folder to a newsitecore_configs folder.
...
# Within this there has been created a rule to allow the Azure load balancer`
# connecting on port 80 and 8983. Since the health probe did not work on port 8983`
# (possibly because of the basic authentication) there is a website within IIS that is running on port 80.
...
#>
# Since we restrict access to the VMs by NSG we can allow any IP to connect`
# to port 81, 2181 and 8983 on the Solr nodes by adding the following firewall rule:
