...
##############################################################################
# This Script was adapted from Jason Fossen. See above. Fossem Rocks!
##############################################################################
...
Function Send-Email {
   ...
   # Write-Host "Sending E-mail to $To`nSubject $Subject`n`n$Body"
	...
	{
		If(! (Test-Path $anexo) ){ Write-Host "Error attaching file $anexo"; Continue}
    ...
    $smtpclient.Credentials = new-object System.Net.NetworkCredential("user@mail.com", "password") 
