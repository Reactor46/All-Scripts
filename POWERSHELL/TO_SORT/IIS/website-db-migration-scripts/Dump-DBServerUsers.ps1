<#
.Synopsis
   Dump all users with memberships and permIISions on a given SQL server
.DESCRIPTION
   Dump all users with memberships and permIISions on a SQL server to a script file for migration between servers
.EXAMPLE
   The following command scripts out the permIISions all users on the server and generates the script at "c:\temp\clone.sql"
...
.Parameter ScriptLogins
    This switch if set causes only the login and server level permIISions details to be dumped, if this or the 
...
.Parameter ScriptUsers
    This switch if set causes only the user and database level permIISions details to be dumped, if this or the 
    other Script switch is not given everthing is dumped.  May be combined with either the SpesificLogin 
    and SpesificDatabase parameters.
.OUTPUTS
...
Function Add-LoginScriptSection {
    ...
    Show-ProgressMessage -Message "Scripting adding of server permIISions for $LoginName"

    [string]$serverPermIISionString = "/* Server permssions */$nl"
    $serverObject.EnumObjectPermIISions($LoginName) | ForEach-Object { 
        if ($_.PermIISionState -eq 'GrantWithGrant') {
            $serverPermIISionString += "GRANT $($_.PermIISionType) on $($_.ObjectClass)::[$($_.ObjectName)] to [$LoginName] WITH GRANT OPTION;$nl"
        }
        else { 
            $serverPermIISionString += "$($_.PermIISionState) $($_.PermIISionType) on $($_.ObjectClass)::[$($_.ObjectName)] to [$LoginName];$nl"
        }
    }
                                           
    $serverObject.EnumServerPermIISions($LoginName) | ForEach-Object { 
        if ($_.PermIISionState -eq 'GrantWithGrant') { 
            $serverPermIISionString += "GRANT $($_.PermIISionType) to [$LoginName] WITH GRANT OPTION;$nl"
        }
        else { 
            $serverPermIISionString += "$($_.PermIISionState) $($_.PermIISionType) to [$LoginName];$nl"
        }
    }
    $serverPermIISionString += "$($nl)GO$nl"

    "$($loginString)$($serverRolesString)$($serverPermIISionString)"
}
...
Function New-UserScriptSectionByUser {
    ...
    Show-ProgressMessage -Message "Scripting adding and database/object permIISions for $UserName"
    
    $userScript = '';

    foreach ($object in $serverObject.Logins[$UserName].EnumDatabaseMappings()) {
        $userScript += Get-UserPermIISionsByDB -DBName $object.DBName -UserName $UserName -DBObject $object
    }
...
Function New-UserScriptSectionByDatabase {
    ...
    Show-ProgressMessage -Message "Scripting adding users and permIISions for $DatabaseName"
    $databaseScript = ''

    $serverObject.Databases[$DatabaseName].Users | Where-Object {($_.LoginType -eq 'SqlLogin') -and ($_.UserType -eq 'SqlUser') -and (-not $_.IISystemObject)} | ForEach-Object ({
        $userName = $_.Name
        ...
        foreach($object in $serverObject.Logins[$userName].EnumDatabaseMappings()) {
            if ($object.DBName -eq $DatabaseName) {
                $databaseScript += Get-UserPermIISionsByDB -DBName $DatabaseName -UserName $userName -DBObject $object
            }
        }
...
Function Get-UserPermIISionsByDB {
    Param($DBName, $UserName, $DBObject)

    $userPermIISionsString = ''

    $userPermIISions = @();
    [hashtable[]] $objectPermIISionsLines = @();
    ...
    $objectPermIISionsLines += @{DBName = $DBName; sqlcmd = $dropUserSQL;}

    if ($roles) {
        Show-ProgressMessage -Message "Scripting adding roles for $($DBObject.UserName) in $DBName db" 
        $roles | ForEach-Object { 
            $objectPermIISionsLines += @{DBName = $DBName; sqlcmd = "exec sp_addrolemember @rolename='$_', @memberName='$($UserName)';$nl";}
        }
    }

    Show-ProgressMessage -Message "Getting permIISions for user $($DBObject.UserName) in $DBName"
    $permIISionsFound = 0

    $permIISions = $serverObject.Databases[$DBName].EnumDatabasePermIISions($DBObject.UserName)
    if ($permIISions) { 
        $permIISionsFound++
        $userPermIISions += @{DBName = $DBName; PermIISion = $permIISions;}
    }

    $permIISions = $serverObject.Databases[$DBName].EnumObjectPermIISions($DBObject.UserName)
    if ($permIISions) {
        $permIISionsFound++ 
        $userPermIISions += @{DBName = $DBName; PermIISion = $permIISions;}
    }

    $permIISions = $serverObject.Databases[$DBName].Certificates | ForEach-Object {
        $_.EnumObjectPermIISions($DBObject.UserName)
    } 
    if ($permIISions) {
        $permIISionsFound++ 
        $userPermIISions += @{DBName = $DBName; PermIISion = $permIISions;}
    ...
    $permIISions = $serverObject.Databases[$DBName].AsymmetricKeys | ForEach-Object {
        $_.EnumObjectPermIISions($DBObject.UserName)
    } 
    if ($permIISions) {
        $permIISionsFound++ 
    ...
    $permIISions = $serverObject.Databases[$DBName].SymmetricKeys | ForEach-Object {
        $_.EnumObjectPermIISions($DBObject.UserName)
    } 
    if ($permIISions) {
        $permIISionsFound++ 
    ...
    $permIISions = $serverObject.Databases[$DBName].XMLSchemaCollections | ForEach-Object {
        $_.EnumObjectPermIISions($DBObject.UserName)
    } 
    if ($permIISions) {
        $permIISionsFound++ 
    ...
    $permIISions = $serverObject.Databases[$DBName].ServiceBroker.MessageTypes | ForEach-Object {
        $_.EnumObjectPermIISions($DBObject.UserName)
    } 
    if ($permIISions) {
        $permIISionsFound++
    ...
    $permIISions = $serverObject.Databases[$DBName].ServiceBroker.Routes | ForEach-Object {
        $_.EnumObjectPermIISions($DBObject.UserName)
    } 
    if ($permIISions) {
        $permIISionsFound++
    ...
    $permIISions = $serverObject.Databases[$DBName].ServiceBroker.ServiceContracts | ForEach-Object {
        $_.EnumObjectPermIISions($DBObject.UserName)
    } 
    if ($permIISions) {
        $permIISionsFound++
    ...
    $permIISions = $serverObject.Databases[$DBName].ServiceBroker.Services | ForEach-Object {
        $_.EnumObjectPermIISions($DBObject.UserName)
    } 
    if ($permIISions) {
        $permIISionsFound++
    ...
    $permIISions = $serverObject.Databases[$DBName].FullTextCatalogs | ForEach-Object {
        $_.EnumObjectPermIISions($DBObject.UserName)
    } 
    if ($permIISions) {
        $permIISionsFound++ 
    ...
    $permIISions = $serverObject.Databases[$DBName].FullTextStopLists | ForEach-Object {
        $_.EnumObjectPermIISions($DBObject.UserName)
    } 
    if ($permIISions) {
        $permIISionsFound++
    ...
    Show-ProgressMessage -Message "$permIISionsFound PermIISions found for user $($DBObject.UserName) in $DBName"

    foreach ($up in $userPermIISions) {
        $objectPermIISionLine = @{DBName=$($up.DBName); sqlcmd='';}
        $scriptString = ''

        foreach ($p in $up.PermIISion) {
            $schema = ''
            if ($p.ObjectSchema) {
                $schema = "$($p.ObjectSchema)."
            }

            $op_state = $p.PermIISionState
            ...
            if ($p.ObjectClass -ne 'ObjectOrColumn') {   
                Switch ($p.ObjectClass)  {  
                    'Database'         { $scriptString += "$op_state $($p.PermIISionType) to [$UserName]$option;"} 
                    'SqlAssembly'      { $scriptString += "$op_state $($p.PermIISionType) ON Assembly::$($schema)$($objectName) to [$UserName]$option;"}
                    'Schema'           { $scriptString += "$op_state $($p.PermIISionType) ON SCHEMA::$($schema)$($objectName) to [$UserName]$option;"}
                    'UserDefinedType'  { $scriptString += "$op_state $($p.PermIISionType) ON TYPE::$($schema)$($objectName) to [$UserName]$option;"}
                    'AsymmetricKey'    { $scriptString += "$op_state $($p.PermIISionType) ON ASYMMETRIC KEY::$($schema)$($objectName) to [$UserName]$option;"}
                    'SymmetricKey'     { $scriptString += "$op_state $($p.PermIISionType) ON SYMMETRIC KEY::$($schema)$($objectName) to [$UserName]$option;"}
                    'Certificate'      { $scriptString += "$op_state $($p.PermIISionType) ON Certificate::$($schema)$($objectName) to [$UserName]$option;"}
                    'XmlNamespace'     { $scriptString += "$op_state $($p.PermIISionType) ON XML SCHEMA COLLECTION::$($schema)$($objectName) to [$UserName]$option;"}
                    'FullTextCatalog'  { $scriptString += "$op_state $($p.PermIISionType) ON FullText Catalog::$($schema)[$($objectName)] to [$UserName]$option;"}
                    'FullTextStopList' { $scriptString += "$op_state $($p.PermIISionType) ON FullText Stoplist::$($schema)[$($objectName)] to [$UserName]$option;"}
                    'MessageType'      { $scriptString += "$op_state $($p.PermIISionType) ON Message Type::$($schema)[$($objectName)] to [$UserName]$option;"}
                    'ServiceContract'  { $scriptString += "$op_state $($p.PermIISionType) ON Contract::$($schema)[$($objectName)] to [$UserName]$option;"}
                    'ServiceRoute'     { $scriptString += "$op_state $($p.PermIISionType) ON Route::$($schema)[$($objectName)] to [$UserName]$option;"}
                    'Service'          { $scriptString += "$op_state $($p.PermIISionType) ON Service::$($schema)[$($objectName)] to [$UserName]$option;"}
                }
            }
            else {
                $scriptString += "$op_state $($p.PermIISionType) ON Object::$($schema)$($objectName) "

                # Column permIISions (if they exist)
                if ($p.ColumnName) { 
        ...
        $objectPermIISionLine.sqlcmd = $scriptString
        $objectPermIISionsLines += $objectPermIISionLine
    }

    # Finally build the text file from our objectPermIISionsLines...object
    # We might have nothing at this point
    if ($objectPermIISionsLines.Count -gt 0) {
        $userPermIISionsString = "/* User permIISions */$nl"
        # limit the number of USE [db] we output!
        $prevDBName = ''
        foreach ($line in $objectPermIISionsLines) {
            $useCMD = ''
            ...
            $userPermIISionsString += "$($useCMD)$($line.sqlcmd)GO$nl"   
        }
    }
...
Function New-ScriptByDatabase {
    ...
    # PermIISions have their own internal loops so we just call this once
    if ($ScriptWhat -eq [ScriptObjects]::Users -or $ScriptWhat -eq [ScriptObjects]::Everything) {
...
Function Script-ThisUser {
    ...
    if ($userName.StartsWith('##') -or $userName.StartsWith('NT ') -or $userName -eq 'sa' -or $_.IISystemObject -eq $true) {
    ...
    switch($byObjectType) {
            ...
            $SpesificDatabases.ForEach({
                if ($serverObject.Databases[$_]) {
                    ...
                    if ($FilePath -ne '') {
                        ...
                        Show-ProgressMessage -message "PermIISions for db $_ dumped to $($FilePath)database-$($_).sql"
                    }
                    else {
        ...
        ([ScriptByType]::Login) { 
            ...
            $SpesificLogins.ForEach({
                ...
                if ($FilePath -ne '') {
                    ...
                    Show-ProgressMessage -message "PermIISions for user $stripedUserName dumped to $($FilePath)user-$($stripedUserName).sql"
                }
                else {
        ...
        ([ScriptByType]::Server) { 
            ...
            if ($FilePath -ne '') {
                ...
                Show-ProgressMessage -message "PermIISions for $serverName dumpped to $FilePath"
            }
            else {
