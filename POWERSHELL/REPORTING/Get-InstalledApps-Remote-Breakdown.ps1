...
# Initialize arrays to store reports
$allReports = @()
$uniqueApps = @()
$dotnetApps = @()
...
"@
            ...
            # Add the current server's applications to the global list for unique apps
            $uniqueApps += $apps

            # Check for .NET related and Python applications
            $dotnetFilter = @("Microsoft .NET Runtime", "Microsoft .NET Targeting Pack", "Microsoft .NET Toolset", "Microsoft ASP .NET Core", "Microsoft Windows Desktop Runtime", "Microsoft Windows Desktop Targeting Pack", "Python")
            foreach ($app in $apps) {
                foreach ($keyword in $dotnetFilter) {
                    if ($app.ApplicationName -like "*$keyword*") {
                        $dotnetApps += [PSCustomObject]@{
...
# Aggregate unique applications from all servers (no duplicates)
$uniqueApps = $uniqueApps | Select-Object ApplicationName, UninstallString | Sort-Object ApplicationName -Unique

# Save the aggregated report of unique applications

# CSV Output for Unique Applications
$csvAggregatedPath = "D:\Reports\InstalledApps\Unique-InstalledApps.csv"
$uniqueApps | Export-Csv -Path $csvAggregatedPath -NoTypeInformation

# HTML Output for Unique Applications with vibrant CSS
$htmlAggregatedPath = "D:\Reports\InstalledApps\Unique-InstalledApps.html"
$htmlContent = $uniqueApps | ConvertTo-Html -Property ApplicationName, UninstallString -PreContent "<h1>Aggregated Unique Installed Applications</h1>" -PostContent "<footer>Generated on $(Get-Date)</footer>"

# Add vibrant CSS
$htmlContent = $htmlContent -replace "</head>", "$css</head>"
$htmlContent | Out-File -FilePath $htmlAggregatedPath

Write-Host "Aggregated report of unique applications generated: $csvAggregatedPath and $htmlAggregatedPath"

# Compile report of .NET and Python related applications
$csvDotnetPath = "D:\Reports\InstalledApps\BreakDown\DotNetAndPythonApps.csv"
$dotnetApps | Export-Csv -Path $csvDotnetPath -NoTypeInformation

$htmlDotnetPath = "D:\Reports\InstalledApps\BreakDown\DotNetAndPythonApps.html"
$htmlDotnetContent = $dotnetApps | ConvertTo-Html -Property ServerName, ApplicationName, UninstallString -PreContent "<h1>Microsoft .NET and Python Applications</h1>" -PostContent "<footer>Generated on $(Get-Date)</footer>"

# Add vibrant CSS for .NET/Python report
$htmlDotnetContent = $htmlDotnetContent -replace "</head>", "$css</head>"
$htmlDotnetContent | Out-File -FilePath $htmlDotnetPath

Write-Host "Report on .NET and Python applications generated: $csvDotnetPath and $htmlDotnetPath"
