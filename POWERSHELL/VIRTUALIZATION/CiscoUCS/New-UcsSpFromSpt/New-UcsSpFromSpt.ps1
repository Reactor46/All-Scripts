...
.SYNOPSIS
	This script will create a UCS Service Profile from a Service Profile Template

.DESCRIPTION
	This script will create a UCS Service Profile from a Service Profile Template

.EXAMPLE
	New-UcsSpFromSpt.ps1
	This script can be run without any command line parameters.  User will be prompted for all parameters and options required

.EXAMPLE
	New-UcsSpFromSpt.ps1 -ucs "1.2.3.4" -ucred
...
.EXAMPLE
	New-UcsSpFromSpt.ps1 -ucs "1.2.3.4" -saved "myucscred.csv" -spt "HyperV-Hosts" -sp "Test1" -skiperror
	-ucs -- UCS Manager IP address(s) or Host Name(s) -- Example: "1.2.3.4" or "myucs" or "myucs.domain.local" or "1.2.3.4,5.6.7.8" or "myucs1,myucs2" or "myucs1.domain.local,myucs2.domain.local"
	...
	-spt -- Service Profile Template to use
	-sp -- Service Profile to create
...
param(
	...
	[string]$SPT,				# Service Profile Template to use
	[string]$SP,				# Service Profile to create
...
Write-Output "Tell the user what the script does"
Write-Output ""
...
if ($SPT)
	...
	{
		Write-Output ""
		Write-Output "What Service Profile Template would you like to use?"
		$SPT = Read-Host "What Service Profile Template would you like to use?"
	}
...
if ($SP)
	...
	{
		Write-Output ""
		Write-Output "What Service Profile would you like to create?"
		$SP = Read-Host "What Service Profile would you like to create?"
	}
#Add UCS Service Profile from a Template
$CheckTemplate = Get-UcsServiceProfile -Name $SPT
if (!$CheckTemplate)
	{
		Write-Output ""
		Write-Output "The Service Profile Template you requested does not exists: $Template"
		Write-Output "	Exiting..."
		Disconnect-Ucs
		exit
	}

$CheckSP = Get-UcsServiceProfile -Name $SP
if ($CheckSP)
	{
		Write-Output ""
		Write-Output "The Service Profile you requested already exists: $SP"
		Write-Output "	Exiting..."
		Disconnect-Ucs
		exit
	}
	
$CreateSP = Get-UcsServiceProfile -Name $SPT -Org org-root | Add-UcsServiceProfileFromTemplate -NewName @($SP) -DestinationOrg org-root

if ($CreateSP)
	{
		Write-Output ""
		Write-Output "Service Profile: $SP successfully created"
	}
else
	{
		Write-Output ""
		Write-Output "Service Profile: $SP creation failed"
		Write-Output "	Exiting..."
		Disconnect-Ucs
		exit
	}
