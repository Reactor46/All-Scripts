...
function detailsESXiHosts{
    try{
        ...
        if ($exporttoOutBoxESXi.Checked -eq $true){
                Get-VMHost | Select-Object NetworkInfo, Name, Model, Version, LicenseKey, ProcessorType, NumCpu, CpuTotalMhz, CpuUsageMhz, MemoryTotalGB, MemoryUsageGB, ConnectionState, PowerState, Parent, Build | sort-object -property NetworkInfo | Out-Gridview -Title "ESXi/Hosts Inventory" -Verbose
                $outputTextBox.text = "`nFull details of Hosts are been exposed in OutGridView"
        }
        if ($exporttoCSVESXi.Checked -eq $true){
                Get-VMHost | Select-Object NetworkInfo, Name, Model, Version, LicenseKey, ProcessorType, NumCpu, CpuTotalMhz, CpuUsageMhz, MemoryTotalGB, MemoryUsageGB, ConnectionState, PowerState, Parent, Build | Export-Csv "$dir\ESXiHosts-Details.csv" -NoTypeInformation
        ...
        if ($exporttoHTMESXi.Checked -eq $true){
                ...
                $a = $a + "</style>"
                Get-VMHost | Select-Object NetworkInfo, Name, Model, Version, LicenseKey, ProcessorType, NumCpu, CpuTotalMhz, CpuUsageMhz, MemoryTotalGB, MemoryUsageGB, ConnectionState, PowerState, Parent, Build | ConvertTo-HTML -head $a -Title "ESXi/Hosts Inventory" -body "<H2>ESXi/Hosts Inventory</H2>" | Out-File "$dir\ESXiHosts-Details.htm"
...
$file = (get-item "C:\LazyWinAdmin\VMWare\Inventory\Images\esxi.png")
