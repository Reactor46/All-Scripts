...
$users = "jbattista@optummso.com","jcrom@optummso.com" # List of users to email your report to (separate by comma)
$fromemail = "ServerDiskSpace@USON.LOCAL"
$server = "192.168.1.8" #enter your own SMTP server DNS name / IP address here
...
$computers = "USONVSVRFS01.USON.LOCAL"
#$computers = Get-ADComputer -Filter {(OperatingSystem -like "Windows Server*") -and (enabled -eq "true")} -ErrorAction SilentlyContinue |%{$_.Name}
...
$tableFragment= Get-WMIObject  -ComputerName $computers Win32_LogicalDisk -ErrorAction SilentlyContinue `
