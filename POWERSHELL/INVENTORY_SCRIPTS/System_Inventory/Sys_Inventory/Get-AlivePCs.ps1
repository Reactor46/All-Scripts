 ...
 Get-ADComputer -Filter {Operatingsystem -Like 'Windows Server*' -and Enabled -eq 'true'}  -ErrorAction SilentlyContinue -Properties *  |
    Select-Object -ExpandProperty Name | Out-File -FilePath C:\LazyWinAdmin\Inventory\Windows-Servers.txt -Append

 Get-Content C:\LazyWinAdmin\Inventory\Windows-Servers.txt | 
 ForEach { if (test-connection $_ -quiet) { write-output "$_" | Out-File C:\LazyWinAdmin\Inventory\Alive.log -append
  } else { 
  write-output "$_ is Dead!!!" | Out-File C:\LazyWinAdmin\Inventory\Dead.log -append}}
  ...
  function System-Inventory
{
    Begin  {
           ...
           $computers = Get-Content C:\LazyWinAdmin\Inventory\Alive.log #DHCPComputer.txt
    ...
    Process {
            foreach($computer in $computers) {
                                             ...
                                             $CPUSpeed = ""
                                             Try {
                                                 $IP = get-wmiobject win32_networkadapterconfiguration -ComputerName $computer -ErrorAction stop
                                             ...
                                             Catch {
                                                   ...
                                                   $computer | Out-File -FilePath C:\LazyWinAdmin\Inventory\Dead.log -Append
                                                   }
                                             Finally {
