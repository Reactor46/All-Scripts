$Base = "C:\LazyWinAdmin\Inventory\TXT-Results"
$FileNames = "$Base\*.txt"
...
Get-ADComputer -Server LASDC02.fnbm.corp -Filter {Operatingsystem -Like 'Windows Server*' -and Enabled -eq 'true'} -Properties * |
    Select -ExpandProperty Name | Out-FileUtf8NoBom $Base\FNBM.TXT  -Append 
...
Get-Content $Base\FNBM.TXT,$Base\PHX.TXT,$Base\C1B-TST.TXT,$Base\C1B-BIZ.TXT |
...
CD "C:\LazyWinAdmin\Inventory\"
& .\PSSydiScriptRemoteVersion.ps1