...
Function Write-DataTable {
    ...
    If ($Username) { 
        $ConnectionString = "Server={0};Database={1};User ID={2};Password={3};Trusted_Connection=False;Connect Timeout={4}" -f $Computername,$Database,$Username,$ConnectionTimeout 
    }
    Else { 
...
Switch($Domain){
    {$_ -eq 'PHX'} {$DN = 'PHX.Contoso.CORP'}
    {$_ -eq 'BIZ'} {$DN = 'CREDITONEAPP.BIZ'}
    {$_ -eq 'TST'} {$DN = 'CREDITONEAPP.TST'}
    {$_ -eq 'Contoso'} {$DN = 'Contoso.CORP'}
               }

Get-ADSIObject -DomainName $DN -LDAPFilter "(&(&(objectCategory=computer)(OperatingSystem=Windows*Server*)(!(useraccountcontrol:1.2.840.113556.1.4.803:=2))))" | Select DNSHostName
}
...
Function Get-UserShareDACL {
    ...
    Try {    
        ...
        $Shares = Get-WmiObject Win32_LogicalShareSecuritySetting -ComputerName $Computername -ErrorAction SilentlyContinue
...
[CmdletBinding()]
    ...
    process {
        try {
            $wpa = Get-WmiObject SoftwareLicensingProduct -ComputerName $ComputerName -Filter "ApplicationID = '55c92734-d682-4d71-983e-d6ec3f16059f'" -Property LicenseStatus -ErrorAction SilentlyContinue
        } catch {
