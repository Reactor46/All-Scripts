...
$Results = Invoke-SQLCMD2 @SQLParams
...
If ($Results.Name -eq $Null) {
    ...
    $SQLParams.TSQL = "CREATE TABLE $Table  (        
        ComputerName nvarchar (256), 
        InstalledVersion nvarchar (256), 
        DefinitionDate nvarchar (256),
        InventoryDate datetime
    )"
    Invoke-SQLCMD2 @SQLParams
}
