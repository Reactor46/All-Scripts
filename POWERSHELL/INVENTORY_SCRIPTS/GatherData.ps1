Param (
    ...
    [string]$SQLServer = 'LASPSHOST.FNBM.CORP',
    [parameter(Mandatory=$False)]
    [string]$SQLDB = 'ServerInventory',
    [parameter(Mandatory=$True)]
    [ValidateSet('FNBM','PHX','TST','BIZ')]
    [string]$Domain = ''
)
...
Function Invoke-SQLCmd {    
    ...
    Write "Building connection string"
    ...
    Write "Opening connection to $($Computername)"
    $Connection.Open()
    # End Region Make Connection
    # Region Initiate Query
    Write "Initiating query -> $Tsql"
    ...
    If ($PSBoundParameters.ContainsKey('SQLParameter')) {
        $SqlParameter.GetEnumerator() | ForEach {
            Write "Adding SQL Parameter: $($_.Key) with Value: $($_.Value)"
            If ($_.Value -ne $null) { 
    ...
    If ($PSCmdlet.ShouldProcess("Computername: $($Computername) - Database: $($Database)",'Run TSQL operation')) {
        Switch ($CommandType) {
            'Query' {
                Write "Performing Query operation"
            ...
            'NonQuery' {
                Write "Performing Non-Query operation"
...
Get-Server | Start-RSJob -Name {$_} -FunctionsToLoad Get-ScheduledTask, Invoke-SQLCmd, Get-LocalGroup, Get-LocalUser, Get-SecurityUpdate,
    ...
    Write "[$($_)] - Initializing" 
    # Region Variables
    $Computername = $_
    $SQLParams = @{
        Computername = $Using:SQLServer
        Database = $Using:SQLDB
    ...
    # End Region Lookups
    # Region General
    If ($Domain = "PHX"){$Server = "PHXDC03.PHX.FNBM.CORP"}
    elseif ($Domain = "BIZ"){$Server = "LASAUTH01.CREDITONEAPP.BIZ" }
    elseif ($Domain = "TST"){$Server = "LASAUTHTST01.creditoneapp.tst"}
    else {$Server = "LASDC02.FNBM.CORP"}
    ...
    Try {
        ...
        If ($General) {
            Write "[$Computername - $(Get-Date)] Removing old data" 
            ...
            If ($Return.DefaultView) {
                Write "[$Computername - $(Get-Date)] Throwing error" 
                Throw 'FAIL'
            }
            Else {
                $SQLParams.CommandType = 'NonQuery'
                $DataTable = $General | Out-DataTable 
                Write "[$Computername - $(Get-Date)] Updating data" 
                Write-DataTable -Computername $Using:SQLServer -Database $Using:SQLDB -TableName tbGeneral -Data $DataTable -ErrorAction Stop 
            }
        }
    }
    Catch {
        Write "WARNING - [$Computername - $(Get-Date)]" 
        Write-Warning $_
        BREAK
    }
    # End Region General
    ...
    $PageFile = Get-WmiObject Win32_PageFileUsage -ComputerName $Computername | Select Name, FileSystem, @{Name="Allocated_GB";Expression={[math]::round(($_.AllocatedBaseSize/1000),1)}}, @{Name="Peak_GB";Expression={[math]::round(($_.PeakUsage/1000),1)}} 
    ...
    If ($OperatingSystem) {
        ...
        Write-DataTable -Computername $Using:SQLServer -Database $Using:SQLDB -TableName tbOperatingSystem -Data $DataTable 
    }
   ...
   $AV = Try {
    ...
    If ($AV) {
        ...
        Write-DataTable -Computername $Using:SQLServer -Database $Using:SQLDB -TableName tbAntiVirus -Data $DataTable 
    }
    ...
    If ($Memory) {
        ...
        Write-DataTable -Computername $Using:SQLServer -Database $Using:SQLDB -TableName tbMemory -Data $DataTable 
    }
    # End Region Memory
    # Region Network
    ...
    If ($Network) {
        ...
        Write-DataTable -Computername $Using:SQLServer -Database $Using:SQLDB -TableName tbNetwork -Data $DataTable 
    }
    # End Region Network
    # Region CPU
    ...
    If ($Processor) {
        ...
        Write-DataTable -Computername $Using:SQLServer -Database $Using:SQLDB -TableName tbProcessor -Data $DataTable 
    }
    # End Region CPU
    # Region Drives
    ...
    If ($Disk) {
        ...
        Write-DataTable -Computername $Using:SQLServer -Database $Using:SQLDB -TableName tbDrives -Data $DataTable 
    }
    # End Region Drives
    # Region AdminShares
    ...
    If ($AdminShare) {
        ...
        Write-DataTable -Computername $Using:SQLServer -Database $Using:SQLDB -TableName tbAdminShare -Data $DataTable 
    }
    ...
    If ($UserShare) {
        ...
        Write-DataTable -Computername $Using:SQLServer -Database $Using:SQLDB -TableName tbUserShare -Data $DataTable 
    }
    ...
    $Users = Try {
        ...
        If ($Users) {
            ...
            Write-DataTable -Computername $Using:SQLServer -Database $Using:SQLDB -TableName tbUsers -Data $DataTable 
        }
    ...
    $Groups = Try {
        ...
        If ($Groups) {
            ...
            Write-DataTable -Computername $Using:SQLServer -Database $Using:SQLDB -TableName tbGroups -Data $DataTable 
        }
    ...
    If ($ServerRoles) {
        ...
        Write-DataTable -Computername $Using:SQLServer -Database $Using:SQLDB -TableName tbServerRoles -Data $DataTable
    }
    ...
    If ($ScheduledTasks) {
        ...
        Write-DataTable -Computername $Using:SQLServer -Database $Using:SQLDB -TableName tbScheduledTasks -Data $DataTable 
    }
    ...
    If ($Software) {
        ...
        Write-DataTable -Computername $Using:SQLServer -Database $Using:SQLDB -TableName tbSoftware -Data $DataTable 
    }
    # End Region Software
    # Region Updates
    ...
    If ($TotalUpdates) {
        ...
        Write-DataTable -Computername $Using:SQLServer -Database $Using:SQLDB -TableName tbUpdates -Data $DataTable 
    }
    # End Region Updates
