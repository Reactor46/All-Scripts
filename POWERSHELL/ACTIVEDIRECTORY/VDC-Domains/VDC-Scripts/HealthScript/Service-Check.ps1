...
$filename3 = 'C:\LazyWinAdmin\WinSysChecklist\Logs\DomainHealth.FNBM'+ $enddate +'.html'
$filename3A = 'C:\LazyWinAdmin\WinSysChecklist\Logs\DomainHealth.PHX.FNBM'+ $enddate +'.html'
...
$fnbmsvc = Get-Content -Path $Configs\fnbmdls.txt
...
$ReportTitle3= "CreditOne Bank FNBM DataLayerService Memory Usage"
$ReportTitle4= "CreditOne Bank FNBM Collections Agent Time Service"
    ...
    Select-Object MachineName, Status, Displayname |
        Sort MachineName |
            ConvertTo-HTML -As Table -Fragment -PreContent "<h2> $ReportTitle2 </h2>" |
                Out-String
$frag3 = Get-CimInstance Win32_Process -ComputerName $fnbmsvc -Filter $filter -Property * -ErrorAction SilentlyContinue |
    Select $OutPut |
        Sort Server |
...
.\chklst_DCDiag-HTML.ps1 -DomainName "fnbm.corp" -HTMLFileName $filename3
.\chklst_DCDiag-HTML.ps1 -DomainName "phx.fnbm.corp" -HTMLFileName $filename3A
