...
{
    $FromAddress = 'certalert@noreply.mailserver.com'
    $ToAddress = 'your.email@mailserver.com'
    $MessageSubject = "Certificate expiration reminder from $env:COMPUTERNAME.$env:USERDNSDOMAIN"
    $SendingServer = 'relay.mailserver.com'
...
$importall = Import-Csv $exportFileName | Where-Object {$_.'Serial Number' -notcontains 'EMPTY'} | Where-Object {$_.'Certificate Expiration Date' -like "*.$arg0*"} | Select-Object -Property 'Request ID','Serial Number','Requester Name','Certificate Expiration Date','Request Common Name','Request Disposition'
...
foreach($i in $importall){
   $expiry = Get-Date $i.'Certificate Expiration Date' -Format 'd.M.yyyy HH:mm'
