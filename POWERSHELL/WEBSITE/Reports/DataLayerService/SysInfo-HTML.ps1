...
Get-WmiObject win32_computersystem -ComputerName $vComputerName|select PSComputerName,Name, Manufacturer , Domain, Model ,Systemtype,PrimaryOwnerName,PCSystemType,PartOfDomain,CurrentTimeZone,BootupState | ConvertTo-html  -Head $test -Body "<h5>Updated: on $(Get-Date)</h5><h2>ComputerSystem</h2>" >> "$strPath"
...
Get-WmiObject win32_networkadapter -ComputerName $vComputerName | Select Name,Manufacturer,Description ,AdapterType,Speed,MACAddress,NetConnectionID,PNPDeviceID | ConvertTo-Html -Head $test -Body "<h2>Netork Adaptor Information</h2>" >> "$strPath"
Get-WmiObject win32_startupCommand -ComputerName $vComputerName | select Name,Location,Command,User,caption | ConvertTo-html  -Head $test -Body "<h2>Startup  Software Information</h2>" >> "$strPath"
Get-WmiObject win32_logicalDisk -ComputerName $vComputerName | select DeviceID,VolumeName,@{Expression={$_.Size /1Gb -as [int]};Label="Total Size(GB)"},@{Expression={$_.Freespace / 1Gb -as [int]};Label="Free Size (GB)"} | ConvertTo-html  -Head $test -Body "<h2>Disk Inforamtion</h2>" >> "$strPath"
Get-WmiObject win32_operatingsystem -ComputerName $vComputerName | select Caption,Organization,InstallDate,OSArchitecture,Version,SerialNumber,BootDevice,WindowsDirectory,CountryCode | ConvertTo-html  -Head $test -Body "<h2>OS Inforamtion</h2>" >> "$strPath"
