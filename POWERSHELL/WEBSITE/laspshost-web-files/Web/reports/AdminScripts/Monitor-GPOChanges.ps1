...
$HTMLoutputpath = 'C:\Scripts\Repository\jbattista\Web\Reports\GPOMonitor\HTML'
...
Write-Host "Getting events from " $DCServer -ForegroundColor Yellow
 ...
 Try {      $Events = Get-WinEvent -ComputerName $DCServer -FilterXml $XMLQuery -ErrorAction Stop
            ...
            ForEach ($Event in $Events) {
				...
				$MailBody = "When: "+ $date +"<BR>Who: " + $CulpritUserFullName +" <BR>GPO: " +  $GPOFriendlyName  +" <BR>How: " + $Eventmeaning +" <BR>GUID: " + $GPOGUID + " <P><A HREF="http://webconnections/reports/reports/GPOMonitor/FullReportGPOEvents.htm">CLICK HERE FOR MORE INFO</A>" 
				Foreach ($mailaddress in $global:DestMailboxes){
                    EmailReport -CurrentEmailAddress $mailaddress -EmailBody $MailBody
                    }
...
$HTMLHeader3 = '<link rel="stylesheet" type="text/css" href="./css/theme.css">'
...
Add-Content $HTMLFullReportFileName $HTMLHeader3
