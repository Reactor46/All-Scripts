...
{
    $error.clear()
    Write-EventLog -EventId $($count + 700) -LogName Application -Message "Count = $count" -Source "SVC_Lockout.ps1"
}
...
$scriptDir = "C:\Scripts\Repository\jbattista\Web\reports\Lockouts\SVC\"


# Start Transcript.
If( -not (Test-Path "$scriptDir\Svc_Transcripts")){New-Item -ItemType Directory -Path "$scriptDir\Svc_Transcripts"}
Start-Transcript -Path ("$scriptDir\Svc_Transcripts\{0:yyyyMMdd}_.Log"  -f $(get-date)) -Append 
...
$MonitorAccounts = $True # (CHANGE ME)

# Build website for monitoring.
$BuildWebPage = $True # (CHANGE ME)
...
# This is used during the inital query to append descriptions to the source. For Example if the sourece is a server called 
...
{
    # Email Alerts
    $SMTPServer = "mailgateway.Contoso.corp" # (CHANGE ME)

    # The address from which to send the lockout alerts.
    $SMTPFrom = "Account_Lockout@creditone.com" # (CHANGE ME)

    # Monitored accounts (CHANGE ME)
    # List of monitored groups and the Email address to which the alert will be sent. Each member of the moitored group is a monitored accout, 
    ...
    # $Monitor['Monitored_Service_Account_Lockouts'] = "john.battista@creditone.com" # (CHANGE ME)
    $Monitor['OU=Service_Accounts,DC=contoso,DC=com'] = "john.battista@creditone.com" # (CHANGE ME)
}
...
If($BuildWebPage)
{
    ...
    $SiteURL="WEBCONNECTIONS" # (CHANGE ME)

    # The Path to the site directory on the server
    $SiteDir = "C:\Scripts\Repository\jbattista\Web\reports\Lockouts\SVC" # (CHANGE ME)
...
# Time presets
# Age of events to query (minutes). 6 minute preset is for a scheduled task running every 5 minutes.
$EventStart = 6 
...
$DataFile = "$scriptDir\Svc_CurrDATA.csv"
$EventLog = "$scriptDir\Svc_EventLog.csv"
$htmlfile = "$SiteDir\Svc_AccountLockouts.htm"
...
# Query the domain controller for lockout or unlock events, set and return the five fields for each event (User,Source,Time,EventType,Controller).
$Servers = "LASDC01.Contoso.corp","LASDC02.Contoso.corp","LASDC05.Contoso.corp"
$GetEvents = @(foreach($server in $Servers){ 
...
If($MonitorAccounts)
{
    # Loop through all the entries in the $monitor hash table, checking for accounts that require alerts.
    foreach($key in $Monitor.Keys)
    {
        foreach($event in $NewEvents)
        {
            # Verify this event has not been seen before so as to avoid repeat alerts.
            if(((Get-ADOrganizationalUnit "$key").members -contains (Get-ADUser $($event.user)).distinguishedname) -AND `
            (($OldEvents |?{$event.User -like $_.User -AND $event.Source -like $_.Source -AND $event.Time -like $_.Time -AND $event.EventType -like $_.EventType}).count -eq 0))
            {
                ...
                Write-Output "$($event.user) is a monitored service account, sending notification...."
...
If($BuildWebPage)
{
    ...
    -PageHeader "<P><FONT FACE='arial'><img src=$LogoWebPath alt='Logo' style='float:left' /><H2>Service Account Lockouts - Last $HistTime Days </H2> `
...
Get-ChildItem "$scriptDir\Svc_Transcripts" | Where {$_.LastWriteTime -lt $(get-date).adddays(-30)} | Remove-Item
