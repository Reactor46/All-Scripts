                ...
                {  
                    ...
                    if ($property.value)
                    {
                        if ($property.value -isnot [System.DBNull]) {
                            $Col.DataType = [System.Type]::GetType("$(Get-Type -type $property.TypeNameOfValue)")
                         }
                    }
                    $DT.Columns.Add($Col)
                }  
...
Function Write-DataTable {
    ...
    Try {
        $SQLConnection.Open()
        $bulkCopy = New-Object -TypeName Data.SqlClient.SqlBulkCopy -ArgumentList $SQLConnection, ([System.Data.SqlClient.SqlBulkCopyOptions]::TableLock),$Null
    ...
    Catch {
        Write-Error -Message "$($TableName): $($_)"
    }
    Finally {
        $SQLConnection.Close()
    }
...
Function Invoke-SQLCmd {    
    ...
    If ($PSBoundParameters.ContainsKey('Verbose')) {
        $Handler = [System.Data.SqlClient.SqlInfoMessageEventHandler] {
            Param($sender, $event) 
            Write-Verbose -Message $event.Message -Verbose
        }
    ...
    Write-Verbose -Message "Opening connection to $($Computername)"
    $Connection.Open()
    # End Region Make Connection

    # Region Initiate Query
    Write-Verbose -Message "Initiating query -> $Tsql"
    $Command=New-Object -TypeName system.Data.SqlClient.SqlCommand -ArgumentList ($Tsql,$Connection)
    If ($PSBoundParameters.ContainsKey('SQLParameter')) {
        $SqlParameter.GetEnumerator() | ForEach-Object {
            Write-Verbose -Message "Adding SQL Parameter: $($_.Key) with Value: $($_.Value)"
            If ($_.Value -ne $null) { 
    ...
    If ($PSCmdlet.ShouldProcess("Computername: $($Computername) - Database: $($Database)",'Run TSQL operation')) {
        Switch ($CommandType) {
            'Query' {
                Write-Verbose -Message 'Performing Query operation'
            ...
            'NonQuery' {
                Write-Verbose -Message 'Performing Non-Query operation'
...
Function Get-LocalUser {
    ...
    Function ConvertTo-SID {
        ...
        (New-Object -TypeName System.Security.Principal.SecurityIdentifier -ArgumentList ($BinarySID,0)).Value
    }
    $adsi = [ADSI]"WinNT://$Computername"
    $adsi.Children | Where-Object {$_.SchemaClassName -eq 'user'} |
    Select-Object -Property @{L='Computername';E={$Computername}}, @{L='Name';E={$_.Name[0]}}, 
...
Function Get-LocalGroup {
    ...
    $adsi = [ADSI]"WinNT://$Computername"
    $adsi.Children | Where-Object {$_.SchemaClassName -eq 'group'} | 
    Select-Object -Property @{L='Computername';E={$Computername}},@{L='Name';E={$_.Name[0]}},
    ...
    Process {     
        ForEach ($Computer in $Computername){ 
            If (Test-Connection -ComputerName $Computer -Count 1 -Quiet) {
                $Paths = @('SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall','SOFTWARE\\Wow6432node\\Microsoft\\Windows\\CurrentVersion\\Uninstall')         
                ForEach($Path in $Paths) { 
                    Write-Verbose -Message "Checking Path: $Path"
                    # Create an instance of the Registry Object and open the HKLM base key 
                    Try { 
                                ...
                                If ($DisplayName -AND $DisplayName -notmatch '^Update for|rollup|^Security Update|^Service Pack|^HotFix') {
                                    ...
                                    If ($Date) {
                                        Try {
				                            ...
				                            Write-Warning -Message "$($Computer): $_ <$($Date)>"
                                            $Date = $Null
                                        }
                                    } 
                                    # Create New Object with empty Properties 
                ...
                Write-Error -Message "$($Computer): unable to reach remote system!"
            }
        } 
    } 
} 
...
Function Get-UserShareDACL {
    ...
    Try {    
        Write-Verbose -Message "Computer: $($Computername)"
        ...
        ForEach ($Share in $Shares) {
            ...
            Write-Verbose -Message "Share: $($Share.name)"
...
Function Get-AdminShare {
    ...
    Get-WmiObject @WMIParams | Select-Object -Property Name, Path, Description, 
...
[CmdletBinding()]
    ...
    process {
        try {
            $wpa = Get-WmiObject -Class SoftwareLicensingProduct -ComputerName $ComputerName -Filter "ApplicationID = '55c92734-d682-4d71-983e-d6ec3f16059f'" -Property LicenseStatus -ErrorAction Stop
        } catch {
...
Get-Server | Start-RSJob -Name {$_} -FunctionsToLoad Get-ScheduledTask, Invoke-SQLCmd, Get-LocalGroup, Get-LocalUser, Get-SecurityUpdate, Get-Software, Get-AdminShare, Get-UserShareDACL, Convert-ChassisType, Write-DataTable, Get-SEPVersion, Get-ActivationStatus, Out-DataTable -ScriptBlock {
    Write-Verbose -Message "[$($_)] - Initializing" -Verbose
    # Region Variables
    $Computername = $_
    $SQLParams = @{
        Computername = 'laspshost.Contoso.corp'
        Database = 'ServerInventory'
    ...
    Try {
        ...
        $Enclosure = Get-WmiObject -Class Win32_SystemEnclosure -ComputerName $Computername
        ...
        $General = [pscustomobject]@{
            ...
            ChassisType = (Convert-ChassisType -ChassisType $Enclosure.ChassisTypes)
        ...
        If ($General) {
            Write-Verbose -Message "[$Computername - $(Get-Date)] Removing old data" -Verbose
            $SQLParams.CommandType = 'NonQuery'
            $SQLParams.TSQL = 'DELETE FROM tbGeneral WHERE Computername = @Computername'
            $SQLParams.TSQL = 'DELETE FROM tbGeneral WHERE InventoryDate < DATEADD(dd,-15,GETDATE())'            
            Invoke-SQLCmd @SQLParams
            If ($Return.DefaultView) {
                Write-Verbose -Message "[$Computername - $(Get-Date)] Throwing error" -Verbose
                Throw 'FAIL'
            }
            Else {
                $SQLParams.CommandType = 'NonQuery'
                $DataTable = $General | Out-DataTable
                Write-Verbose -Message "[$Computername - $(Get-Date)] Updating data" -Verbose
    ...
    Catch {
        Write-Verbose -Message "WARNING - [$Computername - $(Get-Date)]" -Verbose
    ...
    $PageFile = Get-WMIObject -Class win32_PageFile -ComputerName $Computername 
    ...
    If ($OperatingSystem) {
        ...
        $SQLParams.TSQL = 'DELETE FROM tbOperatingSystem WHERE InventoryDate < DATEADD(dd,-15,GETDATE())'
        Invoke-SQLCmd @SQLParams
    ...
    # Region AntiVirus
   $AV = Try {
        Get-SEPVersion -ComputerName $Computername -ErrorAction Stop  | ForEach-Object {
        [pscustomobject]@{
    ...
    If ($AV) {
        ...
        $SQLParams.TSQL = 'DELETE FROM tbAntiVirus WHERE InventoryDate < DATEADD(dd,-15,GETDATE())'
        Invoke-SQLCmd @SQLParams
        $DataTable = $AV | Out-DataTable
        Write-DataTable -Computername $Using:SQLServer -Database ServerInventory -TableName tbAntiVirus -Data $DataTable 
    }
    # End Region AntiVirus

    # Region Memory
    $Memory = @(Get-WmiObject -Class Win32_PhysicalMemory -ComputerName $Computername -ErrorAction Stop| ForEach-Object {
        [pscustomobject]@{
            Computername = $Computername
            DeviceID = $_.tag
    ...
    If ($Memory) {
        ...
        $SQLParams.TSQL = 'DELETE FROM tbMemory WHERE InventoryDate < DATEADD(dd,-15,GETDATE())'
        Invoke-SQLCmd @SQLParams
    ...
    # Region Network
    $Network = @(Get-WmiObject -Class Win32_NetworkAdapterConfiguration -Filter "IPEnabled='True'" -ComputerName $Computername -ErrorAction Stop) | ForEach-Object {
        [pscustomobject]@{
            Computername = $Computername
    ...
    If ($Network) {
        ...
        $SQLParams.TSQL = 'DELETE FROM tbNetwork WHERE InventoryDate < DATEADD(dd,-15,GETDATE())'
        Invoke-SQLCmd @SQLParams
    ...
    # Region CPU
    $Processor = @(Get-WmiObject -Class Win32_Processor -ComputerName $Computername -ErrorAction Stop | ForEach-Object {
        [pscustomobject]@{
            Computername = $Computername
            DeviceID = $_.DeviceID
    ...
    If ($Processor) {
        ...
        $SQLParams.TSQL = 'DELETE FROM tbProcessor WHERE InventoryDate < DATEADD(dd,-15,GETDATE())'
        Invoke-SQLCmd @SQLParams
    ...
    # Region Drives
    $Disk = @(Get-WmiObject -Class Win32_Volume -Filter "(Not Name LIKE '\\\\?\\%')" -ComputerName $Computername -ErrorAction Stop | ForEach-Object {
        [pscustomobject]@{
            Computername = $Computername
            Drive = $_.Name
    ...
    If ($Disk) {
        ...
        $SQLParams.TSQL = 'DELETE FROM tbDrives WHERE InventoryDate < DATEADD(dd,-15,GETDATE())'
        Invoke-SQLCmd @SQLParams
    ...
    # Region AdminShares
    $AdminShare = @(Get-AdminShare -Computername $Computername -ErrorAction Stop | ForEach-Object {
        [pscustomobject]@{
            Computername = $Computername
            Name = $_.Name
            Path = $_.Path
    ...
    If ($AdminShare) {
        ...
        $SQLParams.TSQL = 'DELETE FROM tbAdminShare WHERE InventoryDate < DATEADD(dd,-15,GETDATE())'
        Invoke-SQLCmd @SQLParams
    ...
    # Region UserShares
    $UserShare = Try {
    Get-UserShareDACL -Computername $Computername -ErrorAction Stop | Select-Object -Property *,@{L='InventoryDate';E={$Date}}
    } 
    Catch {}
    If ($UserShare) {
        $SQLParams.TSQL = 'DELETE FROM tbUserShare WHERE Computername = @Computername'
        $SQLParams.TSQL = 'DELETE FROM tbUserShare WHERE InventoryDate < DATEADD(dd,-15,GETDATE())'
        Invoke-SQLCmd @SQLParams
    ...
    If ($Using:ServerGroup -eq 'MemberServer') {
        $Users = @(Get-LocalUser -ComputerName $Computername -ErrorAction Stop) | Select-Object -Property *,@{L='InventoryDate';E={$Date}}

        If ($Users) {
            $SQLParams.TSQL = 'DELETE FROM tbUsers WHERE Computername = @Computername'
            $SQLParams.TSQL = 'DELETE FROM tbUsers WHERE InventoryDate < DATEADD(dd,-15,GETDATE())'
            Invoke-SQLCmd @SQLParams
    ...
    # Region Local Groups
    If ($Using:ServerGroup -eq 'MemberServer') {
        $Groups = @(Get-LocalGroup -ComputerName $Computername -ErrorAction Stop) | Select-Object -Property *,@{L='InventoryDate';E={$Date}}

        If ($Groups) {
            $SQLParams.TSQL = 'DELETE FROM tbGroups WHERE Computername = @Computername'
            $SQLParams.TSQL = 'DELETE FROM tbGroups WHERE InventoryDate < DATEADD(dd,-15,GETDATE())'
            Invoke-SQLCmd @SQLParams
    ...
    # Region Server Roles
    $ServerRoles = Try {
        Get-WmiObject -Class Win32_ServerFeature -ComputerName $Computername -ErrorAction Stop | ForEach-Object {
            [pscustomobject]@{
                Computername = $Computername
                ID = $_.Id
                Name = $_.Name
    ...
    If ($ServerRoles) {
        ...
        $SQLParams.TSQL = 'DELETE FROM tbServerRoles WHERE InventoryDate < DATEADD(dd,-15,GETDATE())'
        Invoke-SQLCmd @SQLParams
    ...
    $ScheduledTasks = Get-ScheduledTask -Computername $Computername -ErrorAction Stop | Select-Object -Property *,@{L='InventoryDate';E={$Date}}

    If ($ScheduledTasks) {
        $SQLParams.TSQL = 'DELETE FROM tbScheduledTasks WHERE Computername = @Computername'
        $SQLParams.TSQL = 'DELETE FROM tbScheduledTasks WHERE InventoryDate < DATEADD(dd,-15,GETDATE())'
        Invoke-SQLCmd @SQLParams
    ...
    # Region Software
    $Software = Get-Software -Computername $Computername -ErrorAction Stop | Sort-Object -Property DisplayName | Select-Object -Property *,@{L='InventoryDate';E={$Date}}

    If ($Software) {
        $SQLParams.TSQL = 'DELETE FROM tbSoftware WHERE Computername = @Computername'
        $SQLParams.TSQL = 'DELETE FROM tbSoftware WHERE InventoryDate < DATEADD(dd,-15,GETDATE())'
        Invoke-SQLCmd @SQLParams
    ...
    $Updates = Get-SecurityUpdate -Computername $Computername -ErrorAction Stop | 
        Select-Object -Property @{L='Computername';E={$Computername}},Description, HotFixID, InstalledOn, Type,@{L='InventoryDate';E={$Date}} | 
        Group-Object -Property HotFixID | ForEach-Object {$_.Group | Sort-Object -Unique -Property DisplayName}
    ...
    If ($TotalUpdates) {
        ...
        $SQLParams.TSQL = 'DELETE FROM tbUpdates WHERE InventoryDate < DATEADD(dd,-15,GETDATE())'
        Invoke-SQLCmd @SQLParams
...
Get-RSJob | Remove-RSJob -Force -Verbose
