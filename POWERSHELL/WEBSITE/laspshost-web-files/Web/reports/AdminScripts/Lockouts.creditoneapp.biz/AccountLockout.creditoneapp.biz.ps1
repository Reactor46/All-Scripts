...
{
    $error.clear()
    Write-EventLog -EventId $($count + 700) -LogName Application -Message "Count = $count" -Source "AccountLockout.creditoneapp.biz.ps1"
}
...
$scriptDir = "C:\Scripts\Repository\jbattista\Web\reports\AdminScripts\Lockouts.creditoneapp.biz"

# Start Transcript.
If( -not (Test-Path "$scriptDir\Transcripts")){New-Item -ItemType Directory -Path "$scriptDir\Transcripts"}
Start-Transcript -Path ("$scriptDir\Transcripts\{0:yyyyMMdd}_.Log"  -f $(get-date)) -Append
...
If($MonitorAccounts)
{
    ...
    $Monitor['Users'] = "john.battista@creditone.com" # (CHANGE ME)
}
...
If($BuildWebPage)
{
    ...
    $SiteDir = "C:\Scripts\Repository\jbattista\Web\reports\AdminScripts\Lockouts.creditoneapp.biz" # (CHANGE ME)
...
$htmlfile = "$SiteDir\AccountLockouts.creditoneapp.biz.htm"
...
##### MAIN #####
################
#$Users = Get-ADUser -Filter * -SearchBase "OU=Service_Accounts,DC=contoso,DC=com" -SearchScope Subtree | Select -ExpandProperty SamAccountName
...
$GetEvents = @(foreach($server in (Get-ADDomainController -filter * -server "lasauth01.creditoneapp.biz" | select -ExpandProperty Hostname)){ 
...
{
    $NewEvents = @($GetEvents)# | Select -ExpandProperty User | Get-ADUser $_.SamAccountName | Where-Object {$_.DistinguishedName -like "OU=Service_Accounts,DC=contoso,DC=com"})
