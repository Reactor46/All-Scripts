 <#
	This script will run the function Get-UserInfoHTML and bring the CSV or HTML output from your AD
	
	Parameter description:
		This script will receive a parameter called "Selector" 
		it accepts 3 options: HTML (default), CSV or BOTH
		if you run the script like:
			- .\GetUsersInfoHTML.ps1 -Selector HTML => Same as default option and you will get the HTML5 report with all the properties of all users in your AD environment
			- .\GetUsersInfoHTML.ps1 -Selector CSV  => You will get a CSV file with all the properties of all users in your AD environment, ready to import into excel or use it as feed to another powershell script (custom actions, even email send)
			- .\GetUsersInfoHTML.ps1 -Selector BOTH => Will run both reports HTML and CSV.
	how to use this script?
	
	* Get simple HTML5 report
		.\GetUsersInfoHTML.ps1
		.\GetUsersInfoHTML.ps1 -Selector HTML 
		 
	* Get Simple CSV file 
		.\GetUsersInfoHTML.ps1 -Selector CSV 
		
	* Get CSV and HTML5 report
		.\GetUsersInfoHTML.ps1 -Selector BOTH 
	
    Even you can see the date of the last bad logon attempt of each users. That's the power of powershell.
#>
[CmdletBinding()]
param(
...
function Get-UserInfoHTML{
 ...
 BEGIN{
    ...
    $FnHTML= "C:\inetpub\wwwroot\Report\Users\UsersInfo-$TodayDate.html"
    $FinalHTML = "C:\inetpub\wwwroot\Report\UsersInfo.html"
    $FnCSV = "C:\inetpub\wwwroot\Report\Users\UsersInfo-$TodayDate.csv"
...
<body>
	<h1>User Report for Contoso.CORP on $DateTime</h1>
...
<br>
 ...
 END{
    if($Selector -eq "HTML" -or $Selector -eq "BOTH"){
        $html | Out-File "$FnHTML" 
    }
    if($Selector -eq "CSV" -or $Selector -eq "BOTH"){
        $AllUsersFilter | ConvertTo-Csv -NoTypeInformation | out-file "$FnCSV" 
    }
 }
 }
...
$FnHTML= "C:\inetpub\wwwroot\Report\Users\UsersInfo-$TodayDate.html"
$FinalHTML = "C:\inetpub\wwwroot\Report\UsersInfo.html"
Copy-Item -Path "$FnHTML" -Destination "$FinalHTML"
