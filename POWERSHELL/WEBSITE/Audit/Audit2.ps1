#########################################################
#							#
#    Usage: Audit2.ps1 'pathtolistofservers'         	#
#							#
...
Foreach ($Target in $Targets){
	...
	$Keyboards = Get-CIMInstance -computername $Target Win32_Keyboard
	...
	$colDisks = Get-CIMInstance -ComputerName $Target Win32_LogicalDisk
	Write-Output "..Network Configuration"
	$NICCount = 0
	$colAdapters = Get-CIMInstance -ComputerName $Target Win32_NetworkAdapterConfiguration
	Write-Output "..Local Shares"
	$colShares = Get-CIMInstance -ComputerName $Target Win32_Share
	Write-Output "..Printers"
	$colInstalledPrinters =  Get-CIMInstance -ComputerName $Target Win32_Printer
	Write-Output "..Services"
	$colListOfServices = Get-CIMInstance -ComputerName $Target Win32_Service
	Write-Output "..Event Log Settings"
	$colLogFiles = Get-CIMInstance -ComputerName $Target Win32_NTEventLogFile
	...
	$colLoggedEvents = Get-CIMInstance -computer $Target -query ("Select * from Win32_NTLogEvent Where Type='Error' and TimeWritten >='" + $WmidtQueryDT + "'")
	Write-Output "..Event Log Warnings"
	$WmidtQueryDT = [System.Management.ManagementDateTimeConverter]::ToDmtfDateTime([DateTime]::Now.AddDays(-14))
	$colLoggedEvents = Get-CIMInstance -computer $Target -query ("Select * from Win32_NTLogEvent Where Type='Warning' and TimeWritten >='" + $WmidtQueryDT + "'")

	$Filename = "C:\Scripts\Repository\jbattista\Web\Audit\TST\" + $Target + "_" + $date.Hour + $date.Minute + "_" + $Date.Day + "-" + $Date.Month + "-" + $Date.Year + ".htm"
	...
	<font face="Arial" size="1"><b><i>Report Created by C1B WinSys</i></b></font><br>
...
$Report+= @"
		...
		<DIV class=filler></DIV>
"@	
	if ((Get-CIMInstance -namespace "root/cimv2" -list) | ? {$_.name -match "Win32_Product"})
	{
		Write-Output "..Installed Software"
		$colApps = Get-CIMInstance -ComputerName $Target Win32_Product | select Name,Version,Vendor,InstallDate
