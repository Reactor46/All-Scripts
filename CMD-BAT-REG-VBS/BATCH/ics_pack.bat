...
SET packCmd=%ics_packAppPath%\7zip\7z.exe a -y -ssw %recursionOptn% %ics_archType% %passOptn%

%~d1

CD %ics_dirSrcRoot%

%packCmd% %excludeOptn% %ics_myWorkDir%\%ics_toPack% %toExport% > %ics_myWorkDir%\ics_pack.out

IF %ERRORLEVEL% EQU 1 (
SET exitCode=0
) ELSE (SET exitCode=%ERRORLEVEL%)

TYPE %ics_myWorkDir%\ics_pack.out | FINDSTR /C:"cannot access" > %ics_myWorkDir%\ics_pack.warr
