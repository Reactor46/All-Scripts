...
'Const strVersion="12.39" '(toddlutt 2/12/2010)
Const strVersion="14.00" '(nassersa 6/1/2011)
...
'Assign Constants
Const strUsageInfo = "Usage: cscript ProcessTrackingLog.vbs <LogFilePath> <NumFiles> <hub|edge|all> [ <mm/dd/yyyy> | today | yesterday ] [UtcStartTimestamp] [UtcEndTimestamp]"
Const bComputeUniqueRecipientStats = TRUE
Const bFilterByFolderName = TRUE
...
Const bArchiveUncompressed = TRUE					'create uncompressed archive if ZIP not found
...
Const iMaxLogAge = 7								'Maximum age (in days) of log files (one created per day)

Set WshShell = CreateObject("WScript.Shell")
strUserProfile = WshShell.ExpandEnvironmentStrings("%USERPROFILE%")
sResultPath = "c:\temp\MSGTRACK\Output\"
'sResultPath = strUserProfile & "\scripts\Output\"
sArchivePath = sResultPath & "Archive\"
sExchangeVersion = "Exchange 2010"  'default to latest

'Assign Variables
bDebugOut = FALSE
'UTC Start Filter (default value 12:00:00 AM)
sStatTimeStart = "12:00:00 AM"
'UTC End Filter (default value 11:59:59 PM)
sStatTimeEnd = "11:59:59 PM"
...
iTopRecipientLight = 20
iTopRecipientMedium = 40
iTopRecipientHeavy = 80
iTopRecipientVeryHeavy = 120
iTopSenderLight = 5
iTopSenderMedium = 10
iTopSenderHeavy = 20
iTopSenderVeryHeavy = 30
iTotalRecipientLimit = 250
...
sTopSenderResultsDistribution = sResultPath & "MTTopSendersbySubmitResultsDistribution.csv"
...
sTopRecipientResultsDistribution = sResultPath & "MTTopRecipientResultsDistribution.csv"
...
sLatencyTrackerDailyResults = sArchivePath & "MTLatencyTrackerDailyResults.csv"
sRecipientLimitDictionaryFile = sArchivePath & "MTRecipientLimitResults.csv"
...
sAcceptedDomainList = sArchivePath & "Get-AcceptedDomain.log" 'output from "Get-AcceptedDomain > Get-AcceptedDomain.log"
sMailSubmissionDistribution = sResultPath & "MTMailSubmissionDistribution.csv"
sTopDomainSenderResults = sResultPath & "MTTopDomainSenderResults.csv"
sTopDomainRecipientsResults = sResultPath & "MTTopDomainRecipientsResults.csv"
sRecipientStatisticsResults = sResultPath & "MTRecipientStatistics.csv"
...
iReceiveStoreDriverRecipientInAcceptedDomainCount = 0
iEventCount = 0 
iTotalMsgBytes = 0
iErrorEvent = 0
iDuplicateDeliver = 0
...
iTransferCount = 0
iExpandCount = 0
iDuplicateExpandCount = 0
iFailCount = 0
iRedirectCount = 0
iDuplicateRedirectCount = 0
iBadmailCount = 0
iDeferCount = 0
iSplitError = 0
...
Set oAcceptedDomainDictionary = CreateObject("Scripting.Dictionary")
Set objTopDomainSendersDictionary = CreateObject("Scripting.Dictionary")
Set objTopDomainRecipientsDictionary = CreateObject("Scripting.Dictionary")
Set objRecipientStatisticsDictionary = CreateObject("Scripting.Dictionary")
Set objRecipientLimitDictionary = CreateObject("Scripting.Dictionary")
Set oLatencyTrackerDailyDictionary = CreateObject("Scripting.Dictionary")
...
If oArgs.count >= 3 Then
	...
	Select Case oArgs.count
		...
		Case 4
			...
			bFilterTimestamps = TRUE
			wscript.echo "Filter Date: " & strFilterDate
		Case 6
			strFilterDate = oArgs(3)
			If lcase(strFilterDate) = "today" Then strFilterDate = cstr(date)
			If lcase(strFilterDate) = "yesterday" Then strFilterDate = cstr(date - 1)
			bFilterTimestamps = TRUE
			sStatTimeStart = oArgs(4)
			sStatTimeEnd = oArgs(5)
		Case Else
			'Must have 3 or 4 or 6 args
...
If bFilterTimestamps Then
	sStatTimeStart = strFilterDate & " " & sStatTimeStart	
	sStatTimeEnd = strFilterDate & " " & sStatTimeEnd
	wscript.echo "Filtering logs between " & sStatTimeStart & " and " & sStatTimeEnd
End If
...
If fso.FileExists(sSummaryResults) Then
	wscript.echo "attempting to delete" & sSummaryResults
	fso.DeleteFile(sSummaryResults)
	If Err.number <> 0 Then 
		Wscript.echo "Unable to delete file " & sSummaryResults & " (" & err.Description & ")"
		Wscript.echo "Exiting Script"
		Wscript.quit
	End If
End If
...
If fso.FileExists(sRunTimeLog) Then 
	fso.DeleteFile(sRunTimeLog)
End If
...
LogText vbcrlf 
If iNumFiles = 0 AND lcase(strRole) = "all" AND strFilterDate <> "" Then
	'LogText "LatencyTracker Daily Summary Results will be written"
	bWriteLatencyDailySummary = TRUE
	If LoadLatencyTrackerDailySummary(oLatencyTrackerDailyDictionary) Then
		LogText "Loaded " & oLatencyTrackerDailyDictionary.Count & " entries from " & sLatencyTrackerDailyResults
	Else
		LogText "Error in LoadLatencyTrackerDailySummary"
		
	End If
	'Load recipient limit dictionary
	If LoadRecipientLimitDictionary(objRecipientLimitDictionary) Then
		LogText "Loaded " & objRecipientLimitDictionary.Count & " entries from " & sRecipientLimitDictionaryFile
	Else
		LogText "Error in LoadRecipientLimitDictionary"
	End If
Else
	bWriteLatencyDailySummary = FALSE
End If
...
If fso.FileExists(sTopSenderResultsDistribution) Then fso.DeleteFile(sTopSenderResultsDistribution)
Set oTopSenderResultsDistribution = fso.OpenTextFile(sTopSenderResultsDistribution,ForAppending,1)
...
If fso.FileExists(sTopRecipientResultsDistribution) Then fso.DeleteFile(sTopRecipientResultsDistribution)
Set oTopRecipientResultsDistribution = fso.OpenTextFile(sTopRecipientResultsDistribution,ForAppending,1)
...
oDeliveryLatencyExceptionResults.writeline "MessageId,OriginalArrivalTime,TotalBytes,Sender,RecipCount,DeliverCount,MinDeliveryLatency,MaxDeliveryLatency,ClientName,MessageInfo"
...
oExpandFileOut.writeline "Number,Address,MinRecipCount,MaxRecipCount,ExpandCount,AvgExpansionLatency"
...
If fso.FileExists(sTopDomainSenderResults) Then fso.DeleteFile(sTopDomainSenderResults)
If fso.FileExists(sTopDomainRecipientsResults) Then fso.DeleteFile(sTopDomainRecipientsResults)
If fso.FileExists(sRecipientStatisticsResults) Then fso.DeleteFile(sRecipientStatisticsResults)
...
If fso.FileExists(sAcceptedDomainList) Then
	If GetAcceptedDomains(sAcceptedDomainList) Then
		LogText now & vbtab & "INFO" & vbtab & "Done Loading " & oAcceptedDomainDictionary.Count & " accepted domains"
	Else
		LogText now & vbtab & "ERROR" & vbtab & "Unable to load Accepted Domains from '" & sAcceptedDomainList & "'"	
	End If
Else
	LogText now & vbtab & "ERROR" & vbtab & "Accepted Domain Log '" & sAcceptedDomainList & "' does not exist"	
End If
...
Function GetFolders(byRef sLogFilePath)
		...
		GetFolders = FALSE
		If instr(1,lcase(sLogFilePath),"\index") Then
			LogText vbcrlf & now & vbtab & "SKIP" & vbtab & "GetFolders skipping Files in " & sLogFilePath
			GetFolders = TRUE
			Exit Function
		End If
		...
		For each oSubFolder in oSubFolders
			...
			Select Case ucase(strRole)
				...
				Case "HUB"
					If instr(1,ucase(oSubFolder.path),"HUB")>0 OR instr(1,ucase(oSubFolder.path),"EXMLT")>0 OR instr(1,ucase(oSubFolder.path),"EX14MLT")>0 OR instr(1,ucase(oSubFolder.path),"EXPFS")>0 Then
				...
				Case "EX14"
					If instr(1,ucase(oSubFolder.path),"EX14")>0 Then
						GetFolders(oSubFolder.path)
					Else
						LogText now & vbtab & "SKIP" & vbtab & "GetFolders: Not an EX14MLT server, skipping Files in " & oSubFolder.path
					End If
				...
				Case Else
					If bFilterByFolderName Then
						If instr(1,ucase(oSubFolder.path),ucase(strRole))>0 Then
							GetFolders(oSubFolder.path)
						Else
							LogText now & vbtab & "SKIP" & vbtab & "GetFolders: Folder name does not contain """ & strRole & """, skipping Files in " & oSubFolder.path
						End If					
					Else
	...
	Do While oLogFile.AtEndOfStream <> True
		...
		if instr(1,left(sFileLine,1),"#") Then
			...
			If instr(1,left(sFileLine,5),"#Date") Then
				...
				If strFilterDate="" OR ((DateDiff("s",sStatTimeStart,sLogDateTime)>=0 AND DateDiff("s",sStatTimeEnd,sLogDateTime)<=0)) Then
					...
					End If
				Else
					LogText now & vbtab & "DATE" & vbtab & "ParseFile " & iEnterParseFileFunctionTotal & " Log file date (" & sLogDateTime & " UTC) is not between " & sStatTimeStart & " and " & sStatTimeEnd & " (" & iEnterFunction & " of " & iMyNumFiles & " files)"		
					Exit Do
				End If
																						
			ElseIf instr(1,left(sFileLine,7),"#Fields") Then 
				sHeaderLine = sFileLine		
				aHeaderLine = split(sHeaderLine,",")
			ElseIf instr(1,left(sFileLine,9),"#Version:") Then
			    If instr(1,left(sFileLine,12),"#Version: 14") Then
				   sExchangeVersion = "Exchange 2010"
				Else
                   sExchangeVersion = "Exchange 2007"
                End If				   
			Else
				'do nothing
			End If
		...
		Elseif instr(1,sFileLine,",") Then
					...
					If ubound(aFileLine) <> ubound(aHeaderLine) Then 
						...
						Else
							...
							If (ExtractEventInfo(aFileLine)) Then
...
Function ExtractEventInfo(byRef aFileLine)
	...
	bStoreDriverFoundDiagStr = FALSE
	
	sCurrentEventTime=GetDateTime(aFileLine(iEventTime))
	If strFilterDate <> "" Then
		If (DateDiff("s",sStatTimeStart,sCurrentEventTime)>=0 AND DateDiff("s",sStatTimeEnd,sCurrentEventTime)<=0) Then
			'wscript.echo "Process event with timestamp " & sCurrentEventTime
			ExtractEventInfo = TRUE
		Else
			'wscript.echo "Do Not Process event with timestamp " & sCurrentEventTime
			ExtractEventInfo = FALSE
			Exit Function
		End If
	End If
	...
	Select Case aFileLine(iEventID)
		...
		Case "RECEIVE"
			...
			Select Case aFileLine(iSource)
				Case "SMTP"
						...
						If instr(1,lcase(aFileLine(iConnectorId)),"client")>0 Then
							'Track MUA submissions using Client receive connector
							Call TrackTopDomain(aFileLine(iEventTime), aFileLine(iSender), aFileLine(iTotalBytes), objTopDomainSendersDictionary)
						End If
						If aFileLine(iMessageInfo)="06I:" OR aFileLine(iMessageInfo)="07I:" Then
							'Track Recipient Statistics when MessageInfo indicates AUTH LOGIN (06I: or 07I:)
							Call UpdateRecipientStatistics(aFileLine,aFileLine(iSender),"ReceivedFromLocalSmtp")
						ElseIf aFileLine(iMessageInfo)="00A:" Then
							'Track Recipient Statistics when MessageInfo indicates anonymous (00A:)
							...
							For i = 0 to ubound(aRecipients)
								sRecipient = aRecipients(i)
								Call UpdateRecipientStatistics(aFileLine,sRecipient,"ReceivedFromForeign")							
							Next						
						End If
				...
				Case "STOREDRIVER"
						...
						bAcceptedDomainRecipientExists = FALSE
						For i = 0 to ubound(aRecipients)
							sRecipient = lcase(aRecipients(i))
							strRecipientDomain = lcase(right(sRecipient,len(sRecipient)-instr(1,sRecipient,"@")))
							If oAcceptedDomainDictionary.Exists(strRecipientDomain) Then
								'message has recipient in accepted domain
								bAcceptedDomainRecipientExists = TRUE
							Else
								'Wscript.echo strRecipientDomain & " not found in accepted domain list"
							End If						
						Next						
						If bAcceptedDomainRecipientExists Then iReceiveStoreDriverRecipientInAcceptedDomainCount = iReceiveStoreDriverRecipientInAcceptedDomainCount + 1
						
						'Track Recipient Statistics
						Call UpdateRecipientStatistics(aFileLine,aFileLine(iSender),"ReceivedFromLocalStore")
		...
		Case "SEND"
			...
			'TODO: fix E12 accounting (maybe filter SEND events with ConnectorId "Intra-Organization SMTP Send Connector")
			'Track Recipient Statistics for DeliveredToForeign
			If instr(1,aFileLine(iMessageInfo),"LSRV") Then
				'Not exiting organization yet
			Else
				'Exiting Organization
				Call UpdateRecipientStatistics(aFileLine,aFileLine(iSender),"DeliveredToForeign")
			End If
			
		Case "DUPLICATEEXPAND"
			iDuplicateExpandCount = iDuplicateExpandCount + 1
		...
		Case "EXPAND"
			...
			If oExpandedDLDictionary.Exists(aFileLine(iRelatedRecipAddr)) Then		
				iGroupRecipCount = int(aFileLine(iRecipCount))			
				aExpandDetail = oExpandedDLDictionary.Item(aFileLine(iRelatedRecipAddr))
				If iGroupRecipCount < aExpandDetail(0) then aExpandDetail(0) = iGroupRecipCount
				aExpandDetail(1) = aExpandDetail(1) + 1
				aExpandDetail(2) = aExpandDetail(2) + iExpandLatencySec
				If iGroupRecipCount > aExpandDetail(3) then aExpandDetail(3) = iGroupRecipCount
			...
			Else
				ReDim aExpandDetail(3) '0=MinRecipCount,1=ExpandCount,2=TotalExpandLatency,3=MaxRecipCount
				iUniqueDL = iUniqueDL + 1
				aExpandDetail(0) = int(aFileLine(iRecipCount))	
				aExpandDetail(1) = 1
				aExpandDetail(2) = iExpandLatencySec
				aExpandDetail(3) = int(aFileLine(iRecipCount))	
				'Track latency
			...
			Call UpdateRecipientStatistics(aFileLine,aFileLine(iRelatedRecipAddr),"Expand")
		...
		Case "DELIVER"
			...
			For i = 0 to ubound(aRecipients)
				...
				End If
				Call TrackTopDomain(aFileLine(iEventTime), aRecipients(i), aFileLine(iTotalBytes), objTopDomainRecipientsDictionary)								
				 
				'Update Recipient Statistics for DeliveredToLocalStore
				Call UpdateRecipientStatistics(aFileLine,aRecipients(i),"DeliveredToLocalStore")
			Next
			...
			strOriginalArrivalTime = GetOriginalArrivalTime(aFileLine)				
				...
				If iDeliveryLatency > iDeliveryLatencySLA Then
					If iDeliveryLatency > iDeliveryLatencySLAExceptionLoggingThreshold Then
						...
						If oDeliveryLatencyExceptionDictionary.Exists(sDeliveryLatencyExceptionDictionaryKey) Then
							...
							If aExistDeliveryLatencyExceptionDictionaryItem(6) < iDeliveryLatency Then 
								aExistDeliveryLatencyExceptionDictionaryItem(6) = iDeliveryLatency
								aExistDeliveryLatencyExceptionDictionaryItem(8) = aFileLine(iMessageInfo)
							End If
						...
						Else
							ReDim aAddDeliveryLatencyExceptionDictionaryItem(8) 'strOriginalArrivalTime,iClientName,iTotalBytes,iSender,iRecipCount,iDeliverCount,iMinDeliveryLatency,iMaxDeliveryLatency,strMessageInfo
							...
							aAddDeliveryLatencyExceptionDictionaryItem(8) = aFileLine(iMessageInfo)
		...
		Case "FAIL"
			...
			If sFailReason<>"" Then 
				' \\exsrc\sources\boneyard\exchange.server.2007\sp1_rtm\sources\dev\mailboxtransport\src\storedriver\storageexceptionhandler.cs
				' Diagnostic string for E12: . The following information should help identify the cause
				If instr(1,sFailReason,"STOREDRV") Then
					...
					If instr(1,strFailReason,". The following information should help identify the cause") > 0 Then 
						...
						bStoreDriverFoundDiagStr = TRUE
					End If
					' \\exsrc\SOURCES\ALL\14.01.0218.015\sources\dev\MailboxTransport\src\StoreDriver\StorageExceptionHandler.cs
					' ' Diagnostic string for E14: Failed to process message due to a 
					If instr(1,strFailReason,"Failed to process message due to a ") > 0 Then 
						strFailReason = left(strFailReason,instr(1,strFailReason,"Failed to process message due to a ")-1)
						bStoreDriverFoundDiagStr = TRUE
					End If	
					if bStoreDriverFoundDiagStr Then	
			...
			If sReference<>"" AND oNdrMsgIdDictionary.Exists(sReference) Then
				...
				'Track recipient statistics for DsnFailure
				If aFileLine(iRecipCount) = 1 Then
					aRecipients = Array(aFileLine(iRecipients))
				Else
					aRecipients = split(aFileLine(iRecipients),";")
				End If				
				For i = 0 to ubound(aRecipients)
					Call UpdateRecipientStatistics(aFileLine,aRecipients(i),"DsnFailure")
				Next
		...
		Case "DUPLICATEREDIRECT"
			iDuplicateRedirectCount = iDuplicateRedirectCount + 1
		...
		Case "DEFER"
			...
			Select Case aFileLine(iSource)
				...
				Case "STOREDRIVER"
					sDeferReason=left(aFileLine(iRecipientStatus),instr(1,aFileLine(iRecipientStatus),"; Failed to process message")-1)	
					sDeferReason = replace(sDeferReason,"'","")
					sDeferReason = replace(sDeferReason,"""","")
					if sDeferReason = "" Then sDeferReason = aFileLine(iRecipientStatus)		
					strDeferSource = aFileLine(iSource) & "(" & sDeferReason & ")"
				Case Else
		...
		Case "INITMESSAGECREATED"
			'Moderation event
		
		Case Else
				LogText "EventID Not Known: " & sFileLine
	...
	If sLocalServerName = "" Then
		'LogText vbcrlf & "**Warning: Found Event with no server name" & vbCRLF & sFileLine & vbCRLF
	End If			
...
Sub WriteSummary
	...
	oSummaryResults.WriteLine "Exchange Version: " & sExchangeVersion
	...
	oSummaryResults.WriteLine "Total Duplicate Expand: " & iDuplicateExpandCount
	...
	iSumEvent = iReceiveCount + iSendCount + iDeliverCount + iDuplicateDeliver + iDsnCount + iResolveCount + iTransferCount + iExpandCount + iFailCount + iRedirectCount + iBadmailCount + iSplitError + iDeferCount + iSubmitEventCount + iDuplicateRedirectCount + iDuplicateExpandCount
	...
	For i = 0 to oExpandedDLDictionary.Count - 1
		...
		'0=MinRecipCount,1=ExpandCount,2=TotalExpandLatency,3=MaxRecipCount
		oExpandFileOut.writeline i & "," & a(i) & "," & aExpandDetail(0) & "," & aExpandDetail(3) & "," & aExpandDetail(1) & "," & round(aExpandDetail(2)/aExpandDetail(1),3)
	Next
	...
	If oTopSendersbySubmitDictionary.Count > 0 Then
		...
		aTotalSenderType = Array(0,0,0,0,0,0) '0=Total,1=Light,2=Medium,3=Heavy,4=VeryHeavy,5=Heaviest
		iMaxNumMsgSent = 0
		...
		For i = 0 to oTopSendersbySubmitDictionary.Count-1
			...
			aTotalSenderType(0) = aTotalSenderType(0) + 1
			iNumMsgSent = oTopSendersbySubmitDictionary(a(i))
			If iNumMsgSent > iMaxNumMsgSent Then iMaxNumMsgSent = iNumMsgSent

			If 0 < iNumMsgSent AND iNumMsgSent <= iTopSenderLight Then
				aTotalSenderType(1) = aTotalSenderType(1) + 1
			ElseIf iTopSenderLight < iNumMsgSent AND iNumMsgSent <= iTopSenderMedium Then
				aTotalSenderType(2) = aTotalSenderType(2) + 1
			ElseIf iTopSenderMedium < iNumMsgSent AND iNumMsgSent <= iTopSenderHeavy Then
				aTotalSenderType(3) = aTotalSenderType(3) + 1
			ElseIf iTopSenderHeavy < iNumMsgSent AND iNumMsgSent <= iTopSenderVeryHeavy Then
				aTotalSenderType(4) = aTotalSenderType(4) + 1
			Else 'iTopSenderVeryHeavy < iNumMsgSent
				aTotalSenderType(5) = aTotalSenderType(5) + 1
			End If
			
		Next
		
		if aTotalSenderType(0) > 0 Then
			oTopSenderResultsDistribution.writeline "UserType,SentMsgRange,MailboxCount,Percent"
			oTopSenderResultsDistribution.writeline "Light,1 to " & iTopSenderLight & "," & aTotalSenderType(1) & "," & round(100*aTotalSenderType(1)/aTotalSenderType(0),1)
			oTopSenderResultsDistribution.writeline "Medium," & iTopSenderLight+1 & " to " & iTopSenderMedium & "," & aTotalSenderType(2) & "," & round(100*aTotalSenderType(2)/aTotalSenderType(0),1)
			oTopSenderResultsDistribution.writeline "Heavy," & iTopSenderMedium+1 & " to " & iTopSenderHeavy & "," & aTotalSenderType(3) & "," & round(100*aTotalSenderType(3)/aTotalSenderType(0),1)
			oTopSenderResultsDistribution.writeline "VeryHeavy," & iTopSenderHeavy+1 & " to " & iTopSenderVeryHeavy & "," & aTotalSenderType(4) & "," & round(100*aTotalSenderType(4)/aTotalSenderType(0),1)
			oTopSenderResultsDistribution.writeline "Heaviest," & iTopSenderVeryHeavy+1 & " to " & iMaxNumMsgSent & "," & aTotalSenderType(5) & "," & round(100*aTotalSenderType(5)/aTotalSenderType(0),1)
		End If
	...
	If objTopDomainSendersDictionary.Count > 0 Then
		oSummaryResults.WriteLine vbCrlf & "Message statistics by Senders Domain available in " & sTopDomainSenderResults 
		Set oTopDomainSenderResults = fso.OpenTextFile(sTopDomainSenderResults,ForAppending,1)
		oTopDomainSenderResults.writeline "Domain,Month,Week,Date,Hour,Count,TotalBytes,AvgMsgSize"
		a = objTopDomainSendersDictionary.Keys
		For i = 0 to objTopDomainSendersDictionary.Count-1
			aTopDomainDictionaryEntry = objTopDomainSendersDictionary(a(i))
			For j = 0 to 23
				strHour = j & ":00"
				oTopDomainSenderResults.writeline a(i) & "," & strHour & "," & aTopDomainDictionaryEntry(0,j) & "," & aTopDomainDictionaryEntry(1,j) & "," & round(aTopDomainDictionaryEntry(1,j)/aTopDomainDictionaryEntry(0,j),0)
			Next
			'TODO: write new file with totals (0,24) and (1,24) for tracking day by day
		Next
	End If
	
	If objTopDomainRecipientsDictionary.Count > 0 Then
		oSummaryResults.WriteLine vbCrlf & "Message statistics by Recipients Domain available in " & sTopDomainRecipientsResults 
		Set oTopDomainRecipientsResults = fso.OpenTextFile(sTopDomainRecipientsResults,ForAppending,1)
		oTopDomainRecipientsResults.writeline "Domain,Month,Week,Date,Hour,Count,TotalBytes,AvgMsgSize"
		a = objTopDomainRecipientsDictionary.Keys
		For i = 0 to objTopDomainRecipientsDictionary.Count-1
			aTopDomainDictionaryEntry = objTopDomainRecipientsDictionary(a(i))
			For j = 0 to 23
				strHour = j & ":00"
				oTopDomainRecipientsResults.writeline a(i) & "," & strHour & "," & aTopDomainDictionaryEntry(0,j) & "," & aTopDomainDictionaryEntry(1,j) & "," & round(aTopDomainDictionaryEntry(1,j)/aTopDomainDictionaryEntry(0,j),0)
			Next
			'TODO: write new file with totals (0,24) and (1,24) for tracking day by day
		Next
	End If	
	
	'objRecipientStatisticsDictionary
	If objRecipientStatisticsDictionary.Count > 0 Then
		oSummaryResults.WriteLine vbCrlf & "Recipient statistics available in " & sRecipientStatisticsResults 
		Set oRecipientStatisticsResults = fso.OpenTextFile(sRecipientStatisticsResults,ForAppending,1)
		oRecipientStatisticsResults.writeline "Recipient,TotalMsgCount,TotalMsgBytes,iReceivedFromLocalSmtp,iReceivedFromForeign,iReceivedFromLocalStore,iDeliveredToForeign,iDeliveredToLocalStore,iDsnFailure,iExpanded,iRecipientCount,iUniqueRecipientCount"
		a = objRecipientStatisticsDictionary.Keys
		For i = 0 to objRecipientStatisticsDictionary.Count-1
			aRecipientStatisticsDictionaryEntries = objRecipientStatisticsDictionary(a(i))
			aRecipientStatisticsDictionaryEntry = aRecipientStatisticsDictionaryEntries(0)
			strRecipientStatisticsDictionaryEntry = aRecipientStatisticsDictionaryEntry(0)
			For j = 1 to ubound(aRecipientStatisticsDictionaryEntry)
				strRecipientStatisticsDictionaryEntry = strRecipientStatisticsDictionaryEntry & "," & aRecipientStatisticsDictionaryEntry(j)
			Next
			If aRecipientStatisticsDictionaryEntry(5) > 0 AND aRecipientStatisticsDictionaryEntry(2) = 0 AND aRecipientStatisticsDictionaryEntry(3) = 0 AND aRecipientStatisticsDictionaryEntry(3) = 0 Then
				'Foreign delivery on behalf of foreign sender 
			Else
				aUniqueRecipients = aRecipientStatisticsDictionaryEntries(1)
				iUniqueRecipientCount = ubound(aUniqueRecipients)+1
				'wscript.echo a(i) & " unique count = " & iUniqueRecipientCount
				oRecipientStatisticsResults.writeline a(i) & "," & strRecipientStatisticsDictionaryEntry & "," & iUniqueRecipientCount
				If aRecipientStatisticsDictionaryEntry(9) > iTotalRecipientLimit AND strFilterDate <> "" Then
					'Add sender to objRecipientLimitDictionary tracking senders exceeding iTotalRecipientLimit
					If objRecipientLimitDictionary.Exists(a(i)) Then
						aRecipientLimitDictionaryEntry = objRecipientLimitDictionary.Item(a(i))	
						aRecipientLimitDictionaryEntry(0) = strFilterDate 
						aRecipientLimitDictionaryEntry(1) = aRecipientLimitDictionaryEntry(1) + 1 
						aRecipientLimitDictionaryEntry(2) = round((((aRecipientLimitDictionaryEntry(1) - 1) * aRecipientLimitDictionaryEntry(2)) + aRecipientStatisticsDictionaryEntry(9) / aRecipientLimitDictionaryEntry(1) ),0)
						objRecipientLimitDictionary.Item(a(i)) = aRecipientLimitDictionaryEntry				
					Else
						objRecipientLimitDictionary.Add a(i), Array(strFilterDate,1,aRecipientStatisticsDictionaryEntry(9))
					End If
				End If
			End If
		Next
	...
	If objRecipientLimitDictionary.Count > 0 Then
		If fso.FileExists(sRecipientLimitDictionaryFile) then fso.DeleteFile(sRecipientLimitDictionaryFile)
		Set oRecipientLimitDictionaryFile = fso.OpenTextFile(sRecipientLimitDictionaryFile,ForAppending,1)
		a = objRecipientLimitDictionary.Keys
		oRecipientLimitDictionaryFile.writeline "Recipient,LastDateExceeded,NumDaysExceeded,AverageRecipientsPerDay"
		For i = 0 to objRecipientLimitDictionary.Count-1
			aRecipientLimitDictionaryEntry = objRecipientLimitDictionary.Item(a(i))	
			oRecipientLimitDictionaryFile.writeline a(i) & "," & aRecipientLimitDictionaryEntry(0) & "," & aRecipientLimitDictionaryEntry(1) & "," & aRecipientLimitDictionaryEntry(2)
		Next
		err.Clear
		LogText now & vbtab & "INFO" & vbtab & "Done saving RecipientLimitDictionaryFile containing " & objRecipientLimitDictionary.Count & " Entries"
		oRecipientLimitDictionaryFile.close
		Set oRecipientLimitDictionaryFile = Nothing
		Set oFile = fso.GetFile(sRecipientLimitDictionaryFile)
		oFile.copy(sResultPath & "\" & oFile.name)
		If err.number <> 0 Then LogText now & vbtab & "ERROR" & vbtab & "Could not copy " & oFile.name & " to " & sResultPath & " (" & err.Description & ")"	
	End If
	...
	If oTopRecipientsDictionary.Count > 0 Then
		...
		aTotalRecipientType = Array(0,0,0,0,0,0) '0=Total,1=Light,2=Medium,3=Heavy,4=VeryHeavy,5=Heaviest
		iMaxNumMsgReceived = 0
		For i = 0 to oTopRecipientsDictionary.Count-1
			...
			aTotalRecipientType(0) = aTotalRecipientType(0) + 1
			iNumMsgReceived = oTopRecipientsDictionary(a(i))
			If iNumMsgReceived > iMaxNumMsgReceived Then iMaxNumMsgReceived = iNumMsgReceived

			If 0 < iNumMsgReceived AND iNumMsgReceived <= iTopRecipientLight Then
				aTotalRecipientType(1) = aTotalRecipientType(1) + 1
			ElseIf iTopRecipientLight < iNumMsgReceived AND iNumMsgReceived <= iTopRecipientMedium Then
				aTotalRecipientType(2) = aTotalRecipientType(2) + 1
			ElseIf iTopRecipientMedium < iNumMsgReceived AND iNumMsgReceived <= iTopRecipientHeavy Then
				aTotalRecipientType(3) = aTotalRecipientType(3) + 1
			ElseIf iTopRecipientHeavy < iNumMsgReceived AND iNumMsgReceived <= iTopRecipientVeryHeavy Then
				aTotalRecipientType(4) = aTotalRecipientType(4) + 1
			Else 'iTopRecipientVeryHeavy < iNumMsgReceived
				aTotalRecipientType(5) = aTotalRecipientType(5) + 1
			End If			
			
		Next
		
		if aTotalRecipientType(0) > 0 Then
			oTopRecipientResultsDistribution.writeline "UserType,ReceivedMsgRange,MailboxCount,Percent"
			oTopRecipientResultsDistribution.writeline "Light,1 to " & iTopRecipientLight & "," & aTotalRecipientType(1) & "," & round(100*aTotalRecipientType(1)/aTotalRecipientType(0),1)
			oTopRecipientResultsDistribution.writeline "Medium," & iTopRecipientLight+1 & " to " & iTopRecipientMedium & "," & aTotalRecipientType(2) & "," & round(100*aTotalRecipientType(2)/aTotalRecipientType(0),1)
			oTopRecipientResultsDistribution.writeline "Heavy," & iTopRecipientMedium+1 & " to " & iTopRecipientHeavy & "," & aTotalRecipientType(3) & "," & round(100*aTotalRecipientType(3)/aTotalRecipientType(0),1)
			oTopRecipientResultsDistribution.writeline "VeryHeavy," & iTopRecipientHeavy+1 & " to " & iTopRecipientVeryHeavy & "," & aTotalRecipientType(4) & "," & round(100*aTotalRecipientType(4)/aTotalRecipientType(0),1)
			oTopRecipientResultsDistribution.writeline "Heaviest," & iTopRecipientVeryHeavy+1 & " to " & iMaxNumMsgReceived & "," & aTotalRecipientType(5) & "," & round(100*aTotalRecipientType(5)/aTotalRecipientType(0),1)
		End If
	...
	If oLatencyTrackerDictionary.Count > 0 Then
		...
		oLatencyTrackerResults.WriteLine "ServerFqdn,Component,AvgLatency,%Lt1Sec,%Lt" & iIndividualComponentLatencySLA & "Sec,%Lt" & iStageComponentLatencySLA & "Sec,%Lt" & iServerLatencySLA & "Sec,%Lt" & iDeliveryLatencySLA & "Sec,TotalInvocationsGtThreshold,MaxInvocationCount,MaxLatency"
		For i = 0 to oLatencyTrackerDictionary.Count-1
			...
			End If
			Dim aE2ELatencyTrackerServers()
			If instr(a(i),"E2ELATENCY") > 0 Then
				iE2ELatencyTrackerServerCount = ubound(aE2ELatencyTrackerServers) + 1
				Redim Preserve aE2ELatencyTrackerServers(iE2ELatencyTrackerServerCount)				
				aE2ELatencyTrackerServers(iE2ELatencyTrackerServerCount) = Array(a(i),oLatencyTrackerDictionary.Item(a(i)))
				'Wscript.echo vbCRLF & "Adding aE2ELatencyTrackerServers entry for " & a(i)
			End If			
		Next
		For i = 0 to oLatencyTrackerDictionary.Count-1
			...
			For x = 0 to ubound(aLatencyTrackerServers)		
				...
				End If
			Next
			For x = 0 to ubound(aE2ELatencyTrackerServers)		
				aE2ELatencyTrackerServersEntry = aE2ELatencyTrackerServers(x)
				If instr(1,aE2ELatencyTrackerServersEntry(0),aLatencyTrackerDictionaryKey(0)) Then
					aLatencyTrackerDictionaryE2EEntry = aE2ELatencyTrackerServersEntry(1)
					'Wscript.echo "Found total (" & aLatencyTrackerDictionaryE2EEntry(0) & ") for " & a(i)
				Else
					'Wscript.echo "Not Found total for " & a(i)
				End If
			Next
			...
			If instr(1,a(i),"TOTAL") Then
				...
				iDeliverySLAPercentile = round(100*(aLatencyTrackerDictionaryEntry(5))/aLatencyTrackerDictionaryTotalEntry(0),2)
			ElseIf instr(1,a(i),"E2ELATENCY") Then
				iAvgComponentLatency = round(aLatencyTrackerDictionaryEntry(1)/aLatencyTrackerDictionaryE2EEntry(0),1)
				iComponentPercentileLt1Sec = ""
				iIndividualComponentSLAPercentile = round(100*(aLatencyTrackerDictionaryEntry(2))/aLatencyTrackerDictionaryE2EEntry(0),2)
				iStageComponentSLAPercentile = round(100*(aLatencyTrackerDictionaryEntry(3))/aLatencyTrackerDictionaryE2EEntry(0),2)
				iServerSLAPercentile = round(100*(aLatencyTrackerDictionaryEntry(4))/aLatencyTrackerDictionaryE2EEntry(0),2)
				iDeliverySLAPercentile = round(100*(aLatencyTrackerDictionaryEntry(5))/aLatencyTrackerDictionaryE2EEntry(0),2)
			Else
				...
				iDeliverySLAPercentile = round(100*(aLatencyTrackerDictionaryTotalEntry(0)-aLatencyTrackerDictionaryEntry(0)+aLatencyTrackerDictionaryEntry(5))/aLatencyTrackerDictionaryTotalEntry(0),2)
			End If
			If a(i) <> "," Then
				oLatencyTrackerResults.WriteLine a(i) & "," & iAvgComponentLatency & "," & iComponentPercentileLt1Sec & "%," & iIndividualComponentSLAPercentile & "%," & iStageComponentSLAPercentile & "%," & iServerSLAPercentile & "%," & iDeliverySLAPercentile & "%," & aLatencyTrackerDictionaryEntry(6) & "," & aLatencyTrackerDictionaryEntry(7) & "," & aLatencyTrackerDictionaryEntry(8)
				If bWriteLatencyDailySummary AND instr(1,a(i),"E2ELATENCY") Then
					sLatencyTrackerDailyDictionaryKey = GetSortableDate(strFilterDate) & "," & a(i)
					'0=Date,1=ServerFqdn,2=Component,3=TotalMsgCount,4=%Lt" & iStageComponentLatencySLA & "Sec,5=%Lt" & iServerLatencySLA & "Sec,6=%Lt" & iDeliveryLatencySLA & "Sec,7=MaxLatency"
					aLatencyTrackerDailyDictionaryEntry = Array(aLatencyTrackerDictionaryEntry(6),iStageComponentSLAPercentile,iServerSLAPercentile,iDeliverySLAPercentile,aLatencyTrackerDictionaryEntry(8))
					If oLatencyTrackerDailyDictionary.Exists(sLatencyTrackerDailyDictionaryKey) Then
						oLatencyTrackerDailyDictionary.Item(sLatencyTrackerDailyDictionaryKey) = aLatencyTrackerDailyDictionaryEntry
					Else
						oLatencyTrackerDailyDictionary.Add sLatencyTrackerDailyDictionaryKey,aLatencyTrackerDailyDictionaryEntry
					End If
				End If
			End If
		Next
		
		If bWriteLatencyDailySummary Then
			'Write oLatencyTrackerDailyDictionary to sLatencyTrackerDailyResults
			If fso.FileExists(sLatencyTrackerDailyResults) Then fso.DeleteFile(sLatencyTrackerDailyResults)
			Set oLatencyTrackerDailyResults = fso.OpenTextFile(sLatencyTrackerDailyResults,ForAppending,1)
			oLatencyTrackerDailyResults.writeline "Date,ServerFqdn,Component,TotalMsgCount,PercentLt" & iStageComponentLatencySLA & "Sec,PercentLt" & iServerLatencySLA & "Sec,PercentLt" & iDeliveryLatencySLA & "Sec,MaxLatency"
			a = oLatencyTrackerDailyDictionary.Keys
			For i = 0 to oLatencyTrackerDailyDictionary.Count-1
				aLatencyTrackerDailyDictionaryEntry = oLatencyTrackerDailyDictionary.Item(a(i))
				oLatencyTrackerDailyResults.writeline a(i) & "," & aLatencyTrackerDailyDictionaryEntry(0) & "," & aLatencyTrackerDailyDictionaryEntry(1) & "," & aLatencyTrackerDailyDictionaryEntry(2) & "," & aLatencyTrackerDailyDictionaryEntry(3) & "," & aLatencyTrackerDailyDictionaryEntry(4)
			Next
			oLatencyTrackerDailyResults.close
			Set oLatencyTrackerDailyResults = Nothing
			err.Clear
			Set oFile = fso.GetFile(sLatencyTrackerDailyResults)
			oFile.copy(sResultPath & "\" & oFile.name)
			If err.number <> 0 Then LogText now & vbtab & "ERROR" & vbtab & "Could not copy " & oFile.name & " to " & sResultPath & " (" & err.Description & ")"
...
Sub ArchiveFiles
		...
		Do While oExec.Status = 0
			...
			If iArchiveRunTimeSec Mod 30 = 0 Then 
				If DateDiff("s",strStartArchive,now)>=600 Then 
					...
					Exit Sub
				Else
					LogText "INFO: attempt to archive files has taken " & iArchiveRunTimeSec & " sec (will timeout at 600 sec)"								
				End If
			End if			
	...
	Else
		...
		sDateFolder = year(now) & month(now) & day(now) & hour(now) & minute(now)
		sUncompressedArchivePath = sArchivePath & replace(GetSortableDate(strFilterDate),"/","") & "_" & sDateFolder	
		If bArchiveUncompressed Then	
			LogText "Could not find " & strExePath & ", creating uncompressed archive at " & sUncompressedArchivePath
			err.Clear
			Set oUncompressedArchiveFolder = fso.CreateFolder(sUncompressedArchivePath)
			If err.number = 0 Then

				Set objLogFolder = fso.GetFolder(sResultPath)
				Set oFileCol = objLogFolder.Files
				For each oFile in oFileCol	
					'wscript.echo "attempting to copy " & oFile.name & " to " & sUncompressedArchivePath & "\" & oFile.name
					oFile.copy(sUncompressedArchivePath & "\" & oFile.name)
					if err.number <> 0 Then wscript.echo "Error 0x" & hex(err.number) & ": " & err.Description
				Next
			
			Else
			
				LogText "Error: unable to create uncompressed archive at " & sUncompressedArchivePath 

			End If
		Else
			LogText "Could not find " & strExePath & " and will not create uncompressed archive"
...
Sub CloseFiles
	...
	oDuplicateDeliveryResults.close
	...
	If oServerLatencyDictionary.Exists(strUniqueMessageId) Then
		...
		If instr(1,"DELIVER,SEND,FAIL,POISONMESSAGE,DUPLICATEDELIVER",ucase(aFileLine(iEventId))) Then
			...
			If instr(1,aFileLine(iMessageInfo),";") Then
				...
				'Parse MessageInfo field to calculate end-to-end delivery (END2END component)
				strOriginalArrivalTime = GetOriginalArrivalTime(aFileLine)				
				strDeliverTime = GetDateTime(aFileLine(iEventTime))
				If strOriginalArrivalTime<>"" AND strDeliverTime<>"" Then
					iEnd2EndLatency = DateDiff("s",strOriginalArrivalTime,strDeliverTime)
				Else
					iEnd2EndLatency = 0
				End If					
				...
				aLatencyTrackerServerInfo = split(aLatencyTrackerEntry(0),"=")
				'strLatencyTrackerFqdn = right(aLatencyTrackerEntry(0),len(aLatencyTrackerEntry(0)) - instr(aLatencyTrackerEntry(0),"="))
				strLatencyTrackerType = aLatencyTrackerServerInfo(0)
				strLatencyTrackerFqdn = aLatencyTrackerServerInfo(1)
				If strLatencyTrackerType <> "LSRV" Then 
					sModifiedLatencyTrackerEntry = "E2ELATENCY=" & iEnd2EndLatency & "|" & aLatencyTrackerEntry(1)
				Else
					sModifiedLatencyTrackerEntry = aLatencyTrackerEntry(1)
				End If
				aLatencyTrackerComponentEntry = split(sModifiedLatencyTrackerEntry,"|")
				For x = 0 to ubound(aLatencyTrackerComponentEntry)					
					...
					If strLatencyTrackerFqdn <> "" AND aLatencyTrackerComponentEntryDetails(0) <> "" Then
						If oLatencyTrackerDictionary.Exists(sLatencyTrackerDictionaryKey) Then
							...
							End If
							'Create Delivery Latency SLA Percentile
							If aLatencyTrackerComponentDictionaryEntry(2) <= iDeliveryLatencySLA Then
							...
							If Not(bNewComponent) Then
								...
								If aLatencyTrackerComponentDictionaryEntry(1) <= iServerLatencySLA Then
									aLatencyTrackerDictionaryEntry(4) = aLatencyTrackerDictionaryEntry(4) - 1
								End If							
								If aLatencyTrackerComponentDictionaryEntry(1) <= iDeliveryLatencySLA Then
									aLatencyTrackerDictionaryEntry(5) = aLatencyTrackerDictionaryEntry(5) - 1
								End If							
							End If
							aLatencyTrackerDictionaryEntry(6) = aLatencyTrackerDictionaryEntry(6) + 1
							If aLatencyTrackerComponentDictionaryEntry(0) > aLatencyTrackerDictionaryEntry(7) Then
								aLatencyTrackerDictionaryEntry(7) = aLatencyTrackerComponentDictionaryEntry(0)
							End If
							If aLatencyTrackerComponentDictionaryEntry(2) > aLatencyTrackerDictionaryEntry(8) Then
								aLatencyTrackerDictionaryEntry(8) = aLatencyTrackerComponentDictionaryEntry(2)
							End If							
							oLatencyTrackerDictionary.Item(sLatencyTrackerDictionaryKey) = aLatencyTrackerDictionaryEntry
						Else
							Dim aLatencyTrackerDictionaryEntry(8) '0=MessageCountGt1Sec,1=TotalLatency,2=IndividualComponentSLA,3=StageComponentSLA,4=ServerSLA,5=DeliverySLA,6=TotalInvocationsGt1Sec,7=MaxInvocationCount,8=MaxLatency
							...
							End If
							'Create Delivery Latency SLA Percentile
							If int(aLatencyTrackerComponentEntryDetails(1)) <= iDeliveryLatencySLA Then
								aLatencyTrackerDictionaryEntry(5) = 1
							Else
								aLatencyTrackerDictionaryEntry(5) = 0
							End If							
							aLatencyTrackerDictionaryEntry(6) = 1
							aLatencyTrackerDictionaryEntry(7) = 1
							aLatencyTrackerDictionaryEntry(8) = int(aLatencyTrackerComponentEntryDetails(1))
...
Sub TrackLastDelivery (byRef aFinalLatencyTrackingEntry, byRef sFinalEventId, byRef sMessageInfo)
	...
	ElseIf instr(1,aFinalLatencyTrackingEntry(3),"RESUBMIT")>0 Then
		...
		If strOriginalArrivalTime = "" Then			
			'Wscript.echo "Error, strOriginalArrivalTime is blank on RESUBMIT Event"
		Else
...
Function GetOriginalArrivalTime(byRef aFileLine)
	On Error Resume Next
	
	sMessageInfo = aFileLine(iMessageInfo)
	
	Select Case aFileLine(iEventId)
	
		Case "DELIVER"
			'parse MessageInfo field
			'2008-05-16T23:48:02.355Z
		...
		Case "SEND"
			'parse SourceContext field
			'08CA803CA7B5436D;2008-05-16T23:48:02.439Z;0
			If instr(1,sMessageInfo,";") Then
				aMessageInfo = split(sMessageInfo,";")
				strOriginalArrivalTime = GetDateTime(aMessageInfo(1))
			Else
				strOriginalArrivalTime = GetDateTime(sMessageInfo)
			End If
...
Sub DeleteLogFiles(sLogDirectory)

	Set objLogFolder = fso.GetFolder(sLogDirectory)
	Set oFileCol = objLogFolder.Files
	For each oFile in oFileCol
		If instr(1,oFile.name,"AdMoveComputer-") AND instr(1,oFile.name,".log") Then
			iLogAge = DateDiff("y",oFile.DateLastModified,date)
			If iLogAge > iMaxLogAge Then
				LogMessage bDebug, "EVENT", "Deleting file " & oFile.path & " (file was " & iLogAge & " days old)"
				fso.DeleteFile(oFile.path)
			End If
		End If
	Next
	Set oLogFileCol = Nothing
	Set objLogFolder = Nothing

End Sub

Function GetAcceptedDomains (ByVal strFilePath)
	On Error Resume Next
	
	GetAcceptedDomains = FALSE

	bNameFound = FALSE
	bTypeFound = FALSE
	bDefaultFound = FALSE	
	
	Set oFile = fso.OpenTextFile(strFilePath,ForReading)
	Do While oFile.AtEndOfStream <> True	
		sFileLine = oFile.Readline
		If instr(1,sFileLine,":") > 0 Then
			aFileLine = split(sFileLine,":")
			Select Case trim(aFileLine(0))
				
				Case "DomainName"
					If bNameFound Then
						Wscript.echo "Incomplete settings for " & sFileLine & ", exit"
						Wscript.quit
					Else 
						sDomainName = lcase(trim(aFileLine(1)))
						bNameFound = TRUE
					End If
				...
				Case "DomainType"
					If bTypeFound Then
						...
						Wscript.quit
					Else 				
						sDomainType = trim(aFileLine(1))
						bTypeFound = TRUE
					End If
				
				Case "Default"
					If bDefaultFound Then
						Wscript.echo "Incomplete settings for " & sFileLine & ", exit"
						Wscript.quit
					Else
						sDefault = trim(aFileLine(1))
						bDefaultFound = TRUE
					End If
			...
			If bNameFound AND bTypeFound AND bDefaultFound Then
			
				If oAcceptedDomainDictionary.Exists(sDomainName) Then
					Wscript.echo "Error: Duplicate found for " & sDomainName
				Else
					'Wscript.echo "Loading dictionary with " & sDomainName 
					oAcceptedDomainDictionary.Add sDomainName, sDomainType & "," & sDefault
				End If
	...
	GetAcceptedDomains = TRUE
	
End Function

Function TrackTopDomain(byVal strEventTime, byVal strAddress, byVal iMsgBytes, byRef objTopDomainDictionary)
	
	If instr(1,lcase(strAddress),"systemmailbox")>0 Then
		'skip SystemMailbox mailboxes
		exit Function
	Else
		'extract RHS of address
		sRoutingAddress = lcase(right(strAddress,len(strAddress)-instr(1,strAddress,"@")))
	End If
	
	sCurrentEventTime=GetDateTime(strEventTime)	
	iHour = hour(sCurrentEventTime)
	
	sTopDomainDictionaryKey = sRoutingAddress & "," & month(sCurrentEventTime) & "," & datePart("ww",sCurrentEventTime) & "," & DateValue(sCurrentEventTime)
	
	If objTopDomainDictionary.Exists(sTopDomainDictionaryKey) Then
		aTopDomainDictionaryEntry = objTopDomainDictionary.Item(sTopDomainDictionaryKey)
		aTopDomainDictionaryEntry(0,iHour) = aTopDomainDictionaryEntry(0,iHour) + 1
		aTopDomainDictionaryEntry(1,iHour) = aTopDomainDictionaryEntry(1,iHour) + int(iMsgBytes)
		aTopDomainDictionaryEntry(0,24) = aTopDomainDictionaryEntry(0,24) + 1
		aTopDomainDictionaryEntry(1,24) = aTopDomainDictionaryEntry(1,24) + int(iMsgBytes)
		objTopDomainDictionary.Item(sTopDomainDictionaryKey) = aTopDomainDictionaryEntry
	Else
		Dim aTopDomainDictionaryEntry(1,24)
		For i = 0 to 1
			For j = 0 to 24
				aTopDomainDictionaryEntry(i,j) = 0
			Next
		Next
		aTopDomainDictionaryEntry(0,iHour) = 1
		aTopDomainDictionaryEntry(1,iHour) = int(iMsgBytes)
		aTopDomainDictionaryEntry(0,24) = 1
		aTopDomainDictionaryEntry(1,24) = int(iMsgBytes)
		objTopDomainDictionary.Add sTopDomainDictionaryKey, aTopDomainDictionaryEntry
	End If
 
End Function

Function UpdateRecipientStatistics(byRef aFileLine, byVal strRecipient, byVal strStatistic)
	
	On Error Resume Next
	
	err.Clear
	strRecipientStatisticsDictionaryKey = lcase(strRecipient)
	iTotalSendReceiveMsgCount = 0
	iReceivedFromLocalSmtp = 0
	iReceivedFromForeign = 0
	iReceivedFromLocalStore = 0
	iDeliveredToForeign = 0
	iDeliveredToLocalStore = 0
	iDsnFailure = 0
	iExpandCount = 0
	iTotalSendReceiveBytes = 0
	iTotalRecipientCount = 0
	
	Select Case strStatistic
		Case "ReceivedFromLocalSmtp"
			iReceivedFromLocalSmtp = 1
			iTotalSendReceiveMsgCount = 1
			iTotalSendReceiveBytes = int(aFileLine(iTotalBytes))
			iTotalRecipientCount = int(aFileLine(iRecipCount))
		Case "ReceivedFromForeign"
			iReceivedFromForeign = 1
		Case "ReceivedFromLocalStore"
			iReceivedFromLocalStore = 1
			iTotalSendReceiveMsgCount = 1
			iTotalSendReceiveBytes = int(aFileLine(iTotalBytes))
			iTotalRecipientCount = int(aFileLine(iRecipCount))
		Case "DeliveredToForeign"
			iDeliveredToForeign = 1
		Case "DeliveredToLocalStore"
			iDeliveredToLocalStore = 1
			iTotalSendReceiveMsgCount = 1
			iTotalSendReceiveBytes = int(aFileLine(iTotalBytes))
		Case "DsnFailure"	
			iDsnFailure = 1
	...
	If objRecipientStatisticsDictionary.Exists(strRecipientStatisticsDictionaryKey) Then
		aRecipientStatisticsDictionaryEntries = objRecipientStatisticsDictionary.Item(strRecipientStatisticsDictionaryKey)
		aRecipientStatisticsDictionaryEntry = aRecipientStatisticsDictionaryEntries(0)
		aRecipientStatisticsDictionaryEntry(0) = aRecipientStatisticsDictionaryEntry(0) + iTotalSendReceiveMsgCount
		aRecipientStatisticsDictionaryEntry(1) = aRecipientStatisticsDictionaryEntry(1) + iTotalSendReceiveBytes
		aRecipientStatisticsDictionaryEntry(2) = aRecipientStatisticsDictionaryEntry(2) + iReceivedFromLocalSmtp
		aRecipientStatisticsDictionaryEntry(3) = aRecipientStatisticsDictionaryEntry(3) + iReceivedFromForeign
		aRecipientStatisticsDictionaryEntry(4) = aRecipientStatisticsDictionaryEntry(4) + iReceivedFromLocalStore
		aRecipientStatisticsDictionaryEntry(5) = aRecipientStatisticsDictionaryEntry(5) + iDeliveredToForeign
		aRecipientStatisticsDictionaryEntry(6) = aRecipientStatisticsDictionaryEntry(6) + iDeliveredToLocalStore
		aRecipientStatisticsDictionaryEntry(7) = aRecipientStatisticsDictionaryEntry(7) + iDsnFailure
		aRecipientStatisticsDictionaryEntry(8) = aRecipientStatisticsDictionaryEntry(8) + iExpandCount
		aRecipientStatisticsDictionaryEntry(9) = aRecipientStatisticsDictionaryEntry(9) + iTotalRecipientCount
		if (iReceivedFromLocalSmtp = 1 or iReceivedFromLocalStore = 1) Then
			aUniqueRecipients = UpdateUniqueRecipientDictionary(aRecipientStatisticsDictionaryEntries(1), aFileLine)
			'wscript.echo "*** Existing entry: " & aFileLine(iSender) & " recipient count: " & ubound(aUniqueRecipients)+1
		Else
			aUniqueRecipients = aRecipientStatisticsDictionaryEntries(1)
		End If
		objRecipientStatisticsDictionary.Item(strRecipientStatisticsDictionaryKey) = Array(aRecipientStatisticsDictionaryEntry,aUniqueRecipients)
		'Wscript.echo "Updated objRecipientStatisticsDictionary " & strRecipientStatisticsDictionaryKey & " for event " & strStatistic
	Else
		ReDim aRecipientStatisticsDictionaryEntry(9)
		aRecipientStatisticsDictionaryEntry(0) = iTotalSendReceiveMsgCount
		aRecipientStatisticsDictionaryEntry(1) = iTotalSendReceiveBytes
		aRecipientStatisticsDictionaryEntry(2) = iReceivedFromLocalSmtp
		aRecipientStatisticsDictionaryEntry(3) = iReceivedFromForeign
		aRecipientStatisticsDictionaryEntry(4) = iReceivedFromLocalStore
		aRecipientStatisticsDictionaryEntry(5) = iDeliveredToForeign
		aRecipientStatisticsDictionaryEntry(6) = iDeliveredToLocalStore
		aRecipientStatisticsDictionaryEntry(7) = iDsnFailure	
		aRecipientStatisticsDictionaryEntry(8) = iExpandCount
		aRecipientStatisticsDictionaryEntry(9) = iTotalRecipientCount						
		if (iReceivedFromLocalSmtp = 1 or iReceivedFromLocalStore = 1) Then
			aUniqueRecipients = UpdateUniqueRecipientDictionary(Array(), aFileLine)
			 'wscript.echo "*** Adding entry: " & aFileLine(iSender) & " recipient count: " & objUniqueRecipientDictionary.Count
		Else
			aUniqueRecipients = Array()
		End If
		'wscript.echo "Dictionary has count: " & objUniqueRecipientDictionary.count
		objRecipientStatisticsDictionary.Add strRecipientStatisticsDictionaryKey,Array(aRecipientStatisticsDictionaryEntry,aUniqueRecipients)
	End If
End Function

Function UpdateUniqueRecipientDictionary (ByVal aRecipientList, byVal aFileLine)
	
	On Error Resume Next
	Set objUniqueRecipientDictionary = CreateObject("Scripting.Dictionary")
	If bComputeUniqueRecipientStats Then		
			
		For i = 0 to ubound(aRecipientList)
			objUniqueRecipientDictionary.add aRecipientList(i),0
		Next
				
		iStartCount = objUniqueRecipientDictionary.Count	
		...
		For i = 0 to ubound(aRecipients)
			strP2Recipient = lcase(aRecipients(i))
			If objUniqueRecipientDictionary.Exists(strP2Recipient) Then
				'wscript.echo ">> duplicate recipient (" & strP2Recipient & "} for " & aFileLine(iSender)
			Else
				'wscript.echo ">> adding recipient " & strP2Recipient & " for " & aFileLine(iSender)
				objUniqueRecipientDictionary.Add strP2Recipient,1
			End If
	...
	UpdateUniqueRecipientDictionary = objUniqueRecipientDictionary.Keys
	
End Function


Function LoadLatencyTrackerDailySummary (byRef oLatencyTrackerDailyDictionary)
	
	On Error Resume Next
	LoadLatencyTrackerDailySummary  = FALSE
	
	If fso.FileExists(sLatencyTrackerDailyResults) Then
		Set oLatencyTrackerDailyResults = fso.OpenTextFile(sLatencyTrackerDailyResults,ForReading)
		If err.number <> 0 Then LogText now & vbtab & "ERROR" & vbtab & "Unable to open " & sLatencyTrackerDailyResults & " (" & err.description & ")"
		Do While oLatencyTrackerDailyResults.AtEndOfStream <> True
			sLatencyTrackerDailyEntry =  oLatencyTrackerDailyResults.readline
			If instr(1,sLatencyTrackerDailyEntry,",") Then
				'0=Date,1=ServerFqdn,2=Component,3=TotalMsgCount,4=%Lt" & iStageComponentLatencySLA & "Sec,5=%Lt" & iServerLatencySLA & "Sec,6=%Lt" & iDeliveryLatencySLA & "Sec,7=MaxLatency"
				aLatencyTrackerDailyEntry = split(sLatencyTrackerDailyEntry,",")
				If aLatencyTrackerDailyEntry(0) <> "Date" Then
					'wscript.echo sLatencyTrackerDailyEntry
					sLatencyTrackerDailyDictionaryKey = aLatencyTrackerDailyEntry(0) & "," & aLatencyTrackerDailyEntry(1) & "," & aLatencyTrackerDailyEntry(2)
					If oLatencyTrackerDailyDictionary.Exists(sLatencyTrackerDailyDictionaryKey) Then
						LogText now & vbtab & "ERROR" & vbtab & "Duplicate LatencyTracker Daily entry found (" & sLatencyTrackerDailyDictionaryKey & ")"
					Else
						oLatencyTrackerDailyDictionary.Add sLatencyTrackerDailyDictionaryKey,Array(aLatencyTrackerDailyEntry(3),aLatencyTrackerDailyEntry(4),aLatencyTrackerDailyEntry(5),aLatencyTrackerDailyEntry(6),aLatencyTrackerDailyEntry(7))
					End If
				End If
			End If
		Loop
	...
	LoadLatencyTrackerDailySummary = TRUE

End Function

Function GetSortableDate (byVal sDate)
	sYear = year(sDate)
	sMonth = month(sDate)
	If len(sMonth) < 2 Then sMonth = "0" & sMonth
	sDay = day(sDate)
	If len(sDay) < 2 Then sDay = "0" & sDay
	GetSortableDate = sMonth & "/" & sDay & "/" & sYear	
End Function

Function LoadRecipientLimitDictionary (byRef objRecipientLimitDictionary)
	On Error Resume Next
	LoadRecipientLimitDictionary = FALSE
	Set oRecipientLimitDictionaryFile = fso.OpenTextFile(sRecipientLimitDictionaryFile,ForReading)

	If err.number <> 0 Then
		LogText now & vbtab & "ERROR" & vbtab & "Unable to open " & sRecipientLimitDictionaryFile & " (" & err.description & ")"
		Exit Function
	Else
		Do While oRecipientLimitDictionaryFile.AtEndOfStream <> True
			sRecipientLimitDictionaryFileEntry =  oRecipientLimitDictionaryFile.readline
			If instr(1,sRecipientLimitDictionaryFileEntry,",") Then
				aRecipientLimitDictionaryFileEntry = split(sRecipientLimitDictionaryFileEntry,",")
				If aRecipientLimitDictionaryFileEntry(0) <> "Recipient" Then
					sRecipientLimitDictionaryFileEntryKey = aRecipientLimitDictionaryFileEntry(0)
					If objRecipientLimitDictionary.Exists(sRecipientLimitDictionaryFileEntryKey) Then
						LogText now & vbtab & "ERROR" & vbtab & "Duplicate RecipientLimitDictionary entry found (" & sRecipientLimitDictionaryFileEntryKey & ")"
					Else
						'Recipient,LastDateExceeded,NumDaysExceeded,AverageRecipientsPerDay
						objRecipientLimitDictionary.Add sRecipientLimitDictionaryFileEntryKey,Array(aRecipientLimitDictionaryFileEntry(1),int(aRecipientLimitDictionaryFileEntry(2)),int(aRecipientLimitDictionaryFileEntry(3)))
						If Err.number <> 0 Then 
							LogText now & vbtab & "ERROR" & vbtab & "RecipientLimitDictionary entry not added (" & sRecipientLimitDictionaryFileEntryKey & ")"
							err.Clear
						End If
					End If
				End If
		...
		Set oRecipientLimitDictionaryFile = Nothing
	End If
	LoadRecipientLimitDictionary = TRUE
End Function