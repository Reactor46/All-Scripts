...
for count = 1 to intRes
	Select Case objIadsTools.MetaDataName(count)
		Case "nTSecurityDescriptor" wscript.echo "nTSecurityDescriptor LastModified : " & objIadsTools.MetaDataLastWriteTime(count)_ 
									& " Time Diff : " & DateDiff("n",reformatdate(objIadsTools.MetaDataLastWriteTime(count)),Now()) 
									If  DateDiff("n",reformatdate(objIadsTools.MetaDataLastWriteTime(count)),Now())  < 15 Then fsnap = 1
		Case "msExchMailboxSecurityDescriptor" wscript.echo "msExchMailboxSecurityDescriptor Last Modified : " _
									& objIadsTools.MetaDataLastWriteTime(count) & " Time Diff : " &  DateDiff("n",reformatdate(objIadsTools.MetaDataLastWriteTime(count)),Now()) 
									If DateDiff("n",reformatdate(objIadsTools.MetaDataLastWriteTime(count)),Now())  < 15 Then fsnap = 1
	End Select
Next
...
If rrRightReport = 1 Then
	...
	objEmail.Configuration.Fields.Item("http://schemas.microsoft.com/cdo/configuration/smtpserver") = EmailServername
...
rdReturnDate = formatdatetime(dateserial(tmArry(0),dtarry(0),dtarry(1)) & " "  & tmArry(1) & " " & tmArry(2))
