...
wfile.writeline("Username,StorageQuoata(MB),OverLimitQuota(MB),HardLimitQuota(MB),Mailbox Size(MB),Percent Used")
...
while not rs.eof	
	...
	strQuery = "<LDAP://"  & strDefaultNamingContext & ">;" & GALQueryFilter & ";distinguishedName,msExchMailboxGuid,displayname,mDBStorageQuota,mDBOverQuotaLimit,mDBOverHardQuotaLimit,mDBUseDefaults,msExchHomeServerName,legacyExchangeDN,homemdb;subtree"
	...
	Do While Not objgrandchild.EOF
		...
		if strUsedefaults = false then
			if strusrQuota <> 0 then
				wfile.writeline  strUsername & "," & replace(formatnumber(strusrQuota/1024,2),",","")  & "," & replace(formatnumber(strusrQuotaLimit/1024,2),",","") & "," & replace(formatnumber(strusrHardQuotaLimit/1024,2),",","") & "," & replace(formatnumber(mbsize/1024,2),",","") & "," & formatnumber((mbsize/strusrQuota)*100,2) & "%"
			else
				wfile.writeline  strUsername & "," & replace(formatnumber(strusrQuota/1024,2),",","")  & "," & replace(formatnumber(strusrQuotaLimit/1024,2),",","") & "," & replace(formatnumber(strusrHardQuotaLimit/1024,2),",","") & "," & replace(formatnumber(mbsize/1024,2),",","") & "," & 0 & "%"
			end if
		else
			if strQuota <> 0 then
				wfile.writeline  strUsername & "," & replace(formatnumber(strQuota/1024,2),",","")  & "," & replace(formatnumber(strQuotaLimit/1024,2),",","") & "," & replace(formatnumber(strHardQuotaLimit/1024,2),",","") & "," & replace(formatnumber(mbsize/1024,2),",","") & "," & formatnumber((mbsize/strQuota)*100,2) & "%"
			else
				wfile.writeline  strUsername & "," & replace(formatnumber(strQuota/1024,2),",","")  & "," & replace(formatnumber(strQuotaLimit/1024,2),",","") & "," & replace(formatnumber(strHardQuotaLimit/1024,2),",","") & "," & replace(formatnumber(mbsize/1024,2),",","") & "," & 0 & "%"
			end if
		end if
