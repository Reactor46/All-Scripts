...
'For information about how to use the ROIScan.vbs script to collect data
' about Office installations and to help detect and troubleshoot installation and
' updates errors, see Collect data
' about Office installations by using Robust Office Inventory Scan.
 
'Changes for version 1.5.4
' - Added:   SP3 version detection for Office 2007 family
' - Added:   New 'Basic' mode. Comprehensive logging to show only the basics of the installed Office applications
' - Added:   Additional ARP data integrity check
' - Fixed:   License detection for some Visio configurations
' - Fixed:   Improved service pack level detection for several products
...
' Name: Robust Office Inventory Scan - Version 1.5.4 
...
' Supported Office Families: 2000 (Office 09), 2002 (Office 10), 2003 (Office 11) 
'                            2007 (Office 12), 2010 (Office 14) 
 
Option Explicit 
On Error Resume Next 
Const SCRIPTBUILD = "1.5.4" 
...
'Strings  
Dim sComputerName, sTemp, sCurUserSid, sDebugErr, sError, sErrBpa 
...
Dim arrVirtProducts(), arrPatch(), arrAipPatch, arrMspFiles, arrIS(), arrFeature() 
Dim arrLog(4), arrLogFormat(), arrUUSids(), arrUMSids() 
Dim arrProdVer09(),arrProdVer10(),arrProdVer11(),arrProdVer12(), arrProdVer14() 
Dim arrFiles() 
'Booleans 
...
Dim TextStream,ShellApp,AppFolder,Ospp 
...
Const WSS2                            = "7000-11D3-8CFE-0150048383C9}" 
...
Const ARRAY_ARP                       = 4 'Add/remove products datat array id 
    Const ARP_CHILDOFFSET             = 4 
' Config Productcode 
...
' Config PackageID 
    Const COL_CONFIGPACKAGEID         = 2 
    arrLogFormat(ARRAY_ARP,COL_CONFIGPACKAGEID)    = "Config PackageID" 
    Const COL_ARPALLPRODUCTS          = 3 
    Const COL_LBOUNDCHAINLIST         = 4 
...
Const BPA_WIMETADATA                  = "This indicates WI metadadata corruption" 
...
' Get computer specific properties 
    If fCScript AND NOT fQuiet Then wscript.echo "Stage 1 of 11: Get computer specific properties" 
    ComputerProperties  
...
' After this point the master array "arrMaster" is instantiated and has the basic product details 
    If fCScript AND NOT fQuiet Then wscript.echo "Stage 2 of 11: Find products" 
    FindAllProducts  
' Get additional product properties and add them to the master array 
    If fCScript AND NOT fQuiet Then wscript.echo "Stage 3 of 11: Find additional product properties" 
    ProductProperties  
' Build an array with data on the InstallSource(s) 
    If fCScript AND NOT fQuiet Then wscript.echo "Stage 4 of 11: Find registered InstallSources" 
...
' Only Office >= 2010 products that use OSPP are covered here 
    If fCScript AND NOT fQuiet Then wscript.echo "Stage 6 of 11: Check licensing (OSPP) data" 
    OsppCollect  
' Build an array with all patch data. 
    If fCScript AND NOT fQuiet Then wscript.echo "Stage 7 of 11: Find patches" 
    FindAllPatches 
    If fFeatureTree Then 
    ' Build a tree structure for the Features  
        If fCScript AND NOT fQuiet Then wscript.echo "Stage 8 of 11: Find Features and FeatureStates" 
        FindFeatureStates 
    Else 
...
' Create file inventory XML files 
    If fFileInventory Then 
        If fCScript AND NOT fQuiet Then wscript.echo "Stage 9 of 11: File Inventory" 
        FileInventory 
    Else 
...
'Ensure there's only a single instance running of this script 
    ...
    For Each Process in Processes 
        If LCase(Mid(Process.Name,2,6))="script" Then  
            If InStr(LCase(Process.CommandLine),"roiscan")>0 Then iInstanceCnt=iInstanceCnt+1 
        End If 
    Next 'Process 
    ...
    If Left(sPathOutputFolder,1)="%" Then 
        sEnvVar = oShell.ExpandEnvironmentStrings(Mid(sPathOutputFolder,1,InStr(2,sPathOutputFolder,"%")))  
        If Len(sPathOutputFolder) = InStr(2,sPathOutputFolder,"%") Then 
            sPathOutputFolder = sEnvVar 
        Else 
            sPathOutputFolder = sEnvVar &  Mid(sPathOutputFolder,InStr(2,sPathOutputFolder,"%")+1,Len(sPathOutputFolder)) 
        End If 
    ...
    'Validate Folder exists 
        If Not oFso.FolderExists(sPathOutputFolder) Then sPathOutputFolder = oShell.ExpandEnvironmentStrings("%TEMP%")  
    Else 
...
Sub ProductProperties 
    ...
    ' collect properties only for products in state '5' (Default) 
        If arrMaster(iPosMaster,COL_STATE) = INSTALLSTATE_DEFAULT Then 
            sProductCode = arrMaster(iPosMaster,COL_PRODUCTCODE) 
        ...
        ' cached .msi package  
            If NOT fBasicMode Then 
                ...
                If NOT arrMaster(iPosMaster,COL_CACHEDMSI) = "" Then Set MsiDb = oMsi.OpenDatabase(arrMaster(iPosMaster,COL_CACHEDMSI), MSIOPENDATABASEMODE_READONLY) 
            End If 'fBasicMode 
        ' ProductVersion 
            arrMaster(iPosMaster,COL_PRODUCTVERSION) = GetProductVersion(sProductCode,iContext,sSid) 
        ' PackageCode 
            If NOT fBasicMode Then arrMaster(iPosMaster,COL_PACKAGECODE) = GetPackageCode(sProductCode,iPosMaster,MsiDb) 
        ' UpgradeCode 
            If NOT fBasicMode Then arrMaster(iPosMaster,COL_UPGRADECODE) = GetUpgradeCode(MsiDb) 
        ' Transforms 
            If NOT fBasicMode Then arrMaster(iPosMaster,COL_TRANSFORMS) = GetTransforms(sProductCode) 
        ' InstallDate 
            If NOT fBasicMode Then arrMaster(iPosMaster,COL_INSTALLDATE) = GetInstallDate(sProductCode,iContext,sSid,arrMaster(iPosMaster,COL_CACHEDMSI)) 
        ' original .MSI Name 
            If NOT fBasicMode Then arrMaster(iPosMaster,COL_ORIGINALMSI) = GetOriginalMsiName(sProductCode) 
        ' some of this is only valid for Office products 
            ...
            ' Architecture (Bitness) 
                If Left(arrMaster(iPosMaster,COL_PRODUCTVERSION),2)>11 Then 
                    If Mid(sProductCode,21,1)="1" Then arrMaster(iPosMaster,COL_ARCHITECTURE) = "x64 (64bit)" Else arrMaster(iPosMaster,COL_ARCHITECTURE) = "x86 (32bit)" 
                End If 
                If NOT fBasicMode Then 
                ' Cx 
            ...
            ' POWERPIVOT_2010 
                If InStr(POWERPIVOT_2010,arrMaster(iPosMaster,COL_UPGRADECODE)) > 0 Then 
                    If CompareVersion(arrMaster(iPosMaster,COL_PRODUCTVERSION),"10.50.1747.0",True) Then 
                        arrMaster(iPosMaster,COL_NOTES) = arrMaster(iPosMaster,COL_NOTES) & ERR_CATEGORYWARN & "This is a beta version. Please obtain version 10.50.1747.0 or higher." & CSV 
                    End If 
...
Function GetOriginalMsiName(sProductCode) 
    Dim iPos 
    Dim sCompGuid, sRegName 
    On Error Resume Next 
 
    sRegName = "" 
    sRegName = oMsi.ProductInfo(sProductCode,"PackageName") 
    ...
    If (Not Err = 0) OR sRegName = "" Then 
        ...
        If sRegName = "-" Then arrMaster(iPos,COL_ERROR) = arrMaster(iPos,COL_ERROR) & ERR_CATEGORYERROR & ERR_BADMSINAMEMETADATA & CSV 
    End If 
...
'This limits the availability to products which are installed 'per-machine' or for the current user! 
'Exception situations WI < 3.x or user profile not available are covered in the 'Error handler' 
Function GetTransforms(sProductCode) 
    Dim sTransforms, sCompGuid, sRegTransforms 
    Dim iPos 
    On Error Resume Next 
 
    GetTransforms = "-" : sTransforms = "" 
    sTransforms = oMsi.ProductInfo(sProductCode,"Transforms") 
...
Function GetPackageCode(sProductCode,iPosMaster,MsiDb) 
    ...
    sPackageCode = "" 
    sPackageCode = oMsi.ProductInfo(sProductCode, "PackageCode") 
...
' ensure to not reference a non existent .msi 
    If sCachedMsi="" Then 
        arrMaster(iPosMaster,COL_ERROR) = arrMaster(iPosMaster,COL_ERROR) & ERR_CATEGORYERROR & ERR_BADPACKAGEMETADATA & CSV 
    Else 
...
'#Devonly   O12_Server = "1014,1015,104B,104E,1080,1088,10D7,10D8,10EB,10F5,10F6,10F7,10F8,10FB,10FC,10FD,1103,1104,110D,1105,1110,1121,1122"     
    ...
    Case 12 
        ...
        If (iLevel=0) AND (CInt(Mid(sProductVersion,6,4))>6219) AND (CInt(Mid(sProductVersion,6,4))<6425) Then 
    ...
    Case 14 
        ...
        If (iLevel=0) AND (CInt(Mid(sProductVersion,6,4))<4763) Then 
            ...
            Exit Function 
        End If 
        If (iLevel=0) AND (CInt(Mid(sProductVersion,6,4))>5999) AND (CInt(Mid(sProductVersion,6,4))<6029) Then 
...
' Office 2010 Products -> KB 
    ReDim arrProdVer14(6,3) 'n,1=RTM; n,2=SP1 
    arrProdVer14(0,0)="" : arrProdVer14(0,1)="14.0.4763.1000" : arrProdVer14(0,2)="14.0.6029.1000" ': arrProdVer14(0,3)="" 
...
Sub FindPatches (iPosMaster) 
    ...
    If iWiVersionMajor > 2 Then 
        Set Patches = oMsi.PatchesEx(arrMaster(iPosMaster,COL_PRODUCTCODE),arrMaster(iPosMaster,COL_USERSID),arrMaster(iPosMaster,COL_CONTEXT),MSIPATCHSTATE_ALL) 
        If Not Err = 0 Then 
            'PatchesEx API call failed 
            'Log the error 
            sErr = GetErrorDescription(Err) 
            If fPatchesExOk Then 
...
Sub AddDetailsFromPackage 
    ...
    Dim iPosMaster,iPosPatch,iCnt,iIndex 
    Dim fAdd 
    Dim qView 
    Dim arrTitle,arrTmpFam,arrTmpSeq,arrVer 
    ...
    For Each Key in dicMspTmp.Keys 
        ...
        Else 
            ...
            If InStr(arrMspFiles(iPosPatch,MSPFILES_TABLES),"MsiPatchSequence")>0 Then 
                ...
                If Not Record Is Nothing Then 
                    Do Until Record Is Nothing 
                        arrMspFiles(iPosPatch,MSPFILES_FAMILY) = arrMspFiles(iPosPatch,MSPFILES_FAMILY)&Record.StringData(1)&"," 
                        arrMspFiles(iPosPatch,MSPFILES_SEQUENCE) = arrMspFiles(iPosPatch,MSPFILES_SEQUENCE)&Record.StringData(2)&"," 
...
Sub FindArpConfigMsi 
    ...
    For n = 0 To UBound(arrArpProducts) 
        ...
        sName = "PackageIds" 
        If UCase(Left(sArpProd,9))="OFFICE14." Then  
            sName = "PackageRefs" 
    ...
    For n = 0 To UBound (arrMaster) 
        ...
        If arrMaster(n,COL_ISOFFICEPRODUCT) Then 
            If Not arrMaster(n,COL_SYSTEMCOMPONENT) = 0  AND arrMaster(n,COL_ARPPARENTCOUNT) = 0 Then 
                ...
                'Log warning except for AER and Rosebud 
                If NOT (Mid(arrMaster(n,COL_PRODUCTCODE),11,4)="0010" OR Mid(arrMaster(n,COL_PRODUCTCODE),11,4)="00B9") Then 
...
Sub FindArpParents 
    Dim ArpProd, ProductCode, Key 
    Dim n, i, j, iMaxVal, iArpCnt, iPosMaster 
    Dim bNoSysComponent, bOConfigEntry, fFoundConfigProductCode 
    Dim hDefKey 
    Dim sSubKeyName,sCurKey,sName,sValue,sNames,sArpProductName 
    Dim arrKeys, arrValues, arrTmpArp,arrNames,arrTypes 
    Dim tModulStart, tModulEnd 
    On Error Resume Next 
     
    iMaxVal = 0: iArpCnt = 0 : Redim arrTmpArp(-1) 
    hDefKey = HKEY_LOCAL_MACHINE 
    sSubKeyName = REG_ARP 
    ...
    If RegEnumKey (hDefKey,sSubKeyName,arrKeys) Then 
        For Each ArpProd in arrKeys 
            ...
            If InStr(sNames,"PackageIds") OR InStr(sNames,"PackageRefs") Then bOConfigEntry = True 
            ...
            If bOConfigEntry Then  
                ...
                If RegReadDWordValue(hDefKey,sCurKey,sName,sValue) Then 
                    If CInt(sValue) = 1 Then bOConfigEntry = False 
            ...
            Else 
                'Handle Click2Run 
                If Len(ArpProd) > 9 Then 
                    If UCase(Left(ArpProd,9)) = "OFFICE14." Then 
                        'Found ARP Parent entry 
                        Redim Preserve arrTmpArp(iArpCnt) 
                        arrTmpArp(iArpCnt) = Mid(ArpProd,10) 
                        iArpCnt = iArpCnt + 1 
                        Cachelog LOGPOS_RAW,LOGHEADING_NONE,Null,"ARP Entry: " & ArpProd 
                    End If 
                End If 'Len > 7 
...
Dim iArpCnt,iPosMaster,iLicCnt,iLicPos,iLic 
Dim sText,sOsppLicenses,sPossibleSkus,sPossibleSkusFull,sTmp 
Dim arrLicData 
...
If CheckArray(arrArpProducts) Then 
    For iArpCnt = 0 To UBound(arrArpProducts) 
        ...
        iPosMaster = GetArrayPosition(arrMaster,arrArpProducts(iArpCnt,ARP_CONFIGPRODUCTCODE)) 
            ...
            If (arrMaster(iPosMaster,COL_ISOFFICEPRODUCT)) AND CInt(GetVersionMajor(arrMaster(iPosMaster,COL_PRODUCTCODE)))>13 Then 
                ...
                sPossibleSkus = "" 
                'Loop all licenses 
                iLicCnt = GetLicCnt(iPosMaster,CInt(GetVersionMajor(arrMaster(iPosMaster,COL_PRODUCTCODE))),sText,sPossibleSkus,sPossibleSkusFull) 
                If iLicCnt > 0 Then 
                    ...
                    For iLic = 1 To iLicCnt 
                        ...
                        If arrLicData(OSPP_PRODUCTKEYID) <> "" Then 
                            ...
                            Select Case arrLicData(OSPP_LICENSESTATUS) 
...
Sub FindAllProducts 
    ...
    For i = 0 To UBound(arrMaster) 
        ...
        dicProducts.Add arrMaster(i,COL_PRODUCTCODE),arrMaster(i,COL_USERSID) 
    Next 'i 
    'Build an array of all virtualized applications (Click2Run) 
    FindVirtualizedProducts 
End Sub 
'======================================================================================================= 
...
If dicVirtProd.Count > 0 Then 
    ...
    For Each VProd in dicVirtProd.Keys 
        ...
        If IsOfficeProduct(VProd) Then 
            ...
            'Architecture (bitness) 
            arrVirtProducts(iVCnt,VIRTPROD_BITNESS) = "x86 (32bit)" 
            If Mid(VProd,21,1)="1" Then arrVirtProducts(iVCnt,VIRTPROD_BITNESS) = "x64 (64bit)" 
        End If 'IsOfficeProduct 
...
End Sub 'FindVirtualizedProducts 
'======================================================================================================= 
...
Function CheckPreReq () 
    ...
    If fIsAdmin Then 
        sSubKeyName = REG_GLOBALCONFIG & "S-1-5-18\" 
        For i = 1 To 4 
            Select Case i 
            Case 1 : lAccPermLevel = KEY_QUERY_VALUE 
            Case 2 : lAccPermLevel = KEY_SET_VALUE 
...
Sub WriteLog 
    ...
    Set LogOutput = oFso.OpenTextFile(sLogFile, FOR_WRITING, True, 0) 
...
'               LogOutput.Write arrLog(i) 
     '           LogOutput.Write vbCrLf & String(160,"*") & vbCrLf 
      ...
      '      If NOT (i = 2) Then LogOutput.Write vbCrLf & String(160,"*") & vbCrLf 
            If NOT (i = 2) Then LogOutput.Write  String(160,"*") 
        End If 
    Next 'i 
    LogOutput.Close 
...
Sub PrepareLog (sLogFormat) 
    ...
    Dim arrOrder(),arrTmp,arrTmpInner,dicTmp,arrLicData 
    On Error Resume Next 
    ...
    Redim arrOrder(17) 
    arrOrder(0)  = COL_PRODUCTVERSION 
    arrOrder(1)  = COL_SPLEVEL 
    arrOrder(2)  = COL_ARCHITECTURE 
    arrOrder(3)  = COL_PRODUCTCODE 
    arrOrder(4)  = COL_PRODUCTNAME 
    arrOrder(5)  = COL_ARPPARENTS 
    arrOrder(6)  = COL_INSTALLDATE 
    arrOrder(7)  = COL_USERSID 
    arrOrder(8)  = COL_CONTEXTSTRING 
    arrOrder(9)  = COL_STATESTRING 
    arrOrder(10) = COL_TRANSFORMS 
    arrOrder(11) = COL_ORIGINALMSI 
    arrOrder(12) = COL_CACHEDMSI 
    arrOrder(13) = COL_PRODUCTID 
    arrOrder(14) = COL_ORIGIN 
    arrOrder(15) = COL_PACKAGECODE 
    arrOrder(16) = COL_NOTES 
    arrOrder(17) = COL_ERROR 
...
' prepare Virtualized Products 
' ---------------------------- 
    If UBound(arrVirtProducts) > -1 Then 
        CacheLog LOGPOS_PRODUCT,LOGHEADING_H1,Null,"Products - Virtualized"  
        fLoggedVirt = True 
    ...
    'Prepare Office 12 & 14 Products 
    '------------------------------- 
    If CheckArray(arrArpProducts) Then 
        CacheLog LOGPOS_PRODUCT,LOGHEADING_H1,Null,"Multi .msi Products View"  
        fLoggedMulti = True 
...
Sub RelaunchAsCScript 
    ...
    sCmdLine = "cscript.exe " & Chr(34) & WScript.scriptFullName & Chr(34) 
    If Wscript.Arguments.Count > 0 Then 
        For Each Argument in Wscript.Arguments 
            sCmdLine = sCmdLine  &  " " & chr(34) & Argument & chr(34) 
        Next 'Argument 
...
Function GetArpProductname (sUninstallSubkey) 
    ...
    If RegReadExpStringValue(hDefKey,sSubKeyName,sName,sValue) Then  
        If Not sValue = "" Then GetArpProductname = sValue 
    End If 
...
Function GetRegOriginalMsiName(sProductCodeCompressed,iContext,sSid) 
    ...
    sFallBackName = "" 
    If sSid = sCurUserSid Or iContext = MSIINSTALLCONTEXT_MACHINE Or iContext = MSIINSTALLCONTEXT_MACHINE Then 
        hDefKey = GetRegHive(iContext,sSid,False) 
...
Function GetRegTransforms(sProductCodeCompressed,iContext,sSid) 
    ...
    If sSid = sCurUserSid Or iContext = MSIINSTALLCONTEXT_MACHINE Then 
        hDefKey = GetRegHive(iContext,sSid,False) 
...
Function GetRegProductName(sProductCodeCompressed,iContext,sSid) 
    ...
    sFallBackName = "" 
    If sSid = sCurUserSid Or iContext = MSIINSTALLCONTEXT_USERMANAGED Or iContext = MSIINSTALLCONTEXT_MACHINE Then 
        ...
        For iCnt = 0 To iArgCnt-1 
            ...
            Case "/L","-L","/LOGFOLDER","-LOGFOLDER","LOGFOLDER" 
                If iArgCnt > iCnt Then 
                    Set oFso = CreateObject("Scripting.FileSystemObject") 
                    If oFso.FolderExists(Wscript.Arguments(iCnt+1)) Then sPathOutputFolder = Wscript.Arguments(iCnt+1) 
                    Set oFso = Nothing 
                End If 'iArgCnt 
