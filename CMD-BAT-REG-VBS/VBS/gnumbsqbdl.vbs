...
GALQueryFilter =  "(&(mail=*)(objectCategory=msExchDynamicDistributionList))"
...
while not rs.eof
	...
	ioNumberIorgPersons = 0
	QbNumberQBDLs = 0
	Set qbdl = getobject("LDAP://" & replace(rs.fields("distinguishedName"),"/","\/"))
	trackmembers.add rs.fields("distinguishedName"),1
	enumQBDL(qbdl)
	wscript.echo qbdl.msExchDynamicDLBaseDN
...
if obdlobject.msExchDynamicDLBaseDN = "" then
	strQuerydl = "<LDAP://"  & strDefaultNamingContext & ">;" & obdlobject.msExchDynamicDLFilter & ";mail,ObjectClass,distinguishedName,displayname,legacyExchangeDN,homemdb;subtree"
else
	strQuerydl = "<LDAP://"  & obdlobject.msExchDynamicDLBaseDN & ">;" & obdlobject.msExchDynamicDLFilter & ";mail,ObjectClass,distinguishedName,displayname,legacyExchangeDN,homemdb;subtree"
end if
set com1 = createobject("ADODB.Command")
Com1.ActiveConnection = Conn
Com1.Properties("SearchScope") = 2          ' we want to search everything
Com1.Properties("Page Size") = 500          ' and we want our records in lots of 500 (must be < query limit)
wscript.echo strQuerydl
Com1.CommandText = strQuerydl
Set qdlRs = Com1.Execute
While Not qdlRS.eof
	Set member = getobject("LDAP://" & replace(qdlRS.fields("distinguishedName"),"/","\/"))
	if Not trackmembers.exists(member.distinguishedName) Then
		trackmembers.add member.distinguishedName,1
		wscript.echo member.class  & " " & member.displayname
		If member.mail <> "" then
