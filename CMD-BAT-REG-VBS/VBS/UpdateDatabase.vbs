...
oldDBName = node.getAttribute("oldDBName")
	...
	' Perform the Update here.
	MgmtDbCmd.CommandText = "Update adm_OtherBackupDatabases set DatabaseName = '" & newDBName & "', ServerName = '" & newDBServer & "', BTSServerName = '" & newDBServer & "' WHERE DefaultDatabaseName = '" & Name & "'"
	MgmtDbCmd.Execute

	' We also need to update the adm_OtherDatabases table as this is used to manage hotfix installments
	MgmtDbCmd.CommandText = "Update adm_OtherDatabases set DatabaseName = '" & newDBName & "', ServerName = '" & newDBServer & "', BTSServerName = '" & newDBServer & "' WHERE DefaultDatabaseName = '" & Name & "'"
	MgmtDbCmd.Execute

    ' Update the edi_DbConfig table if present
    if (name = "MsEDIAS2") then
        MgmtDbCmd.CommandText = "Update edi_DbConfig set EdiRuntimeDbServerName = '" & newDBServer & "' WHERE EdiRuntimeDbName = '" & Name & "'"
	    MgmtDbCmd.Execute
    end if

    ' BAM Alerts entries in the adm_OtherDatabases are not updated by the Restore method in the BAM section below.
'    if (name = "BAMAlertsApplication" or name = "BAMAlertsInstance") then
'        ' We need to update the adm_OtherDatabases table as this is used to manage hotfix installments
'	    MgmtDbCmd.CommandText = "Update adm_OtherDatabases set DatabaseName = " & newDBName & ", ServerName = " & newDBServer & "', BTSServerName = '" & newDBServer & "WHERE DefaultDatabaseName = '" & Name & "'"
'	    MgmtDbCmd.Execute
'    end if
next
...
if (oldRuleEngineDBServer <> "RuleEngineDBServerName" ) then
	MgmtDbCmd.CommandText = "Update adm_OtherBackupDatabases set DatabaseName = " & newRuleEngineDBName & ", ServerName = " & newRuleEngineDBServer & ", BTSServerName = " & newRuleEngineDBServer & " WHERE DefaultDatabaseName = 'RuleEngine DB'"
	MgmtDbCmd.Execute

	' We also need to update the adm_OtherDatabases table as this is used to manage hotfix installments
	MgmtDbCmd.CommandText = "Update adm_OtherDatabases set DatabaseName = " & newRuleEngineDBName & ", ServerName = " & newRuleEngineDBServer & ", BTSServerName = " & newRuleEngineDBServer & " WHERE DefaultDatabaseName = 'RuleEngine DB'"
	MgmtDbCmd.Execute
end if
